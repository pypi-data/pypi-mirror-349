...
def _tool_params(func: Callable[..., Any]):
    ...
    return [name for name, param in sig.parameters.items() if name != "self" and param.default is param.empty]
...
class ToolBox:
    ...
    def register(self, name: str, func: Callable[..., Any], description: str = ""):
        ...
        if not inspect.iscoroutinefunction(func):
            raise ValueError(f"decorated func <{name}> must be an async function.")
    ...
    def is_args_satisfied(self, func, args_dict):
        """
    ...
    def tool(cls, name: str | None = None, description: str = ""):
        ...
        def decorator(func: Any) -> Any:
            ...
            if not inspect.iscoroutinefunction(func):
                raise ValueError(f"decorated func <{func._tool_name}> must be an async function.")
