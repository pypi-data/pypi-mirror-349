...
class Agent:
    ...
    def serve(self) -> tuple[Flask, str]:
        ...
        # Create a new class that bridges back to the old style of class-based
        # MinecraftAgent. This allows us to use AgentManager.serve unchanged
        # while working through the details of what this API would look like.
        class BridgeAgent(MinecraftAgent):
            username = owner.name
            ...
            def init_participant(self, challenge_info: ChallengeInfo) -> InitParticipantResponse:
                ...
                context.api_client = owner.kradle._api_client
                context.update(owner.config)

                if owner._init_handler is not None:
                    owner._init_handler(challenge_info, context)

                event_names = list(owner._event_handlers.keys())
            ...
            def on_event(self, observation: Observation) -> OnEventResponse:
                ...
                event_handler = owner._event_handlers.get(event_key)
