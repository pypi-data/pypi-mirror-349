...
class ArrayTable(LabelArray2D, Table):
    def __init__(self, file:str, csv_dtype: DTypeEnum = DTypeEnum.FLOAT32):
        file_ = file.lower()
        ...
        if buffer is not None:
            ...
            with open(file, "r") as f:
                ...
                rows = len(content)
                f.seek(0)
                super().__init__(rows, cols, csv_dtype, labels, None)
                for i, line in enumerate(content):
                    values = _split_string_skip_quotes(line.rstrip())
                    for j, value in enumerate(values):
                        if csv_dtype in [DTypeEnum.INT8, DTypeEnum.INT16, DTypeEnum.INT32, DTypeEnum.INT64]:
                            self._d[i][j] = csv_dtype.value(int(value))
                        else:
                            self._d[i][j] = csv_dtype.value(float(value)) # type: ignore
