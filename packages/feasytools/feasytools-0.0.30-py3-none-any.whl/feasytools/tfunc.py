...
class SegFunc(TimeFunc):
    ...
    def interpolate(self, tl:int, tr:int) -> "SegFunc":
        ...
        if len(self._tl) == 0:
            ret.add(tl, 0)
            return ret
        #assert self._tl[0] == tl, f"TimeSeg must start at {tl}, but starts at {self._tl[0]}"
    ...
    def values_at(self,times:'List[int]')->List[float]:
        ...
        for time in times:
            while i < len(self._tl) and self._tl[i] < time:
                i += 1
            if i == len(self._tl):
                if len(self._d) > 0:ret.append(self._d[-1])
                else: ret.append(0)
            elif self._tl[i] == time:
                if len(self._d) > 0:ret.append(self._d[i])
                else: ret.append(0)
            else:
                if i == 0:
                    ret.append(self._d[0])
                else:
    ...
    def value_at(self,time:int) -> float:
        ...
        i = bisect.bisect_left(self._tl, time)
        if i == len(self._tl):
            return self._d[-1]
        return self._d[i]
