...
class ConnexityTwilioObserver(BaseObserver):
    ...
    async def on_push_frame(
        ...
        if isinstance(frame, BotStartedSpeakingFrame) and isinstance(src,
            ...
            self.assistant_data['start'] = time_sec
        elif isinstance(frame, BotStoppedSpeakingFrame) and isinstance(src,
                                                                       FastAPIWebsocketOutputTransport) and direction == FrameDirection.DOWNSTREAM:
            self.assistant_data['end'] = time_sec
        ...
        if isinstance(frame, FunctionCallInProgressFrame) and isinstance(src,
                                                                         FastAPIWebsocketOutputTransport) and direction == FrameDirection.DOWNSTREAM:
            self.tool_calls['start'] = time_sec
        ...
        if self.user_data.get('start') and self.user_data.get('end') and self.user_data.get('content'):
        ...
        if self.assistant_data.get('start') and self.assistant_data.get('end') and self.assistant_data.get('content'):
            latency = None
        ...
        if isinstance(frame, CancelFrame) and isinstance(src, FastAPIWebsocketOutputTransport):
