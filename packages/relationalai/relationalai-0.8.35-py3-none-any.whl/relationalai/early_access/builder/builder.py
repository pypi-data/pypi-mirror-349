    ...
    def _ref_scheme_hierarchy(self):
        ...
        for ix, scheme in enumerate(ref_schemes[1:]):
            ...
            if not self._scheme_mapping.get(parent):
                self._scheme_mapping[parent] = Relationship(f"{{{cur._name}}} to {{{parent._name}}}", short_name=f"{cur._name}_to_{parent._name}", model=self._model)
...
class Relationship(Producer):
    ...
    def __getitem__(self, arg:str|int|Concept) -> Any:
        if isinstance(arg, int):
            ...
            if self._arity() <= arg:
                raise ValueError(f"Relationship '{self._name}' has only {self._arity()} fields")
    ...
    def __call__(self, *args: Any, **kwargs) -> Any:
        ...
        if kwargs:
            ...
            clean_args = list(args)
        if len(clean_args) < self._arity():
            if self._parent:
                clean_args = [self._parent, *clean_args]
        if len(clean_args) != self._arity():
            raise ValueError(f"Expected {self._arity()} arguments, got {len(clean_args)}")
...
class Compiler():
    ...
    def construct_dict(self, items:dict[Relationship|Concept, Producer], ctx:CompilerContext) -> dict[ir.Relation, list[ir.Var]]:
    ...
    def explode_ref_schemes(self, item:ConceptExpression, ctx:CompilerContext, update=False):
        ...
        if not hierarchy:
            ...
            ctx.add(f.construct(out, self.construct_dict(item._construct_args(), ctx)))
            return out
        ...
        for ix, info in enumerate(hierarchy):
            ...
            ctx.add(f.construct(cur, self.construct_dict(item._construct_args(scheme), ctx)))
            if not out:
                out = cur
            if info.get("mapping"):
