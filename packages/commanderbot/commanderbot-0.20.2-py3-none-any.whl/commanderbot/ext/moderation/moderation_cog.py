...
class ModerationCog(Cog, name="commanderbot.ext.moderation"):
    ...
    def _create_kick_status_embed(
        self, user: Member, reason: Optional[str], sent_dm: bool, kicked: bool
    ) -> Embed:
        # Create list of actions taken
        actions_taken: list[str] = []
        if reason:
            if sent_dm:
                actions_taken.append("- ✉️ DM sent")
            else:
                actions_taken.append("- 🔥 An error occurred while sending the DM")

        if kicked:
            actions_taken.append("- 👢 Kicked user")
        else:
            actions_taken.append("- 🔥 An error occurred while kicking the user")

        # Create embed
        embed = Embed(description=f"### 👢 Kicked {user.mention}", color=0x00ACED)
        embed.add_field(
    ...
    def _create_ban_status_embed(
        self, user: Member, reason: Optional[str], sent_dm: bool, banned: bool
    ) -> Embed:
        return Embed()

    def _create_kick_compromised_status_embed(
        self, user: Member, reason: str, sent_dm: bool, banned: bool, unbanned: bool
    ) -> Embed:
        return Embed()
    ...
    async def cmd_kick(
        ...
        if reason:
            try:
                ...
                dm_kick_embed = Embed(
                    description=f"### 👢 You were kicked from {guild_name}",
                    color=0x00ACED,
                )
