...
class StableAptamerGenerator:
    ...
    def evaluate_aptamer(self, sequence: str) -> float:
    ...
    def mutate_aptamer(self, aptamer, mutation_rate=0.3, indel_rate=0.1, min_len=10, max_len=40):
        new_seq = []
        for nt in aptamer:
    ...
    def run(self):
        ...
        for gen in range(self.generations):
            ...
            min_len = max(min(lengths) - 1, 10)
            max_len = min(max(lengths) + 1, 50)
