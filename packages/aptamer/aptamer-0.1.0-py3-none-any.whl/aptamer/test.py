import pytest
from aptamer.selex import StableAptamerGenerator
...
def test_generate_random_aptamers():
    ...
    for aptamer in pool:
        ...
        assert all(nt in NUCLEOTIDES for nt in aptamer)

def test_evaluate_aptamer():
    gen = StableAptamerGenerator()
    aptamer = "GATCGATC"
    mfe = gen.evaluate_aptamer(aptamer)
...
def test_mutate_aptamer_length_and_content():
    ...
    assert all(nt in NUCLEOTIDES for nt in mutated)
