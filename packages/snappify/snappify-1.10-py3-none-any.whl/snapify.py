...
from pathlib.parse import urlparse, unquote
...
async def make_session():
    ...
    if PRIVATE_AUTH:
        ...
        if "_sc-sid" not in names:
            logging.warning("`_sc-sid` cookie not found! Private stories may fail.")
        logging.debug(f"Cookie names: {names}")

        # Build a jar with those cookies
        ...
        for c in cookies:
            ...
            if name and val:
                ...
                logging.debug(f"Set cookie {name} for domain {domain}")
...
async def process_username(username, session, data, force_zip=False):
    ...
    story_urls = [s["snapUrls"]["mediaUrl"] for s in snaps if s.get("snapUrls", {}).get("mediaUrl")]
    ...
    highlight_pairs = [
        (title, u) for title in highlight_keys for u in media_map.get(title, [])
    ]
