...
ZIP_THRESHOLD   = 35      # auto-zip over this many files
...
async def process_username(username, session, data, force_zip=False):
    ...
    # Decide whether to zip
    should_zip = force_zip or len(files) > ZIP_THRESHOLD
    if should_zip:
...
def main():
    ...
    parser.add_argument(
        ...
        help="always bundle downloaded snaps into a ZIP"
    )
    parser.add_argument(
    ...
    # On config change without “start”: save + one-off downloads
    if changed and args.command != "start":
        ...
        print(Fore.CYAN + "  snapify start" + (args.zip and " -z") + "\n")
        sys.exit(0)
