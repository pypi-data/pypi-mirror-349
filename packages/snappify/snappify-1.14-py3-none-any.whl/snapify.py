...
# —————— Fetch and download friend stories ——————————————————
async def fetch_friend_stories(session):
    url = "https://story.snapchat.com/api/v1/friends_stories"
    async with session.get(url) as resp:
        if resp.status == 200:
            return await resp.json()
        else:
            logging.error(f"Failed to fetch friend stories: HTTP {resp.status}")
            return []

async def download_friend_stories(session):
    friends = await fetch_friend_stories(session)
    for friend in friends:
        username = friend.get("username")
        stories = friend.get("stories", [])
        for story in stories:
            media_url = story.get("media_url")
            if media_url:
                dest = os.path.join("snap_media", "friends", username)
                await download_media_to_dir(session, media_url, dest)

# —————— Build an aiohttp session, with optional private cookies ——————————————————
async def make_session():
    headers = {"User-Agent": "Mozilla/5.0"}
    if PRIVATE_AUTH:
        path = COOKIE_FILE or input("Path to your cookies.json for private auth: ").strip()
        if not os.path.exists(path):
            logging.error(f"Cookie file not found: {path}")
            sys.exit(1)
        ...
        for c in cookies:
            ...
            if name and val:
                ...
                names.append(name)
        if "__Host-X-Snap-Client-Cookie" not in names:
            logging.warning("`__Host-X-Snap-Client-Cookie` cookie not found! Private stories may fail.")
        session = aiohttp.ClientSession(cookie_jar=jar, headers=headers)

        # test login
        try:
            async with session.get("https://story.snapchat.com/") as resp:
                if resp.status == 200:
                    print("Login successful")
                    # immediately fetch & download all friend stories
                    await download_friend_stories(session)
                else:
                    print(f"Login failed (HTTP {resp.status})")
                    text = await resp.text()
                    with open("login_error.txt", "w") as ef:
                        ef.write(f"Status: {resp.status}\n\n{text}")
                    sys.exit(1)
        except Exception as e:
...
def main():
    ...
    parser.add_argument("-p", "--private", action="store_true",
        help="enable private story download (requires cookies.json)")
    parser.add_argument("--cookie-file",
        help="path to cookies.json for private auth")
