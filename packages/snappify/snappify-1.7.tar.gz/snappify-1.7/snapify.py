...
async def make_session():
    ...
    if PRIVATE_AUTH:
        ...
        if not os.path.exists(path):
            print(Fore.RED + f"Cookie file not found: {path}")
            sys.exit(1)
        try:
            cookies = json.load(open(path))
        except Exception as e:
            print(Fore.RED + f"Failed to load cookies.json: {e}")
            sys.exit(1)
        ...
        for c in cookies:
            ...
            if name and val:
                jar.update_cookies({name: val}, response_url=f"https://{domain}")
...
async def get_json(session, username):
    ...
    async with session.get(url) as resp:
        ...
        if resp.status in (401, 403):
            print(Fore.RED + f"{username}: access denied (private?).\n"
                              "Make sure your cookies.json is valid, and check your email for verification links.")
            sys.exit(1)
...
async def process_username(username, session, data, force_zip=False):
    ...
    if new_story:
        ...
        tasks = [download_media_to_dir(session, u, os.path.join("snap_media", username, "stories")) for u in new_story]
    ...
    spotlight_urls = [url for key, lst in media_map.items() if "spotlight" in key.lower() for url in lst]
    ...
    if new_spot:
        ...
        tasks = [download_media_to_dir(session, u, dest) for u in new_spot]
        results = await asyncio.gather(*tasks)
        ...
        if spot_files:
            if force_zip:
                ...
                for f in spot_files:
                    os.remove(f)
                print(Fore.MAGENTA + f"{username}: downloaded {len(spot_files)} spotlight snaps (zipped → {z})")
            else:
    ...
    high_files = []
    if new_high:
        print(Fore.GREEN + f"{username}: found {len(new_high)} new highlight item(s), downloading…")
        tasks = []
        ...
        if high_files:
            if force_zip:
                ...
                for f in high_files:
                    os.remove(f)
                print(Fore.MAGENTA + f"{username}: downloaded {len(high_files)} highlight snaps (zipped → {z})")
            else:
