        ...
        try:
            ...
            for r in self.__routes:
                ...
                if route_match:
                    ...
                    view = r["view"](page=self.__page)

                    if not hasattr(view, "state") or view.state is None:
                        if type(self.__state) == FlowViewFtState:
            ...
            # Not Found View
            view = self.__not_found_view(self.__page)
            if not hasattr(view, "state") or view.state is None:
                if type(self.__state) == FlowViewFtState:
                    view.state = self.__state
            view.params = FlowViewFtParams()
            ...
            view = self.__internal_error_view(self.__page)
            if not hasattr(view, "state") or view.state is None:
                if type(self.__state) == FlowViewFtState:
                    view.state = self.__state
            view.params = FlowViewFtParams()
