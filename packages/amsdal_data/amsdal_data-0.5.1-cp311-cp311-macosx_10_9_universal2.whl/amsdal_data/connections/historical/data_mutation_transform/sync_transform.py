...
class DataMutationTransform(BaseDataMutationTransform):
    ...
    def _transform_insert_data(self, mutation: glue.InsertData) -> Sequence[DataMutation]:
        ...
        return self._build_insert_mutations(mutation.schema, mutation.data, self.connection.table_schema)
    ...
    def _transform_delete_data(self, mutation: glue.DeleteData) -> Sequence[DataMutation]:
        ...
        return self._build_insert_mutations(mutation.schema, stored_items, self.connection.table_schema)
    ...
    def _resolve_object_versions(
        ...
        if prior_version is not None:
            if schema_reference.name == BaseClasses.CLASS_OBJECT.value or (
                ...
                self.historical_table_schema.copy_table_schema_from_prior_class_version(
    ...
    @classmethod
    def _build_insert_mutations(
        cls,
        schema: glue.SchemaReference,
        data: list[glue.Data],
        table_schema: HistoricalTableSchema,
    ) -> list[glue.InsertData]:
        TableNameTransform.process_table_name(schema, table_schema)
        _mutations = []
