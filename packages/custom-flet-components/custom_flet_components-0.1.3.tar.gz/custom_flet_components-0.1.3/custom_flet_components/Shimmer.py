...
class ShimmerWidget(ft.Container):
    def __init__(self, control, page: ft.Page, loading_duration=3):
        """
        A reusable Flet component that applies a shimmer effect to a control and auto-transitions
        to the actual content after a specified loading duration.
        ...
        self.control = self._mark_shimmer_data(control)
        self.page = page
        self.loading_duration = loading_duration
        self.content = Shimmer(control=self.control, auto_generate=True)  # Start with shimmer

        # Automatically start the shimmer display process
        self.page.run_task(self._show)

    def _mark_shimmer_data(self, control):
        """
        ...
        if hasattr(control, "content") and control.content:
            self._mark_shimmer_data(control.content)
        if hasattr(control, "controls") and control.controls:
            for ctrl in control.controls:
                self._mark_shimmer_data(ctrl)
        return control
...
#     # Create and add ShimmerWidget (no .build() or run_task needed)
#     page.add(ShimmerWidget(control=content, page=page, loading_duration=3))
