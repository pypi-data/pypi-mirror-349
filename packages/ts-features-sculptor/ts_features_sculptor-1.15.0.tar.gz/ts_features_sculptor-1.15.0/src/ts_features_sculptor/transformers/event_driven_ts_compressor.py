...
class EventDrivenTSCompressor(BaseEstimator, TransformerMixin, TimeValidator):
    ...
    - лаг (в днях) от последней записи в базовом временном ряде до
      начала события (`<event_name>_lag_days`).
    ...
    def transform(self, X_):
        ...
        lag_days = (starts - last_times) / np.timedelta64(1,'D')

        ## событие не принято
        mask_empty = counts == 0
        if mask_empty.any():
            idx_next_after_start = np.searchsorted(
                times, starts, side="left"
            )
            next_times = np.full_like(starts, np.datetime64("NaT"))
            valid = idx_next_after_start < len(times)
            next_times[valid] = times[idx_next_after_start[valid]]

            lag_to_next = (
                    (next_times - starts) / np.timedelta64(1, "D")
            )
            lag_days[mask_empty] = lag_to_next[mask_empty]
