...
def compute_map(model, x_val, y_val, conf_threshold, iou_threshold, NUM_CLASSES, GRID_SIZE, NUM_BOXES, IMAGE_SIZE):
    ...
    for i in range(0, x_val.shape[0], 1):
        ...
        all_classes.append(cp.array(classes) if classes else None)
    ...
    for class_id in range(NUM_CLASSES):
        ...
        for i in range(len(all_boxes)):
            ...
            class_mask = pred_classes == class_id if pred_classes is not None else cp.array([])
            class_boxes = pred_boxes[class_mask] if pred_boxes is not None else None
            class_scores = pred_scores[class_mask] if pred_scores is not None else None
