...
from tesslate_chainlit.auth import create_jwt, decode_jwt, get_configuration, get_current_user
from tesslate_chainlit.auth.cookie import (
    clear_auth_cookie,
...
from tesslate_chainlit.config import (
    APP_ROOT,
    BACKEND_ROOT,
    DEFAULT_HOST,
    FILES_DIRECTORY,
...
from tesslate_chainlit.data import get_data_layer
from tesslate_chainlit.data.acl import is_thread_author
from tesslate_chainlit.logger import logger
from tesslate_chainlit.markdown import get_markdown_str
from tesslate_chainlit.oauth_providers import get_oauth_provider
from tesslate_chainlit.secret import random_secret
from tesslate_chainlit.types import (
    CallActionRequest,
    ConnectMCPRequest,
...
async def lifespan(app: FastAPI):
    ...
    if config.run.watch:
        ...
        async def watch_files_for_changes():
            extensions = [".py"]
            files = ["tesslate_chainlit.md", "config.toml"]
    ...
    if discord_bot_token := os.environ.get("DISCORD_BOT_TOKEN"):
        from tesslate_chainlit.discord.app import client
...
if os.environ.get("SLACK_BOT_TOKEN") and os.environ.get("SLACK_SIGNING_SECRET"):
    from tesslate_chainlit.slack.app import slack_app_handler
...
if os.environ.get("TEAMS_APP_ID") and os.environ.get("TEAMS_APP_PASSWORD"):
    ...
    from tesslate_chainlit.teams.app import adapter, bot
...
def get_html_template(root_path):
    ...
    default_url = "https://github.com/tesslate_chainlit/tesslate_chainlit"
    default_meta_image_url = (
        "https://tesslate_chainlit-cloud.s3.eu-west-3.amazonaws.com/logo/chainlit_banner.png"
    )
...
async def set_session_cookie(request: Request, response: Response):
    ...
    response.set_cookie(
        key="X-tesslate_chainlit-Session-id",
        value=session_id,
        path="/",
        httponly=True,
        secure=not is_local,
...
async def update_thread_element(
    ...
    from tesslate_chainlit.context import init_ws_context
    from tesslate_chainlit.element import Element, ElementDict
    from tesslate_chainlit.session import WebsocketSession
...
async def delete_thread_element(
    ...
    from tesslate_chainlit.context import init_ws_context
    from tesslate_chainlit.element import CustomElement, ElementDict
    from tesslate_chainlit.session import WebsocketSession
...
async def connect_mcp(
    ...
    from tesslate_chainlit.context import init_ws_context
    from tesslate_chainlit.mcp import SseMcpConnection, StdioMcpConnection, validate_mcp_command
    from tesslate_chainlit.session import WebsocketSession
...
import tesslate_chainlit.socket  # noqa
