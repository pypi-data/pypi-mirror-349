...
class DataManager(BaseModel):
    ...
    """
    # Data storage
    nodes: pd.DataFrame = Field(default_factory=pd.DataFrame, description="DataFrame containing node data.")
    edges: pd.DataFrame = Field(default_factory=pd.DataFrame, description="DataFrame containing edge data.")
    ...
    year_col: Optional[str] = Field(None, description="Column name for years; auto-detected.")
    ...
    available_years: List[int] = Field(default_factory=list, description="Available years in the data.")
    node_types: List[str] = Field(default_factory=list, description="List of node types.")
    edge_types: List[str] = Field(default_factory=list, description="List of edge types.")
    
    # File paths
    data_dir: Path = Field(default=DATA_DIR, description="Directory for data storage.")
    nodes_path: Optional[Union[str, Path]] = Field(None, description="Path to nodes file.")
    edges_path: Optional[Union[str, Path]] = Field(None, description="Path to edges file.")
    ...
    def _setup(self):
        ...
        logger.info("DataManager ready: %d nodes, %d edges", len(self.nodes), len(self.edges))
        return self
    ...
    def load_example_data(self) -> None:
        ...
        if not nodes_path.exists() or not edges_path.exists():
            logger.error(f"Example data files do not exist. Expected at {nodes_path} and {edges_path}.")
        ...
        self._detect_years()
        self._detect_types()
        logger.info("Loaded example data: %d nodes, %d edges", len(self.nodes), len(self.edges))
    ...
    def _detect_years(self) -> None:
        ...
        # Look for columns that might contain year information
            ...
            year_columns = [c for c in cols if re.search(r"\byear\b|date|time", c, re.IGNORECASE)]
            if year_columns:
                col = year_columns[0]
        ...
        if col:
            ...
            try:
                ...
                logger.info("Detected year column '%s' with %d years", col, len(self.available_years))
            except (ValueError, TypeError):
                # If conversion to int fails, try to extract years from date strings
                logger.warning("Year column '%s' contains non-integer values, attempting date parsing", col)
                try:
                    ...
                    logger.info("Extracted %d years from date column '%s'", len(self.available_years), col)
                except:
    ...
    def _derive_nodes_if_needed(self) -> None:
        ...
        if self.nodes.empty and not self.edges.empty:
            ...
            ids = pd.Series(
                pd.concat([
                    self.edges[self.src_col],
                    self.edges[self.tgt_col]
                ]).dropna().unique(),
                name="id"
            )
    ...
    def save_data(self, nodes_name: str = 'nodes', edges_name: str = 'edges', format: str = 'csv') -> None:
        """
        ...
        if format == 'csv':
            ...
            self.nodes.to_json(self.data_dir / f"{nodes_name}.json", orient='records', lines=True)
            self.edges.to_json(self.data_dir / f"{edges_name}.json", orient='records', lines=True)
        elif format == 'xlsx':
    ...
    def filter_by_year(self, year: int) -> 'DataManager':
        ...
        if year not in self.available_years:
            logger.warning(f"Year {year} not in available years: {self.available_years}")
            return self
        ...
        logger.info(f"Filtered data by year {year}: {len(new_manager.edges)} edges, {len(new_manager.nodes)} nodes")
        return new_manager
    
    def filter_by_type(self, node_type: Optional[str] = None, edge_type: Optional[str] = None) -> 'DataManager':
        """
        ...
        if node_type and 'type' in self.nodes.columns:
            if node_type not in self.node_types:
                logger.warning(f"Node type {node_type} not in available types: {self.node_types}")
            else:
        ...
        if edge_type and 'type' in self.edges.columns:
            if edge_type not in self.edge_types:
                logger.warning(f"Edge type {edge_type} not in available types: {self.edge_types}")
            else:
        ...
        logger.info(f"Filtered data by types: {len(new_manager.edges)} edges, {len(new_manager.nodes)} nodes")
        return new_manager

    def to_networkx(self, directed: bool = True, weight_attr: Optional[str] = None, 
                    node_id_col: Optional[str] = None, filter_year: Optional[int] = None) -> nx.Graph:
        """
        ...
        if filter_year is not None and self.has_year and filter_year in self.available_years:
            edges_to_use = self.edges[self.edges[self.year_col] == filter_year]
            logger.info(f"Filtered edges by year {filter_year}: {len(edges_to_use)} edges")
        ...
        if not self.nodes.empty:
            ...
            name_col = self.label_col if self.label_col and self.label_col in self.nodes.columns else 'country'
            ...
            if name_col in self.nodes.columns:
                ...
                if filter_year is not None and self.has_year and 'year' in self.nodes.columns:
                    nodes_to_use = self.nodes[self.nodes['year'] == filter_year]
                    logger.info(f"Filtered nodes by year {filter_year}: {len(nodes_to_use)} nodes")
                
                # Create mapping of country names to their IDs
                for _, row in nodes_to_use.iterrows():
                    if row[name_col] in all_nodes:  # Only include nodes that appear in edges
        ...
        if not edges_to_use.empty:
            ...
            for _, row in edges_to_use.iterrows():
                ...
                source_id = name_to_id.get(source, source) if not self.nodes.empty else source
                target_id = name_to_id.get(target, target) if not self.nodes.empty else target
    ...
    def from_networkx(self, G: nx.Graph) -> None:
        ...
        logger.info(f"Loaded data from NetworkX graph: {len(self.nodes)} nodes, {len(self.edges)} edges")
    ...
    def data_summary(self) -> None:
        ...
        if self.has_year:
            ...
            year_table.add_row("Available Years", ", ".join(map(str, self.available_years)))
...
if __name__ == "__main__":
    ...
    G = dm1.to_networkx()
    console.print(f"Created graph with {G.number_of_nodes()} nodes and {G.number_of_edges()} edges")
