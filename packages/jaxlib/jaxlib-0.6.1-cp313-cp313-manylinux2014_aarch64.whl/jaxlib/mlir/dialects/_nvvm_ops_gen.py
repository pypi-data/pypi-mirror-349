...
@_ods_cext.register_operation(_Dialect)
class CvtBF16x2ToF8x2Op(_ods_ir.OpView):
  OPERATION_NAME = "nvvm.cvt.bf16x2.to.f8x2"
  ...
  def __init__(self, dst, type_, a, *, rnd=None, sat=None, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('CVTFP8TypeAttr')) else
      _ods_ir.AttrBuilder.get('CVTFP8TypeAttr')(type_, context=_ods_context))
...
def cvt_bf16x2_to_f8x2(dst, type_, a, *, rnd=None, sat=None, loc=None, ip=None) -> _ods_ir.Value:
  return CvtBF16x2ToF8x2Op(dst=dst, type_=type_, a=a, rnd=rnd, sat=sat, loc=loc, ip=ip).result

@_ods_cext.register_operation(_Dialect)
class CvtF16x2ToF8x2Op(_ods_ir.OpView):
  OPERATION_NAME = "nvvm.cvt.f16x2.to.f8x2"
...
def cvt_f16x2_to_f8x2(dst, type_, a, *, relu=None, loc=None, ip=None) -> _ods_ir.Value:
  return CvtF16x2ToF8x2Op(dst=dst, type_=type_, a=a, relu=relu, loc=loc, ip=ip).result

@_ods_cext.register_operation(_Dialect)
class CvtF32x2ToF6x2Op(_ods_ir.OpView):
  OPERATION_NAME = "nvvm.cvt.f32x2.to.f6x2"
  ...
  def __init__(self, dst, type_, a, b, *, relu=None, loc=None, ip=None):
    ...
    attributes["type"] = (type_ if (
    isinstance(type_, _ods_ir.Attribute) or
    not _ods_ir.AttrBuilder.contains('CVTFP6TypeAttr')) else
      _ods_ir.AttrBuilder.get('CVTFP6TypeAttr')(type_, context=_ods_context))
    if relu is not None: attributes["relu"] = (relu if (
        isinstance(relu, _ods_ir.Attribute) or
        not _ods_ir.AttrBuilder.contains('BoolAttr')) else
          _ods_ir.AttrBuilder.get('BoolAttr')(relu, context=_ods_context))
...
def cvt_f32x2_to_f6x2(dst, type_, a, b, *, relu=None, loc=None, ip=None) -> _ods_ir.Value:
  return CvtF32x2ToF6x2Op(dst=dst, type_=type_, a=a, b=b, relu=relu, loc=loc, ip=ip).result

@_ods_cext.register_operation(_Dialect)
class CvtF32x2ToF8x2Op(_ods_ir.OpView):
  OPERATION_NAME = "nvvm.cvt.f32x2.to.f8x2"
...
def cvt_f32x2_to_f8x2(dst, type_, a, b, *, rnd=None, sat=None, relu=None, loc=None, ip=None) -> _ods_ir.Value:
  return CvtF32x2ToF8x2Op(dst=dst, type_=type_, a=a, b=b, rnd=rnd, sat=sat, relu=relu, loc=loc, ip=ip).result
...
class DotAccumulate4WayOp(_ods_ir.OpView):
  ...
  def __init__(self, res, a, a_type, b, b_type, c, *, loc=None, ip=None):
    ...
    not _ods_ir.AttrBuilder.contains('DotAccumulate4WayTypeAttr')) else
      _ods_ir.AttrBuilder.get('DotAccumulate4WayTypeAttr')(a_type, context=_ods_context))
    attributes["b_type"] = (b_type if (
    isinstance(b_type, _ods_ir.Attribute) or
    not _ods_ir.AttrBuilder.contains('DotAccumulate4WayTypeAttr')) else
      _ods_ir.AttrBuilder.get('DotAccumulate4WayTypeAttr')(b_type, context=_ods_context))
    results.append(res)
    _ods_successors = None
    super().__init__(self.OPERATION_NAME, self._ODS_REGIONS, self._ODS_OPERAND_SEGMENTS, self._ODS_RESULT_SEGMENTS, attributes=attributes, results=results, operands=operands, successors=_ods_successors, regions=regions, loc=loc, ip=ip)
