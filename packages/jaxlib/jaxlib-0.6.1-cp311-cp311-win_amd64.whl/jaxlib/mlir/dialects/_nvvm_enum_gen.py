...
class CVTFP6Type(IntEnum):
    """NVVM CVTFP6Type kind"""

    E2M3 = 0
    E3M2 = 1

    def __str__(self):
        if self is CVTFP6Type.E2M3:
            return "e2m3"
        if self is CVTFP6Type.E3M2:
            return "e3m2"
        raise ValueError("Unknown CVTFP6Type enum entry.")



@register_attribute_builder("CVTFP6Type")
def _cvtfp6type(x, context):
    return _ods_ir.IntegerAttr.get(_ods_ir.IntegerType.get_signless(32, context=context), int(x))

class CVTFP8Type(IntEnum):
    """NVVM CVTFP8Type kind"""
    ...
    def __str__(self):
        if self is CVTFP8Type.E4M3:
            return "e4m3"
        if self is CVTFP8Type.E5M2:
            return "e5m2"
        if self is CVTFP8Type.UE8M0:
            return "ue8m0"
        raise ValueError("Unknown CVTFP8Type enum entry.")



@register_attribute_builder("CVTFP8Type")
def _cvtfp8type(x, context):
    return _ods_ir.IntegerAttr.get(_ods_ir.IntegerType.get_signless(32, context=context), int(x))

class DotAccumulate4WayType(IntEnum):
    ...
    def __str__(self):
        if self is DotAccumulate4WayType.S8:
            return "s8"
        if self is DotAccumulate4WayType.U8:
            return "u8"
        raise ValueError("Unknown DotAccumulate4WayType enum entry.")



@register_attribute_builder("DotAccumulate4WayType")
def _dotaccumulate4waytype(x, context):
    return _ods_ir.IntegerAttr.get(_ods_ir.IntegerType.get_signless(32, context=context), int(x))
...
@register_attribute_builder("CVTFP6TypeAttr")
def _cvtfp6typeattr(x, context):
    return _ods_ir.Attribute.parse(f'#nvvm.cvt_fp6_type<{str(x)}>', context=context)

@register_attribute_builder("CVTFP8TypeAttr")
def _cvtfp8typeattr(x, context):
    return _ods_ir.Attribute.parse(f'#nvvm.cvt_fp8_type<{str(x)}>', context=context)

@register_attribute_builder("DotAccumulate4WayTypeAttr")
def _dotaccumulate4waytypeattr(x, context):
    return _ods_ir.Attribute.parse(f'#nvvm.dot_accumulate_4way_type<{str(x)}>', context=context)
