...
class APIGetDetailView(BaseAPIGetView):
    ...
    def get_data_res(self):
        return ModelToJson(model=self.model).get_data_list(
            self.get_queryset(), self.page_field_list, detail_field_list=self.detail_field_list,
            query_params=self.query_params, order_by_params=self.order_by_params)
...
class ModelToJson:
    ...
    def get_data_list(self, queryset, field_list: List[BaseReturnField],
        ...
        related_list = self.__get_related_list(all_field_list)
        if related_list:
            select_related_list = []
        ...
        model_field_list = self.__get_only_field_name_list(all_field_list)
    ...
    def get_detail(self, queryset, field_list, query_params=None, exclude_params=None):
