def search_usda_foods(query, api_key, max_results=6):
...
def extract_nutrients_df(food_list):
    ...
    return pd.DataFrame(records)                            # Return structured DataFrame
...
def plot_radar_chart(row):
    ...
    daily = {                                        # Daily recommended values for each nutrient
        "Calories": 2000, "Protein (g)": 50, "Fat (g)": 70,
        "Carbs (g)": 275, "Sugar (g)": 50, "Fiber (g)": 28, "Sodium (mg)": 2300
    }
...
# === Tech Summary ===
# This function filters a DataFrame of foods based on optional nutrient constraints.
# - Supports minimum protein and maximum limits for carbs, fat, and sugar.
# - If any constraint is passed as None, it will be skipped.
# - A copy of the original DataFrame is used to avoid side effects.
# - Returns a filtered DataFrame that satisfies all specified conditions.
# =====================



def filter_foods_by_nutrients(df, min_protein=None, max_carb=None, max_fat=None, max_sugar=None):
    """
    Filter foods by optional nutrient constraints.
    Any constraint can be skipped by passing None.
    """

    filtered = df.copy()  # Work on a copy of the original DataFrame

    if min_protein is not None:
        filtered = filtered[filtered["Protein (g)"] >= min_protein]  # Keep rows with enough protein

    if max_carb is not None:
        filtered = filtered[filtered["Carbs (g)"] <= max_carb]       # Filter by carb upper limit

    if max_fat is not None:
        filtered = filtered[filtered["Fat (g)"] <= max_fat]          # Filter by fat upper limit

    if max_sugar is not None and "Sugar (g)" in filtered.columns:    # Only filter sugar if present
        filtered = filtered[filtered["Sugar (g)"] <= max_sugar]
...
logo_url = "https://cdn-icons-png.flaticon.com/512/590/590685.png"
...
with col_title:
    st.markdown("## Nutrition Scoring App 2.5")
    st.caption("Your personalized guide to smarter food choices!")
    ...
    if auth_mode == "Login":
        ...
        if st.button("Login"):
            if username in USER_DB and USER_DB[username]["password"] == password:
                ...
                st.session_state.user_profile = USER_DB[username]
        ...
        if st.button("Register"):
            if new_user in USER_DB:
                ...
                st.warning("⚠️ Username must be 3+ chars, password 4+.")
            else:
                USER_DB[new_user] = {
                    "password": new_pass,
                    "gender": gender,
                ...
                st.session_state.logged_in = True
                st.session_state.user_profile = USER_DB[new_user]
...
if "user_profile" in st.session_state:
    ...
    # --- Show profile and exercise info in sidebar ---
    ...
    burn_data = calories_to_exercise_with_distance(tee / 3, bmi, age)


    bmr = calculate_bmr(gender, age, height, weight)
    
    st.sidebar.markdown("### 🔥 Daily Energy Estimates")
    st.sidebar.write(f"**BMR (Basal Metabolic Rate):** {round(bmr)} kcal/day")
    st.sidebar.write(f"**TEE (Total Energy Expenditure):** {round(tee)} kcal/day")

    st.sidebar.markdown("**Burn 1 Meal (~1/3 TEE):**")
    ...
    keyword = st.text_input("Search food keyword", value="beef")
    ...
    if submitted:
        ...
        if "Calories" in df.columns:
            ...
            #st.markdown(f"### 🏃 To burn one meal (~{round(avg_calories)} kcal):")
            #for activity, stats in exercise_data.items():
            #    st.write(
            #        f"\u2022 {activity}: {stats['time_min']} min at {stats['speed_kmh']} km/h → {stats['distance_km']} km"
            #    )
        ...
        st.subheader(f"Top Foods for '{keyword}' (Goal: {goal.replace('_',' ').title()})")
        st.dataframe(scored)
