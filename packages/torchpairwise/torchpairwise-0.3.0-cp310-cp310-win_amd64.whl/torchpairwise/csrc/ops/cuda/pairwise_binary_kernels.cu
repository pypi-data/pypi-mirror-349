    ...
    namespace ops {
        namespace {
            ...
            namespace impl {
                ...
                C10_LAUNCH_BOUNDS_1(1024) __global__ void pairwise_binary_forward_kernel_impl(
                        index_t n_kernels,
                        const at::GenericPackedTensorAccessor<input_t, 3, at::RestrictPtrTraits, index_t> x1,
                        const at::GenericPackedTensorAccessor<input_t, 3, at::RestrictPtrTraits, index_t> x2,
                        at::GenericPackedTensorAccessor<output_t, 4, at::RestrictPtrTraits, index_t> output) {
                ...
                C10_LAUNCH_BOUNDS_1(1024) __global__ void pairwise_binary_reduction_forward_kernel_impl(
                        index_t n_kernels,
                        const at::GenericPackedTensorAccessor<input_t, 3, at::RestrictPtrTraits, index_t> x1,
                        const at::GenericPackedTensorAccessor<input_t, 3, at::RestrictPtrTraits, index_t> x2,
                        at::GenericPackedTensorAccessor<output_t, 3, at::RestrictPtrTraits, index_t> output) {
                    CUDA_1D_KERNEL_LOOP(index, n_kernels) {
            ...
            at::Tensor pairwise_binary_forward_kernel(
                    const at::Tensor &x1,
                    const at::Tensor &x2) {
                at::CheckedFrom c = "pairwise_binary_forward";
                auto args = {
                ...
                TORCHPAIRWISE_DISPATCH_CONSTEXPR_BINARY_OP_TYPES(
                        op, common_type, "pairwise_binary_forward_cuda", ([&] {
                    auto index_type = get_index_type(n_kernels);
                    if (index_type == at::ScalarType::Long) {
                        using index_t = int64_t;
                        auto output_accessor =
                                output.generic_packed_accessor<bool, 4, at::RestrictPtrTraits, index_t>();
                        impl::pairwise_binary_forward_kernel_impl<op, scalar_t, bool, index_t><<<blocks, threads>>>(
                                n_kernels,
                                        ...
                                        output_accessor);
                    } else if (index_type == at::ScalarType::Int) {
                        using index_t = int;
                        auto output_accessor =
                                output.generic_packed_accessor<bool, 4, at::RestrictPtrTraits, index_t>();
                        impl::pairwise_binary_forward_kernel_impl<op, scalar_t, bool, index_t><<<blocks, threads>>>(
            ...
            at::Tensor pairwise_binary_reduction_forward_kernel(
                    const at::Tensor &x1,
                    const at::Tensor &x2) {
                at::CheckedFrom c = "pairwise_binary_reduction_forward";
                auto args = {
                        at::TensorArg(x1, "x1", 1),
                        at::TensorArg(x2, "x2", 2)};
                at::checkAllSameGPU(c, args);
                ...
                TORCHPAIRWISE_DISPATCH_CONSTEXPR_BINARY_REDUCTION_OP_TYPES(
                        binary_op, reduction_op, common_type, "pairwise_binary_reduction_forward_cuda", ([&] {
                    auto index_type = get_index_type(n_kernels);
                    if (index_type == at::ScalarType::Long) {
                        using index_t = int64_t;
                        auto output_accessor =
                                output.generic_packed_accessor<output_t, 3, at::RestrictPtrTraits, index_t>();
                        impl::pairwise_binary_reduction_forward_kernel_impl<
                        binary_op, reduction_op, output_t, scalar_t, index_t ><<<blocks, threads>>>(
                                n_kernels,
                                        x1_c.generic_packed_accessor<scalar_t, 3, at::RestrictPtrTraits, index_t>(),
                                        x2_c.generic_packed_accessor<scalar_t, 3, at::RestrictPtrTraits, index_t>(),
                                        output_accessor);
