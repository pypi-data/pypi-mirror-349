...
@app.get("/start-daemon/{key_name}", summary="Lance un daemon", description="Cette route permet de lancer le daemon correspondant au 'key_name' passé en paramètre. Pour kill le dameon 'key_name' devient key_name_D par exemple /start-daemon/{toto} peut etre arreter utiliser par process")
...
@app.get("/kill-daemon/{key_name}", summary="Kill daemon and associated process", description="On passe la key name pour savoir si un process est en cours, si c'est le cas on le kill")
...
@app.get("/exit-srv", summary="Quit the server instance", description="On  quitte l instance actuelle")
def exit_srv():
