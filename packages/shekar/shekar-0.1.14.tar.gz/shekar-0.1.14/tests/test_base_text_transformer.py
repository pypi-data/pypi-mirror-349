...
class TestBaseTextTransformer:
    class MockTextTransformer(BaseTextTransformer):
    ...
    def test_compile_patterns(self):
        ...
        compiled_patterns = BaseTextTransformer._compile_patterns(mappings)
    ...
    def test_map_patterns(self):
        ...
        patterns = BaseTextTransformer._compile_patterns(
            [("گربه", "سگ"), ("دیوار", "حیاط")]
        )
        expected_output = "سگ روی حیاط نشست."
        assert BaseTextTransformer._map_patterns(text, patterns) == expected_output

    def test_abstract_function_error(self):
        with pytest.raises(NotImplementedError):
            BaseTextTransformer._function(None, None)
