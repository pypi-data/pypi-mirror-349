...
def make_h264_test_video(path: str) -> None:
    ...
    output_container = quickcodec.open(path, "w")

    streams = []
    for _ in range(2):
        stream = output_container.add_stream("libx264", rate=24)
        assert isinstance(stream, quickcodec.VideoStream)
        stream.width = 1280
        stream.height = 720
    ...
    for _ in range(24):
        frame = quickcodec.VideoFrame.from_ndarray(
...
class TestDecode(TestCase):
    ...
    def test_decode_audio_sample_count(self) -> None:
        ...
        assert isinstance(audio_stream, quickcodec.AudioStream)
    ...
    def test_decoded_time_base(self) -> None:
        ...
        for packet in container.demux(stream):
            for frame in packet.decode():
                assert not isinstance(frame, quickcodec.subtitles.subtitle.SubtitleSet)
    ...
    def test_side_data(self) -> None:
        container = quickcodec.open(fate_suite("mov/displaymatrix.mov"))
        frame = next(container.decode(video=0))
        assert frame.rotation == -90

    def test_hardware_decode(self) -> None:
        hwdevices_available = quickcodec.codec.hwaccel.hwdevices_available()
        ...
        hwaccel = quickcodec.codec.hwaccel.HWAccel(
            device_type=HWACCEL_DEVICE_TYPE, allow_software_fallback=False
        )

        container = quickcodec.open(test_video_path, hwaccel=hwaccel)
        video_stream = container.streams.video[0]
