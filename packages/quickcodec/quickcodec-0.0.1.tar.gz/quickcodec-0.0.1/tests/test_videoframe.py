...
import quickcodec
from quickcodec import VideoFrame
from quickcodec.video.reformatter import ColorRange, Colorspace, Interpolation
...
def test_opaque() -> None:
    with quickcodec.open(fate_suite("h264/interlaced_crop.mp4")) as container:
        ...
        ctx.flags |= quickcodec.codec.context.Flags.copy_opaque
        ...
        for packet_idx, packet in enumerate(container.demux()):
            ...
            for frame in packet.decode():
                assert isinstance(frame, quickcodec.frame.Frame)
...
def test_ndarray_gray() -> None:
    ...
    for format in ("gray", "gray8"):
        ...
        assert format in quickcodec.video.frame.supported_np_pix_fmts
...
def test_ndarray_pal8():
    ...
    assert frame.format.name in quickcodec.video.frame.supported_np_pix_fmts
