...
class KubernetesService(BaseService):
    ...
    def get_resource(self, resource_type: str, name: str, namespace: str = None,
                    api_version: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if api_version:
                api_resource = dynamic_client.resources.get(api_version=api_version, kind=resource_type.capitalize())
            else:
                # Find the API resource by name
                api_resources = self.get_api_resources()
                matching_resources = [r for r in api_resources if r["name"].lower() == resource_type.lower()]
                
                if not matching_resources:
                    raise ValueError(f"Resource type not found: {resource_type}")
                
                # Use the first matching resource
                resource_info = matching_resources[0]
            ...
            self.logger.error(f"Kubernetes API error: {e}")
            raise ServiceOperationError("kubernetes", f"get_{resource_type}", str(e))
        except ValueError as e:
            self.logger.error(f"Value error: {e}")
            raise ServiceOperationError("kubernetes", f"get_{resource_type}", str(e))
        except Exception as e:
            self.logger.error(f"Error getting {resource_type}/{name}: {e}")
            raise ServiceOperationError("kubernetes", f"get_{resource_type}", str(e))
