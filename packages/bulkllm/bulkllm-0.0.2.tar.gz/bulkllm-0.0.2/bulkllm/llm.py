...
from .usage_tracker import convert_litellm_usage_to_usage_record, track_usage
...
def patch_LLMCachingHandler():
    ...
    if not hasattr(LLMCachingHandler._convert_cached_result_to_model_response, "is_patched"):
        ...
        def wrapped_method(self, *args, **kwargs):
            ...
            if isinstance(result, litellm.ModelResponse):  # Assuming ModelResponse is the correct class
                result.is_cached_hit = True
            return result

        wrapped_method.is_patched = True  # Mark as patched
        LLMCachingHandler._convert_cached_result_to_model_response = wrapped_method


def scrubbing_callback(m):
...
def initialize_litellm(enable_logfire=False):
    ...
    litellm.cache = litellm.Cache(type="disk", disk_cache_dir=disk_cache_dir)
