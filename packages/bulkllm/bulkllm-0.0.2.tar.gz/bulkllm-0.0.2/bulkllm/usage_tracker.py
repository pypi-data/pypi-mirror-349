...
if TYPE_CHECKING:
    from litellm.utils import Usage as LiteUsage
...
def convert_litellm_usage_to_usage_record(
    ...
    *,
    ...
    input_total = (
        getattr(litellm_usage, "prompt_tokens", None)
        if getattr(litellm_usage, "prompt_tokens", None) is not None
        else itext + iimg + iaud + icache
    )
    ...
    if "xai/grok-3-mini-beta" in model and computed_output_total > logged_output_total:
        output_total = computed_output_total

    # ---------- grand total ------------------------------------------
    grand_total = (
        getattr(litellm_usage, "total_tokens", None)
        if getattr(litellm_usage, "total_tokens", None) is not None
...
def track_usage(
    ...
    *,
    ...
    litellm_usage: LiteUsage | None = None,
    # direct counts ----------------------------------------------------
    input_text_tokens: int = 0,
    input_image_tokens: int = 0,
    input_audio_tokens: int = 0,
