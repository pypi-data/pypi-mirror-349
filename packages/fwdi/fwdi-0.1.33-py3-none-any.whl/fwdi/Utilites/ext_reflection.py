...
class ExtReflection():
    ...
    def __get_default(lst_sign:list[inspect.Parameter], name_key:str) -> Any:
        search = Utilities.get_value_by_key(lst_sign, name_key) #search = [item for item in lst_sign if item.name == name_key and item.default is not inspect._empty]
    ...
    @staticmethod
    def __get_inst_method_info_v3(fn:Callable[..., Any], args:tuple, kwargs:dict)->dict:
            fn_datas:dict = {}
            fn_args:list[dict] = []
