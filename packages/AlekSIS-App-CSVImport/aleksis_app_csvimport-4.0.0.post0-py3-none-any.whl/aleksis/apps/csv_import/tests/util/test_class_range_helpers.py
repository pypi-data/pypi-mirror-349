...
from aleksis.apps.csv_import.util.class_range_helpers import (
    get_classes_from_class_selection,
...
def test_get_classes_from_class_selection():
    assert get_classes_from_class_selection(CLASSES_PER_GRADE, "Q2a+b") == ["Q2a", "Q2b"]
    assert get_classes_from_class_selection(CLASSES_PER_GRADE, "Q2a+c+d") == ["Q2a", "Q2c", "Q2d"]
    assert get_classes_from_class_selection(CLASSES_PER_GRADE, "Q2c+d") == ["Q2c", "Q2d"]
    assert get_classes_from_class_selection(CLASSES_PER_GRADE, "Q2c") == ["Q2c"]
    assert get_classes_from_class_selection(CLASSES_PER_GRADE, "Q2") == ["Q2a", "Q2b", "Q2c", "Q2d"]
    assert get_classes_from_class_selection(CLASSES_PER_GRADE, "Q2ab") == ["Q2a", "Q2b"]
    assert get_classes_from_class_selection(CLASSES_PER_GRADE, "Q2acd") == ["Q2a", "Q2c", "Q2d"]
...
def test_parse_class_range():
    ...
    classes = parse_class_range(classes_per_short_name, CLASSES_PER_GRADE, "5a+d")
    assert sorted([x.short_name for x in classes]) == ["5a", "5d"]

    classes = parse_class_range(classes_per_short_name, CLASSES_PER_GRADE, "5ad")
    assert sorted([x.short_name for x in classes]) == ["5a", "5d"]
