...
REGEX_CLASS_SELECTION = r"^([0-9]+|E|Q1|Q2)([a-z\+]*)"
...
def get_classes_from_class_selection(
    classes_per_grade: dict[str, Sequence[str]], class_selection: str
) -> list[str]:
    """Parse a class selection into single class names.

    >>> get_classes_from_class_selection(classes_per_grade, "5a+c+d")
    ["5a", "5c", "5d"]

    :param classes_per_grade: All available class labels grouped by grades
    :param class_selection: Selection to parse
    :return: List of class names
    """
    match = re.match(REGEX_CLASS_SELECTION, class_selection)

    prefix = match.group(1)
    classes = match.group(2)

    if not classes:
        classes = classes_per_grade.get(prefix, [])
    elif "+" in classes:
        classes = classes.split("+")
    else:
        classes = list(classes)

    return [f"{prefix}{klass}" for klass in classes]
...
def parse_class_range(
    ...
    if re.match(REGEX_CLASS, class_range):
    ...
    elif re.match(REGEX_CLASS_RANGE, class_range):
        # Class range
        (
            grade_start,
            group_start,
        ...
        return [
            classes_per_short_name[klass]
            for klass in get_classes_from_class_selection(classes_per_grade, class_range)
        ]
