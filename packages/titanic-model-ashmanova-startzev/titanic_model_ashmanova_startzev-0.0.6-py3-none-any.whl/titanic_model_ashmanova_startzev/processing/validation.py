...
def drop_na_inputs(*, input_data: pd.DataFrame) -> pd.DataFrame:
    ...
    new_vars_with_na = [
        ...
        if var not in config.model_configuration.categorical_vars_with_na + config.model_configuration.numerical_vars_with_na
...
def validate_inputs(*, input_data: pd.DataFrame) -> Tuple[pd.DataFrame, Optional[dict]]:
    ...
    input_data.drop(labels=config.model_configuration.variables_to_drop, axis=1, inplace=True)

    # Columns should coinside with config.model_config.feature
    assert input_data.columns.tolist() == config.model_configuration.features
