...
class Commander:
	...
	def __print_commands(self):
		for command in self.__commands[self.__internal_commands:] + self.__commands[:self.__internal_commands]:
			docs = " ".join(parse_docs(command.func)[0]) if self.autocomplete_description else ""
			for synonym in command.cmds:
				print(f"{synonym}\t{docs}" if docs else synonym)
