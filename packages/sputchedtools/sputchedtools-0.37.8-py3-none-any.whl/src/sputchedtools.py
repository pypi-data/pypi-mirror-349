...
class Anim:
	...
	def get_final_line(self, timer: Optional[Timer] = None) -> str:
		base = '\r'

		if timer:
			append = f'{self.append_text}{" " if self.append_text else ""}{timer.format_output(timer.diff, self.do_timer)}'
			...
			# Clear only animated char
			return base + self.text_format.format(
				prepend = self.prepend_text,
				char = ' ' * (len(self.char) - len(self.append_text)),
				append = self.append_text
			)
	...
	def anim(self):
		...
		final_line = self.get_final_line(t if self.do_timer else None)
