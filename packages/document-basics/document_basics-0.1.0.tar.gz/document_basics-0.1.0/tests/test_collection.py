...
def test_library_hierarchy(doc_list):
    lib = Library(docs=doc_list)
    graph = lib.graph
    # Nodes and edges
    assert set(graph.nodes) == set(d.identifier for d in doc_list)
    expected_edges = {
        ("Root", "ChildA"),
        ("Root", "ChildB"),
        ("ChildA", "GrandA1"),
        ("ChildA", "GrandA2"),
        ("GrandA1", "GreatGrandA1"),
        ("OtherRoot", "OtherChild"),
    }
    assert set(graph.edges) >= expected_edges

    # get_successors
    root = doc_list.filter.by_identifier("Root")
    succ = lib.get_successors(root)
    assert {d.title for d in succ} == {"ChildA", "ChildB"}
    childA = doc_list.filter.by_identifier("ChildA")
    succA = lib.get_successors(childA)
    assert {d.title for d in succA} == {"GrandA1", "GrandA2"}

def test_levels_and_maximum(doc_list):
    lib = Library(docs=doc_list)
    # level 0: roots
    level0 = lib.get_level(0)
    assert sorted([d.title for d in level0]) == sorted(["Root", "OtherRoot"])
    # level 1
    level1 = lib.get_level(1)
    assert sorted([d.title for d in level1]) == sorted(["ChildA", "ChildB", "OtherChild"])
    # level 2
    level2 = lib.get_level(2)
    assert sorted([d.title for d in level2]) == sorted(["GrandA1", "GrandA2"])
    # level 3
    level3 = lib.get_level(3)
    assert sorted([d.title for d in level3]) == sorted(["GreatGrandA1"])
    # maximum level
    assert lib.maximum_level == 3
