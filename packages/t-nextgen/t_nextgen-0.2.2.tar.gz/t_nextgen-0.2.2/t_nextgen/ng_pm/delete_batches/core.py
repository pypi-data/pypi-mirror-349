...
class BatchDeleterCore:
    ...
    def mark_batch_for_deletion(self, data_item: WindowSpecification, description: str):
        ...
        retries = 0
        while not batch_selected and retries < 4:
            check_box = data_item.descendants(control_type="CheckBox")[0]
            try:
                check_box.toggle()
            except _ctypes.COMError:
                logger.warning("COMError occurred while toggling the checkbox. Retrying...")
                time.sleep(1)

            if check_box.get_toggle_state() == 1:
                batch_selected = True
                logger.info(f"Batch {description} selected for deletion.")
            else:
                retries += 1
