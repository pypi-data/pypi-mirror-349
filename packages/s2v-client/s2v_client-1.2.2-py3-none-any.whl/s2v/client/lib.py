...
class S2VClient(AbstractContextManager["S2VClient"]):
    ...
    def _setup_params(
        ...
        # deploy_objects: str | list[str] | None,
        # deploy_objects_except: str | list[str] | None,
    ) -> dict[str, str]:
        path_params = {
        ...
        params = {k: str(v.relative_to(input_dir)) for k, v in path_params.items() if v is not None}

        # objects = []
        # if isinstance(deploy_objects, str):
        #     objects = [deploy_objects]
        # elif isinstance(deploy_objects, list):
        #     objects = list(deploy_objects)
        # params["deploy_objects"] = objects
        #
        # objects = []
        # if isinstance(deploy_objects_except, str):
        #     objects = [deploy_objects_except]
        # elif isinstance(deploy_objects_except, list):
        #     objects = list(deploy_objects_except)
        # params["deploy_objects_except"] = objects
    ...
    def validate(
        ...
        # deploy_objects: str | list[str] | None = None,
        # deploy_objects_except: str | list[str] | None = None,
    ) -> ValidationResult:
        ...
        # objects_d = [deploy_objects] if isinstance(deploy_objects, str) else list(deploy_objects or [])
        # objects_not_d = (
        #     [deploy_objects_except] if isinstance(deploy_objects_except, str) else list(deploy_objects_except or [])
        # )
        # params["deploy_objects"] = ",".join(objects_d)
        # params["deploy_objects_except"] = ",".join(objects_not_d)
