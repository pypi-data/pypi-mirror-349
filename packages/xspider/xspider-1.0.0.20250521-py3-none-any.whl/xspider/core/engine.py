...
from xspider.core.processor import Processor
...
class Engine(object):
    ...
    async def start_spider(self, spider: Spider):
        ...
        if hasattr(self.scheduler, "open"):
            self.scheduler.open()
        self.downloader = Downloader.create_instance(self.crawler)
        if hasattr(self.downloader, "open"):
            self.downloader.open()
        self.tasker = Tasker.create_instance(self.crawler)
        if hasattr(self.tasker, "open"):
            self.tasker.open()
        self.processor = Processor.create_instance(self.crawler)
    ...
    async def _handle_spider_outputs(self, outputs):
        async for output in outputs:
            if isinstance(output, (Request, Item)):
                await self.processor.process(output)
            else:
                raise OutputException(f"{type(self.spider)} 必须返回 'Request' 或 'Item'！")

    async def _exit(self):
        if self.scheduler.idle() and self.downloader.idle() and self.tasker.idle() and self.processor.idle():
            return True
        return False
