...
class ChatManagerClient(AbstractClient):
    ...
    def get_models(self, request_id: str = "") -> List[DictStr]:
        request = ModelsRequest()
        response: ModelsResponse = self._stub.GetModels(request, metadata=form_metadata(request_id))
        models: List[ModelInfo] = response.Models
        res = [{"ModelId": mi.ModelId, "Name": mi.Name} for mi in models]
        return res
