...
def main_function(config, CONFIG_PATH,simulate_change, selected_causal = None, change_percent = {}):
    ...
    original_data[config['date_column']] = original_data[config['date_column']].astype(str)
    ...
    if "monthly_seasonality" in config['causal_features']:
        original_data['month_no'] = original_data[config['date_column']].apply(lambda x: int(str(x)[4:]))
...
def get_date_and_format(config, CONFIG_PATH):
    ...
    with st.expander("ℹ️ Instructions"):
        ...
        - `YYYY-MM-DD` (e.g., 2023-04-30)  
        - `DD-MM-YYYY` (e.g., 30-04-2023)  
        - `MM/DD/YYYY` (e.g., 04/30/2023)  
        - `YYYY/MM/DD` (e.g., 2023/04/30)  
        - `YYYYMM` (e.g., 202304 for April 2023)  
        - `YYYYMMDD` (e.g., 20230430)  
...
def get_training_testing_data(config, CONFIG_PATH):
    ...
    with st.expander("ℹ️ Instructions"):
        ...
        - You can choose one of two methods:
            ...
            2. **By Date**  
                ...
                - Manually enter the **Test Start Date**.  
...
def main():
    ...
    if config["required_columns"]:
        ...
        if config["causal_features"]:
            ...
            config = get_date_and_format(config, CONFIG_PATH)
            ...
            if config["date_column"]:
                ...
                if config["models"]:
                    ...
                    config = get_training_testing_data(config, CONFIG_PATH)
