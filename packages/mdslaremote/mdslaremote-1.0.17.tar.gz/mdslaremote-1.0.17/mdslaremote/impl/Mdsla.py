...
class Mdsla(Common):
    ...
    def getVersion(self):
        # /info/version
        try:
            jsonResponse = self.requestUrl('/infos/version', useUserID=False)
    ...
    def login(self, host, port, username, password):
        ...
        try:
            if username != '' and password != '':
                jsonResponse = self.requestUrl('/connection/logon?username={}&password={}'.format(username, password), useUserID=False)
            else:
                jsonResponse = self.requestUrl('/connection/logon', useUserID=False)
        except Exception as ex:
            if Common.VERBOSE:
                print('Exception:', ex)
            return '%s' % ex

        Common.userId = jsonResponse['data']
