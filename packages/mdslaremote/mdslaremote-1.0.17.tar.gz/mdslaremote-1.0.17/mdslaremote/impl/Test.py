...
class Test(Parent):
    ...
    def getresultlist(self, testID, resulttype, eventnumber):
        try:
            if eventnumber is not None:
                jsonResponse = self.requestUrl('/tests/%s/results' % (testID), queryParams={'resultType' :resulttype, 'measuretype': eventnumber})
            else:
    ...
    def getmetaresultlist(self, testID):
        try:
            jsonResponse = self.requestUrl('/tests/%s/results' % (testID), queryParams={'meta': '1'})
        except Exception as ex:
            if Parent.VERBOSE:
                print('Exception', ex)
            return '%s' % ex
