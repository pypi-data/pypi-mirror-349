...
class Result(Parent):
    ...
    def getcallresultlist(self, callid, resulttype, measuretype):
        try:
            if (measuretype == None):
                jsonResponse = self.requestUrl('/results', queryParams={'callID': callid, 'resultType': resulttype})
            else:
                jsonResponse = self.requestUrl(
                    '/results', queryParams={'callID': callid, 'resultType': resulttype, 'measureType': measuretype})
        except Exception as ex:
    ...
    def gettestsummary(self, testID, bynode):
        try:
            jsonResponse = self.requestUrl('/results', queryParams= {'testid': testID, 'summary': True} )
        except Exception as ex:
            if Parent.VERBOSE:
                print('Exception', ex)
            return '%s' % ex
    ...
    def exportreporttofile(self, *args):
        ...
        if (len(args) > 4):
            if (args[4] == Parent.UNICODE):
                parameter['switch'] = '-UNICODE'
            elif (args[4] == Parent.V1TO3):
