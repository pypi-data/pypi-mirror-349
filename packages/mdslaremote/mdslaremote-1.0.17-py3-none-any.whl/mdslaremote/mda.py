...
def exportconfig(nodename,configname):
    config = mdsla.node.getConfigByName(nodename, configname)
    if Constants.Common.VERBOSE:
        print('getConfigByName', config)
    if 'ERROR' in config:
            return config
    return mdsla.node.exportConfig(config)
...
def getresultlist(testID, resulttype, eventnumber=None):
    if eventnumber is not None:
        if not isinstance(eventnumber, int):
            return "measure type must be of type int. Call getmeassuretypes() to get measure type value."
    return mdsla.test.getresultlist(testID, resulttype, eventnumber)

def getmetaresultlist(testID):
    return mdsla.test.getmetaresultlist(testID)
