...
def detection_training(epochs, model, train_dl, val_dl, optimizer=None, lr=3e-3, device=None, cls_weight=1.0, reg_weight=1.0):
    """
    ...
    for epoch in range(epochs):
        ...
        for imgs, lbls, clss in train_dl:
            imgs = imgs.to(device)
            lbls = [l.to(device) for l in lbls]
            clss = [c.to(device) for c in clss]
            
            target_cls_batch, target_reg_batch, positive_mask_batch = [], [], []
            for gt_boxes, gt_labels in zip(lbls, clss):
                target_cls, target_reg, positive_mask = compute_targets(
                    model.anchors, gt_boxes, gt_labels, device=device
                )
                target_cls_batch.append(target_cls)
                target_reg_batch.append(target_reg)
                positive_mask_batch.append(positive_mask)
            
            target_cls_batch = torch.stack(target_cls_batch)
            target_reg_batch = torch.stack(target_reg_batch)
            positive_mask_batch = torch.stack(positive_mask_batch)
            
            cls_scores, box_offsets = model(imgs)
            loss = detection_loss(
                cls_scores, box_offsets, target_cls_batch, target_reg_batch, positive_mask_batch,
