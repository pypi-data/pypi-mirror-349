...
def compute_iou(box1, box2):
    # Both box1 and box2 are [x_min, y_min, x_max, y_max]
...
def compute_targets(anchors, gt_boxes, gt_labels, iou_threshold=0.5):
    ...
    # gt_boxes are already [x_min, y_min, x_max, y_max]
    ...
    for i in range(len(anchors)):
        if positive_mask[i]:
            anchor = anchors[i]  # [cx, cy, w, h]
            gt = gt_boxes[max_idx[i]]  # [x_min, y_min, x_max, y_max]
            gt_cx = (gt[0] + gt[2]) / 2
            gt_cy = (gt[1] + gt[3]) / 2
            gt_w = gt[2] - gt[0]
            gt_h = gt[3] - gt[1]
