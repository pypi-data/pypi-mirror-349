...
class Detection(nn.Module):
    def __init__(self, num_classes=2, hidden_units=256, in_channels=1, max_faces=10, generate_embeddings=False):
        super().__init__()
...
def inference(model, image_path, transform, device):
    ...
    cls_probs = cls_probs.squeeze(0).cpu()

    results = []
    for i in range(cls_probs.size(0)):
        score = cls_probs[i].max().item()
        if score > 0.1:
            bbox = bbox_preds[i].numpy()
            bbox[0] *= img_size[0]  # x
            bbox[1] *= img_size[1]  # y
            bbox[2] *= img_size[0]  # w
            bbox[3] *= img_size[1]  # h
            results.append({
                "class_id": cls_probs[i].argmax().item(),
                "score": score,
                "bbox": bbox.tolist()
            })
