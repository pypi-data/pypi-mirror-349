...
class Detection(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.backbone):
            x = layer(x)
            if i == 4:  # Feature map at 16x16
                features.append(x)
            elif i == 5:  # Feature map at 8x8
                features.append(x)
    ...
    def decode_boxes(self, offsets):
        # offsets: (total_anchors, 4) [dx, dy, dw, dh]
        # self.anchors: (total_anchors, 4) [cx, cy, w, h]
    ...
    def detect(self, x, conf_threshold=0.5, nms_threshold=0.5):
        """
        Performs inference, outputting bounding boxes directly.
        Returns a list of tensors, each of shape (num_detections, 5) [x_min, y_min, x_max, y_max, score].
        """
        cls_scores, box_offsets = self.forward(x)
        batch_size = x.size(0)
        probs = F.softmax(cls_scores, dim=-1)[:, :, 1]  # Face class probabilities
        final_detections = []
        
        for i in range(batch_size):
            offsets = box_offsets[i]  # (total_anchors, 4)
            boxes = self.decode_boxes(offsets)  # (total_anchors, 4)
            scores = probs[i]  # (total_anchors,)
