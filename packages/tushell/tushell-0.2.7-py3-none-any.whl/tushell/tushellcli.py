...
@click.command()
@click.option('--key', required=False, help='Memory key to retrieve (required unless --jkey is used)')
...
def get_memory(key, jkey, list_keys, json_flag, json_file, md_flag, md_file):
    """Get fractal stone memory value by key. Ritual: --json/--md for stdout, --json-file/--md-file for file output. --jkey <keyname> saves as <keyname>.json and can be used as the memory key if --key is omitted."""
    import os, requests, json
    EH_API_URL = os.getenv("EH_API_URL")
    EH_TOKEN = os.getenv("EH_TOKEN")
    # If --key is not provided but --jkey is, use jkey as the memory key
    memory_key = key or jkey
    if not memory_key:
        click.echo("Error: You must provide either --key or --jkey.")
        return
...
def render_markdown_from_result(result):
    ...
    - If result is a dict with a single key, use that as the main section.
    - If result is a string, output as a code block.
    - Otherwise, render as best as possible.
    """
    import json
