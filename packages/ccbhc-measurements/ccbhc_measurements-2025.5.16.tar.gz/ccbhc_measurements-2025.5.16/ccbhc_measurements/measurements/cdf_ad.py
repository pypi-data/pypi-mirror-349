...
class _Sub_1(Submeasure):
    ...
    def __filter_mental_exclusions(self, exclusions: pd.DataFrame) -> None:
        """
        Removes all patients whose first exclusion_date is before their encounter
        """
        # keep only the earliest exclusion_date per patient
        excl = exclusions.sort_values(by=['patient_id', 'exclusion_date']).drop_duplicates('patient_id', keep='first')
        excl['exclusion_date'] = pd.to_datetime(excl['exclusion_date'])
        # merge exclusions
        pop = self.__populace__.copy()
        pop_with_excl = pop.merge(excl[['patient_id', 'exclusion_date']], how='left')
        # filter to rows where encounter is on or before exclusion_date, or if situation of no exclusion exists
        valid = pop_with_excl[
            (pop_with_excl['encounter_datetime'] <= pop_with_excl['exclusion_date'])
            | pop_with_excl['exclusion_date'].isna()
        ]
        # drop the temp column and reassign
        self.__populace__ = valid.drop(columns='exclusion_date')

    def __get_depressions(self) -> pd.DataFrame:
        """
        Finds all patients with a diagnosis of depression
        """
        depressions = self.__DIAGNOSIS__[self.__DIAGNOSIS__['diagnosis'] == 'depression'].copy()
        depressions.rename(columns={'encounter_datetime':'exclusion_date'}, inplace=True)
        return depressions
    ...
    def __get_bipolars(self) -> pd.DataFrame:
        ...
        """
        bipolars = self.__DIAGNOSIS__[self.__DIAGNOSIS__['diagnosis'] == 'bipolar'].copy()
        bipolars.rename(columns={'encounter_datetime': 'exclusion_date'}, inplace=True)
        return bipolars
    ...
    def _set_stratification(self) -> None:
        ...
        """
        self.__stratification__ = self.__populace__[['patient_measurement_year_id','patient_id','encounter_id','encounter_datetime','screening_date','last_encounter']].sort_values(['patient_measurement_year_id','encounter_id']).drop_duplicates('patient_measurement_year_id')
