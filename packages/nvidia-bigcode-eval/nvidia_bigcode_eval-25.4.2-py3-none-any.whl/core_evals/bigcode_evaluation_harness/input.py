...
def get_framework_evaluations(
    ...
    run_config_framework_defaults_with_cli = deep_update(run_config_framework_defaults,
                                                run_config_cli_overrides or {},
                                                skip_nones=True)
...
def get_available_evaluations(run_config_cli_overrides: Optional[dict] = None) -> tuple[dict[str, dict[str, Evaluation]], dict[str, Evaluation]]:
    ...
    framework_name, pkg_name, framework_evaluations,run_config_framework_defaults_with_cli  = get_framework_evaluations(def_file, run_config_cli_overrides)
    ...
    return framework_eval_mapping, eval_name_mapping, run_config_framework_defaults_with_cli
...
def validate_evaluation(run_config_cli_overrides: dict) -> Evaluation:
    ...
    framework_evals_mapping, all_evals_mapping, run_config_framework_defaults_with_cli  = get_available_evaluations(run_config_cli_overrides)
    ...
    if framework_name:
        try:
            ...
            raise MisconfigurationError(f"Unknown framework {framework_name}. Frameworks available: {', '.join(framework_evals_mapping.keys())}")
