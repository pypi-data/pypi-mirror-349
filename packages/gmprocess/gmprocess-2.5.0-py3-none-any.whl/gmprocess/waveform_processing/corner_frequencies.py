...
def from_snr(st, event, same_horiz=True, smoothing_parameter=20):
    ...
    for tr in st:
        ...
        if tr.passed:
            ...
            if len(lows) == 0:
                tr.fail(f"SNR not greater than required threshold ({threshold}).")
                continue
            ...
            for low_value, high_value in zip(lows, highs):
                if low_value <= min_freq and high_value > max_freq:
                    low_corner = low_value
                    high_corner = high_value
                    break
            else:
            ...
            duration = tr.get_parameter("signal_end")["end_time"] - tr.stats.starttime
            low_corner = max(1 / duration, low_corner)

            # Make sure high pass is greater than minimum SNR
            idx_max = np.nanargmax(snr)
            if idx_max > 0:
                # If SNR is flat, then max SNR could be at loweest freq
                if np.all(np.isnan(snr[0:idx_max])):
                    idx_min = idx_max
                else:
                    idx_min = np.nanargmin(snr[0:idx_max])
                freq_hp = max(freq[idx_min], low_corner)
            else:
                freq_hp = low_corner

            # Check if highpass is greater than min freq of noise spectrum
            n_noise = len(tr.get_cached("preevent_noise_trace")["data"])
            min_freq_noise = 1.0 / (n_noise * tr.stats.delta)
            if freq_hp < min_freq_noise:
                tr.warning(
                    f"Using high-pass corner ({freq_hp:.5f}) lower than minimum "
                    f"frequency of noise (pre-event) window ({min_freq_noise:.5f})."
                )
