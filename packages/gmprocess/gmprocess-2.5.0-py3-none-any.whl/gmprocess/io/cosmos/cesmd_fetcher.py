...
class CESMDFetcher(DataFetcher):
    ...
    def get_matching_events(self, solve=True):
        ...
        for event in tmp_events:
            ...
            tevent["time"] = UTCDateTime(event["time"])
    ...
    def retrieve_data(self, event_dict):
        ...
        starttime = self.time - timedelta(seconds=self.dt // 2)
        endtime = self.time + timedelta(seconds=self.dt // 2)
