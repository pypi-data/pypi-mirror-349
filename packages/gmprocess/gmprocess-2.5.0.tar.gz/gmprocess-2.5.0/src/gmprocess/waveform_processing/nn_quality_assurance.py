...
def compute_quality_metrics(st):
    ...
    for tr_i in st:
        if ind[i] != "v":
            ...
            # Fourier spectrum
            str_i = "ft" + ind[i]
            sig_spec = tr_i.get_cached("signal_spectrum")["spec"]
            sig_spec = np.where(np.isnan(sig_spec), 0.0, sig_spec)
            tr[str_i] = sig_spec

            # Freq of the Fourier spectrum
            ...
            # SNR
            str_i = "snr" + ind[i]
            snr = tr_i.get_cached("snr")["snr"]
            snr = np.where(np.isnan(snr), 0.0, snr)
            tr[str_i] = snr
