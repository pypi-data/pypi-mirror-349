...
def read_obspy(filename, config=None, **kwargs):
    ...
    for ttrace in tstream:
        ...
        nslc = f"{network}.{station}.{channel}"
        ...
        if inventory:
            ...
            if tinventory[0][0][0].sample_rate != ttrace.stats.sampling_rate:
                raise ValueError(
                    f"Sample rate is not consistent betwen data and metadata ({nslc})"
                )
        else:
            tinventory = None
        ...
        if not (
            np.issubdtype(trace.data.dtype, np.integer)
            or np.allclose(trace.data % 1, 0)
        ):
