    ...
    def calculate(self):
        ...
        for metric, metric_steps in self.steps.items():
            ...
            try:
                ...
                # This catches the error that arises when rotd is requested but cannot
                # be computed because there aren't two horizontals.
                logging.warning(
                    f"IndexError occurred when computing {metric}, skipping."
                )
                break
            except ValueError:
                # This catches the error when:
                #  - rotd is requested but one or more of the horizontals have failed
                #    QA checks.
                logging.warning(
                    f"ValueError occurred when computing {metric}, skipping."
                )
                break
