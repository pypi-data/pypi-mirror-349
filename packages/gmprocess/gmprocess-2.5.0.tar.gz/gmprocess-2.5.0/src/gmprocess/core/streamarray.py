...
class StreamArray(object):
    ...
    def select(self, network=None, station=None, location=None, instrument=None):
        """Select Streams.

        Return a new StreamCollection with only those StationStreams that
        match network, station, location, and/or instrument selection criteria.
        ...
        for st in self:
            inst = st.get_inst()
            net_sta_loc = st.get_net_sta_loc().split(".")
            net = net_sta_loc[0]
            sta = net_sta_loc[1]
            loc = net_sta_loc[2]
            ...
            if location is not None:
                if not fnmatch.fnmatch(loc.upper(), location.upper()):
                    continue
