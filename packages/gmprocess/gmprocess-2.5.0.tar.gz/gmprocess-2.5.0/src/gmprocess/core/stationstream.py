...
class StationStream(Stream):
    ...
    def __check_channels(self):
        if len(self):
            ...
            for tr in self:
                stats = tr.stats
                all_codes.append(f"{stats.network}.{stats.station}.{stats.location}.{stats.channel}")
    ...
    def __check_id(self):
        # Check that id is consistent, and set id if it passes the check.
        if len(self):
            use_array = hasattr(self, "use_array") and self.use_array
            stats = self[0].stats
            if use_array:
                id_str = ".".join([
                    stats.network,
            ...
            for tr in self: # Use self to avoid creating temporary object
                stats = tr.stats
                if use_array:
                    test_str = ".".join([
                        stats.network,
                ...
                if id_str != test_str:
                    raise ValueError(f"Inconsistent stream IDs {id_str} and {test_str} for different traces.")
            self.id = id_str
    ...
    def get_net_sta(self):
        """Get just the network and station components of the ID."""
        return ".".join(self.id.split(".")[0:2])

    def get_net_sta_loc(self):
        """Get just the network, station, and location components of the ID."""
