...
async def generate_with_openai_stream(client: AsyncOpenAI, model_name: str, conversation: List[Dict],
    ...
    try:
        # Validate formatted_functions
        if not isinstance(formatted_functions, list):
            formatted_functions = []
        elif not all(isinstance(f, dict) for f in formatted_functions):
            formatted_functions = []
        ...
        # Only add tools and tool_choice if we have valid functions
        if formatted_functions:
            api_params["tools"] = [{"type": "function", "function": f} for f in formatted_functions]
