...
class MqttSender(AbcSender):
    ...
    def _get_topic(self, dest_id, data_source):
        epoch_ms = int(time.time() * 1000)
        filename = f"{epoch_ms}.json"
        return f"vyom-mqtt/message/{dest_id}/{self.device_id}/{data_source}/{filename}"
    ...
    def send_message(
        ...
        """
        try:
            topic = topic if target_des_id == "s3" else self._get_topic(target_des_id, data_source)
