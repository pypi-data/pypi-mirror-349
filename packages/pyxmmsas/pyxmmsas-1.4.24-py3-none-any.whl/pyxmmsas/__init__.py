...
def check_correlations(fit_by_bin, threshold=0.2, make_plots=True, verbose=True, n_sample=1000,
                       to_exclude=['factor'], forced_x=['times'], rearrange=True):
    old_size = plt.rcParams['figure.figsize']
    if rearrange:
        fit_by_par = rearrange_fit_parameters(fit_by_bin)
    else:
        fit_by_par = fit_by_bin
...
class xmmanalysis(object):
    ...
    def write_ds9_region_files(self, base_filename='',
                               i_source_region=None, i_background_region=None):
        """
        Dumps the region files for DS9 in base_filename + 'source.reg' and base_filename + 'background.reg'

        :param base_filename: Base file name
        :param source_region: Source region
        :param background_region: Background region

        :return: None
        """
        
        preamble = '# Region file format: DS9 version 4.1\n' + \
            'global color=green dashlist=8 3 width=1 font="helvetica 10 normal roman" select=1 highlite=1 dash=0 fixed=0 edit=1 move=1 delete=1 include=1 source=1\n' + \
            'physical\n'
            
        if hasattr(self, 'sas_source_coord'):
            source_region = self.sas_source_coord
        else:
            source_region = i_source_region
        if hasattr(self, 'sas_back_coord'):
            background_region = self.sas_back_coord
        else:
            background_region = i_background_region
        
        print("Writing DS9 region files")
        print("Source region: %s" % return_none_string(source_region))
        print("Background region: %s" % return_none_string(background_region))
        
        if source_region is not None:
            with open(base_filename + 'source.reg', 'w') as f:
                source = [float(num) for num in re.findall(r'[\d.]+', 
                                                           source_region)]
                f.write(preamble)
                f.write('circle(%.3f, %.3f, %.3f)\n' % (source[0], source[1],
                                                         source[2]))
        if background_region is not None:
            with open(base_filename + 'background.reg', 'w') as f:
                f.write(preamble.replace('green', 'white'))
                back = [float(num) for num in re.findall(r'[\d.]+',
                                                         background_region)]
                f.write('circle(%.3f, %.3f, %.3f)\n' % (back[0], back[1],
                                                         back[2]))
...
''' % (min_psf, max_psf, default_psf,
       ...
       critical_surface_brightness)
        ...
        self.sas_source_coord = sas_source_coord
        self.sas_back_coord = sas_back_coord
...
then 
    ...
    export HEADAS=`ls -dA /opt/heasoft/x86_*`;. $HEADAS/headas-init.sh;source /opt/CalDB/software/tools/caldbinit.sh;export RELXILL_TABLE_PATH=/home/user/data/relline_tables/
fi
...
if [ -z "$HEADAS" ]
then 
    export HEADAS=/opt/heasoft/`ls -dA /opt/heasoft/x86_*`;. $HEADAS/headas-init.sh;source /opt/CalDB/software/tools/caldbinit.sh;export RELXILL_TABLE_PATH=/home/user/data/relline_tables/
fi
