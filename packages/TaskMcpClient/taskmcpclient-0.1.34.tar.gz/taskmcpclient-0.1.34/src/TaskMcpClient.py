...
class TaskMcpClient:
    ...
    async def _gather_available_tools(self):
        ...
        lsFuncDetail = []  # Func详细描述信息列表，用于打印
        iModuleCnt = 0
        for sModuName, oSession in self.dictSessionByName.items():
            ...
            for tool in response.tools:
                ...
                lsFuncSimple.append((sFullFuncName, tool.name, tool.description[:30]))
        ...
        if self.iConnectMcpCount < 1:
            if self.sRunMode == 'chat':
                iIdx = 0
                for (sFunc, sTool, sDesc, sSchema) in lsFuncDetail:
                    iIdx += 1
                ...
                for (sFunc, sTool, sDesc) in lsFuncSimple:
                    iIdx += 1
                    PrintInline(f"{iIdx}. {sFunc}[{sTool}]={sDesc}\n")
        ...
        while True:
            try:
                ...
                if sQuery.startswith('#f'):
                    ...
                    await self.oLlm.process_query(sQuery, self.lsServFuncTools, self._callbackTool)
                    mcp_text = '\n'.join(self.oLlm.lsMcpTextOut)
                    PrintTimeMsg(f"loop_mcp_chat.mcp_text={mcp_text}=")
