...
def getBuildingFootprintSpark(countryISO, boundary_file, out_dir, tile_size, projectID=None):
    ...
    elif isinstance(boundary_file, gpd.GeoDataFrame):
        boundary = boundary_file.to_crs("EPSG:4326")
    elif isinstance(boundary_file, (dict, ee.Geometry, ee.FeatureCollection)):
        raise ValueError("Earth Engine or dict geometries not yet supported here.")
    else:
        # If user provides a shapely geometry directly
        boundary = gpd.GeoDataFrame(geometry=[boundary_file], crs="EPSG:4326")
