...
from tako.types.visualize.types import SimpleDataset, TakoDataFormatDataset, VisualizeRequest
...
class TakoClient:
    ...
    def beta_visualize(self, tako_formatted_dataset: TakoDataFormatDataset) -> KnowledgeSearchResults:
...
class AsyncTakoClient:
    ...
    async def beta_visualize(self, tako_formatted_dataset: TakoDataFormatDataset, timeout_seconds: Optional[float] = None) -> KnowledgeSearchResults:
        url = f"{self.server_url}/api/{self.api_version}/beta/visualize"
        visualize_request = VisualizeRequest(
            tako_formatted_dataset=tako_formatted_dataset,
        )
        ...
        async with httpx.AsyncClient(
            ...
            response = await client.post(url, json=payload, headers={"X-API-Key": self.api_key})
            raise_exception_from_response(response)
            return KnowledgeSearchResults.model_validate(response.json())