...
class ShiftCommunityMember(ecommunity.simulator.SynchronizedMember):
    ...
    def day_interactions(self, the_date: datetime.date, day_hour_indices: list[int], interaction: int, init: bool):
        ...
        for hour_index in day_hour_indices:
            if self.manager.get_hour_colors(hour_index) == ecommunity.simulator.COLOR.RED:
            ...
            if self.manager.get_hour_colors(hour_index) == ecommunity.simulator.COLOR.GREEN:
...
class CommunityManager(ecommunity.simulator.Manager):

    def __init__(self, pv_system: core.solar.PVsytem, no_alert_threshold, randomize_ratio: int = .2, averager_depth_in_hours: int = 3) -> None:
    ...
    def day_interactions(self, the_date: datetime.date, day_hour_indices: list[int], interaction: int, init: bool) -> None:
        ...
        for i in range(len(day_productions_kWh)):
            if self.datetimes[i].hour < 8 or self.datetimes[i].hour > 22:
                self.set_hour_colors(day_hour_indices[i], ecommunity.simulator.COLOR.WHITE)
            elif day_productions_kWh[i] > day_consumptions[i] + self.no_alert_threshold:
                self.set_hour_colors(day_hour_indices[i], ecommunity.simulator.COLOR.GREEN)
            elif day_consumptions[i] > day_productions_kWh[i] + self.no_alert_threshold:
                self.set_hour_colors(day_hour_indices[i], ecommunity.simulator.COLOR.RED)
            else:
                self.set_hour_colors(day_hour_indices[i], ecommunity.simulator.COLOR.WHITE)
