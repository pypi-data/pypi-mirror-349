...
class MQDEA:
    ...
    def __init__(self, data,id,year,sent = "inputvar=outputvar",  gy=[1], gx=[0], rts=RTS_VRS1, \
        ...
        self.gy, self.gx, self.inputvars,self.outputvars = tools.assert_MQDEA(
                        data, sent, gy, gx
                    )
    ...
    def get_total(self,data,sent,id,year):
        ...
        if self.input_oriented or self.output_oriented or (self.hyper_oriented and self.rts == RTS_CRS):
            # Standard case: Expect 'te' and calculate a single 'D11'
            expected_cols = ['te']
            output_cols = ['D11']
            process_type = 'single_d11'

        elif (self.hyper_oriented and self.rts == RTS_VRS1):
        ...
        for tindex in self.tlt.index:
            ...
            model = DEA2(data=data, sent=sent, gy=self.gy, gx=self.gx,
        ...
        if self.input_oriented or self.output_oriented or (self.hyper_oriented and self.rts == RTS_CRS):
            ...
            for tindex in self.tlt.index[1:]:
                ...
                if not all(col in data12_results.columns for col in expected_cols):
                    # This check is crucial. If DEA2 doesn't return the expected columns, stop.
                    # Consider adding more specific error messages or handling based on model.optimize() status
                    raise KeyError(f"DEA2 results DataFrame for year {current_year} does not contain required columns '{expected_cols}' for the chosen orientation/RTS combination. Available columns: {list(data12_results.columns)}")
            ...
            for tindex in self.tlt.index[:-1]:
                ...
                if not all(col in data21_results.columns for col in expected_cols):
                    # This check is crucial. If DEA2 doesn't return the expected columns, stop.
                    # Consider adding more specific error messages or handling based on model.optimize() status
                    raise KeyError(f"DEA2 results DataFrame for year {current_year} does not contain required columns '{expected_cols}' for the chosen orientation/RTS combination. Available columns: {list(data21_results.columns)}")
            ...
            print("CONTEMPORARY tech (Hyperbolic VRS) calculation finished.")
...
class MQDDF(MQDEA):
    ...
    def get_total(self,data,sent,id,year):
        ...
        # Determine which columns to expect and how to process based on orientation and RTS
        if self.input_oriented  or (self.hyper_oriented and self.rts == RTS_CRS):
            # Standard case: Expect 'te' and calculate a single 'D11'
            expected_cols = ['tei']
            output_cols = ['D11']
            process_type = 'single_d11'
        ...
        for tindex in self.tlt.index:
            ...
            # Use the calculated gx and gy, and the mapped RTS
            model = DDF2(data=data, sent=sent, gy=self.gy, gx=self.gx,
                            rts=self.rts, baseindex=f"{year}=[{current_year}]",
                            refindex=all_years_ref_index) # Reference set is all years
