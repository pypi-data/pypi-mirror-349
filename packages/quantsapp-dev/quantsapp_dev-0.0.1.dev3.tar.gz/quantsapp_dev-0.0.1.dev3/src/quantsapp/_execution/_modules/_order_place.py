...
class PlaceOrder:
    ...
    def _parse_order(self) -> ApiPayloadPlaceOrder_Type:
        ...
        parsed_order: ApiPayloadPlaceOrder_Type = {
            ...
            'settings': {
                'margin_benefit': self.order.marigin_benefit,
            }
        }
    ...
    def _place_order(self, orders_to_place: ApiPayloadPlaceOrder_Type) -> ResponsePlaceOrders_Type:
        ...
        place_orders_resp: ApiResponsePlaceOrder_Type = self.ws.invoke_api(
            ...
            payload={
                ...
                'ui_orderid': f"{generic_utils._internal_session_data['qapp_ac_data'].API_KEY}_{int(time.time()*1000)}",  # type: ignore[reportPrivateUsage]
            },
        ) # type: ignore
            ...
            'orders': {
                execution_models.BrokerClient_Pydantic.from_api_str(_broker_client): {
                    execution_enums.OrderStatus(_order_status): {
                        _inst: [
                            {
                                'b_order_id': _leg_order_data.get('b_order_id', None),
                                'buy_sell': execution_enums.OrderTransactionType(_leg_order_data['buy_sell']),
