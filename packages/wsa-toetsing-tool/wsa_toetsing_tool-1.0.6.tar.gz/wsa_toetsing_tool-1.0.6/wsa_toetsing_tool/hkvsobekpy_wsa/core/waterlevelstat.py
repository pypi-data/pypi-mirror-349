...
try:
    ...
    pass
from wsa_toetsing_tool.hkvsobekpy_wsa.io.his import __his_class
...
class __waterlevelstat_class(object):
    ...
    def _enkeleGumbelFit(self, df_enkel, N, vensterArray=[0,9], GumbelT=[25,50,100.0], Ggi=0.44, GgN=0.12):
        ...
        if len(df_enkel.name) == 2:
            enkel_location = df_enkel.name[1]
            enkel_parameter = df_enkel.name[0]
        else: 
            enkel_location = enkel_parameter = 'niet gedefinieerd'   
    ...
    def SchrijfTabellen(self,stat_object,shp_key,decimals=3, init_tabel=True):
        ...
        # stat_object.stats.df_table1 = stat_object.stats.df_table1.concat(table1_list, ignore_index=True)
        stat_object.stats.df_table1 = pd.concat([stat_object.stats.df_table1, pd.DataFrame(table1_list)],ignore_index=True)
        ...
        pd.concat([stat_object.stats.df_table2, pd.DataFrame(table2_list)], ignore_index=True)
        if init_tabel==True:
