...
TextContentT = TypeVar("TextContentT", bound=TextContentModel)
ToolCallT = TypeVar("ToolCallT", bound=ToolCallModel)


class ParsedResponseCandidate(BaseModel, Generic[TextContentT, ToolCallT]):
...
class ParsedResponse(BaseModel, Generic[TextContentT, ToolCallT]):
    """
    ...
    provider: Provider
    candidates: list[ParsedResponseCandidate[TextContentT, ToolCallT]]
...
class ResponseParserProtocol(Protocol, Generic[ResponseType, TextContentT, ToolCallT]):
    """Protocol for provider-specific response parsers."""

    @classmethod
    def parse_response(
        cls, response: ResponseType, provider: Provider
    ) -> ParsedResponse[TextContentT, ToolCallT]: ...
