...
turpe_indexes_file_path = os.path.join(current_directory, 'turpe_indexes.json')
print("turpe_indexes_file_path  =",turpe_indexes_file_path  )
...
class TurpeCalculator:
    def __init__(self,contrat,tarif,facture, coefficients_file="coefficients.json"):
        ...
        self.turpe_indexes_file = os.path.join(current_directory, turpe_indexes_file_path)
        print("turpe_indexes_file =",self.turpe_indexes_file)
...
# Example usage:
    ...
    def calculate_turpe(self):
        ...
        self.turpe_index=self.get_taux_index(self.facture.start)
        print("self.turpe_index = ",self.turpe_index)
...
######################Fonctions pour le calcul du TURPE#######################################""
    ...
    def get_taux_index(self, date):
        print("lancement de get_taux_index")
        # get index from json file
        with open(self.turpe_indexes_file, 'r') as fichier:
            donnees = json.load(fichier)
        for index in donnees:
            start_date = parse(index["start_date"]).date()
            expiration_date = parse(index["end_date"]).date()
            if start_date <= date <= expiration_date:
                print("index = ", index)
                self.taux_index = 1+index["taux_index"]/100
                return self.taux_index
