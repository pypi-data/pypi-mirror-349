...
from my_asana.comments_helper import get_up_for_sale_again_message, add_comment_to_task, \
    add_followers_to_task, get_property_sold_message, get_off_market_message
from my_asana.consts import LEADS_PROPERTIES_SECTION, DECLINED_LEADS_SECTION
...
class OffMarketHandler(AbstractContingentHandler):
    ...
    def handle(self):
        """
            ...
            1. Add a comment to the task (notify the property is off market)
            2. Mark the task as completed
            3. Add the task to the Declined section
            :param under_contract_property:
            :return:
