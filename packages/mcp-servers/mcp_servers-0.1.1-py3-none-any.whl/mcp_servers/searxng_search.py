...
class MCPServerSearXNG:
    def __init__(self):
        ...
        if not self.SEARXNG_INSTANCE_URL:
            print("Error: SEARXNG_INSTANCE_URL environment variable is required.", file=sys.stderr)
            sys.exit(1)
        if not self.SEARXNG_INSTANCE_URL.endswith('/'):
        ...
        print(f"INFO: Targeting SearxNG instance: {self.SEARXNG_INSTANCE_URL}")
    ...
    async def _init_client(self):
        if not self.http_client:
            ...
            self.http_client = httpx.AsyncClient(
                base_url=self.SEARXNG_INSTANCE_URL,
