...
from ......base.adapter_interface import IReplicateAdapter
...
class TextToVideoAdapter(IReplicateAdapter):
    ...
    def convert_input(self) -> dict:
        ...
        if not self.skip_validation:
            input_model = TextToVideoInput.model_validate(self.data)
        else:
            input_model = TextToVideoInput.model_construct(**self.data)
...
class ImageToVideoAdapter(IReplicateAdapter):    
    def convert_input(self) -> dict:
        # Validate the input data if required
        if not self.skip_validation:
            input_model = ImageToVideoInput.model_validate(self.data)
        else:
            input_model = ImageToVideoInput.model_construct(**self.data)
...
class ReferenceImagesToVideoAdapter(IReplicateAdapter):    
    def convert_input(self) -> dict:
        # Validate the input data if required
        if not self.skip_validation:
            input_model = ReferenceImagesToVideoInput.model_validate(self.data)
        else:
            input_model = ReferenceImagesToVideoInput.model_construct(**self.data)
...
class VideoToVideoAdapter(IReplicateAdapter):    
    def convert_input(self) -> dict:
        # Validate the input data if required
        if not self.skip_validation:
            input_model = VideoToVideoInput.model_validate(self.data)
        else:
            input_model = VideoToVideoInput.model_construct(**self.data)
...
class VideoInpaintingAdapter(IReplicateAdapter):    
    def convert_input(self) -> dict:
        # Validate the input data if required
        if not self.skip_validation:
            input_model = VideoInpaintingInput.model_validate(self.data)
        else:
            input_model = VideoInpaintingInput.model_construct(**self.data)
