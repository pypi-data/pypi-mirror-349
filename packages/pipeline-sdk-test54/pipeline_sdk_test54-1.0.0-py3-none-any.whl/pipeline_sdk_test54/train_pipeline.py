...
def train_pipeline(
    ...
    callback: Optional[Callable[[str, int, int, str, str], None]] = None,
    epochs: int = 1,
    imgsz: int = 640
):
    ...
    def model_train_dsl_component(
        ...
        print("inside dsl component")
        if callback:
            callback(model_artifact_path, epochs, imgsz, s3_bucket_name, s3_source_folder)
        
    model_train_dsl_component(s3_bucket_name=s3_bucket_name, s3_source_folder=s3_source_folder, epochs=epochs, imgsz=imgsz)
