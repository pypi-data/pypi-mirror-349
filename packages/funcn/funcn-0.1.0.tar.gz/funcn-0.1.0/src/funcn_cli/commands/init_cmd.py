...
app = typer.Typer(help="Initialise funcn in the current project.")

CONFIG_FILE_NAME = "funcn.json"
SCHEMA_FILE_NAME = "funcn.schema.json"
...
def init(
    ...
    """Interactive initialisation for funcn within the current project."""
    ...
    # Determine default base directory for funcn components
    src_dir = project_root / "src"
    app_dir = project_root / "app"
    if src_dir.exists():
        default_base = src_dir / "funcn"
    elif app_dir.exists():
        default_base = app_dir / "funcn"
    else:
        default_base = project_root / "packages" / "funcn_registry" / "components"
    ...
    if config_exists and not force:
        _print(f":sparkles: [bold green]funcn already initialised in[/] {project_root} ({CONFIG_FILE_NAME} exists)")
