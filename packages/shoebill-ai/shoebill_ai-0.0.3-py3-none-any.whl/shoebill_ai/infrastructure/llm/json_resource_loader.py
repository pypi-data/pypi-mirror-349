...
class JsonResourceLoader:
    ...
    Usage:
        ...
        config = loader.load_json_resource("autonomous_agent.json", relative_to_caller=True)
    """
    ...
    def load_json_resource(self, filename: str, is_absolute_path: bool = False,
        ...
        if relative_to_caller:
            ...
            # Add .json extension if not present and not absolute path
            if not filename.endswith('.json') and not os.path.isabs(filename):
                filename = f"{filename}.json"

            relative_path = os.path.join(caller_dir, filename)
