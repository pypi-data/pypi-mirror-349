...
class LMI:
    ...
    @measure_runtime
    def _run_kmeans(
        n_buckets: int, dimensionality: int, X: torch.Tensor, max_iter: int
    ) -> torch.Tensor:
        return LMIBase._run_kmeans(n_buckets, dimensionality, X, max_iter)
    ...
    def create(
        ...
        y_train = LMI._run_kmeans(n_buckets, data_dim_original, X_train, 25)
