...
class FlowStateList(BaseModel):
    """
    FlowStateList
    """ # noqa: E501
    pagination: Optional[PaginationResponse] = None
    rows: Optional[List[FlowState]] = None
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in FlowStateList) in the input: " + _key)

        _obj = cls.model_validate({
            "pagination": PaginationResponse.from_dict(obj["pagination"]) if obj.get("pagination") is not None else None,
            "rows": [FlowState.from_dict(_item) for _item in obj["rows"]] if obj.get("rows") is not None else None
        })
        return _obj
