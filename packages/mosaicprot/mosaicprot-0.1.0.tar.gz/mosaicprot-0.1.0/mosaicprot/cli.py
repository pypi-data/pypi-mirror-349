...
def process_fasta_output(input_file, min_aa_lim, output_file, temp_file_name):
    """Process FASTA file to detect ORFs."""
    print("Finding ORFs...")
    try:
        log_step(temp_file_name, f"Reading input file: {input_file}")
        # print(f"Processing ORFs from {input_file}...")
...
def generate_mosaic_proteins(input_file, refprots_file, altprots_file, transcript_file, temp_file_name, idx_num):
    ...
    def translate_all_frameshifted(sequence, skip_value=30, altprot_name="noname", refprot_name="noname",
        ...
        # Next four lines are added after the debug.
    ...
    # Due to altprot/altprot overlap -start
    seq_dict_refprots = dict(chain.from_iterable(d.items() for d in (seq_dict_refprots, seq_dict_altprots)))
    # Open and read the list_of_altProts.txt file from .mosaic_prot_results
    ...
    for altprot in list_of_altProts:
        try:
            ...
            # Following for loop may seems strange, because every transcript has only one refprot.
            # However, due to altprot/altprot overlap, one is considered as altprot and another (others) is considered as refprot.
            for refprot_transcript in refprot_transcript_list:
                try:
                    refprot_transcript = str(refprot_transcript)
                    # print(refprot_transcript)
                    ...
                    if (int(AS1) >= int(RS1)) and (int(AS1) <= int(RS2)) and (int(AS2) >= int(RS1)) and (
                        ...
                        # print("Within (Embedded) Reference ORF")
                        # 5 prime of altprot
                        overlapped_region = sorted(list(set(altprot_range) & set(refprot_range)))
                        if len(overlapped_region) > 0:
                    ...
                    if (int(AS1) <= int(RS1)) and (int(AS1) <= int(RS2)) and (int(AS2) >= int(RS1)) and (
                        ...
                        # print("5' UTR side overlap with Reference ORF")
                    ...
                    if (int(AS1) <= int(RS1)) and (int(AS1) <= int(RS2)) and (int(AS2) <= int(RS1)) and (
                        ...
                        # print("5' UTR side (No overlapped)")
                    ...
                    if (int(AS1) <= int(RS1)) and (int(AS1) <= int(RS2)) and (int(AS2) >= int(RS1)) and (
                        ...
                        # print("5' UTR side of altProt")

                        # 5' UTR side of altProt
                        overlapped_region = sorted(list(set(altprot_range) & set(refprot_range)))
                        if len(overlapped_region) > 0:
                    ...
                    if (int(AS1) <= int(RS1)) and (int(AS1) <= int(RS2)) and (int(AS2) >= int(RS1)) and (
                        ...
                        # print("3' UTR side of altProt")
                        # 3' UTR side of altProt
                        overlapped_region = sorted(list(set(altprot_range) & set(refprot_range)))
                        if len(overlapped_region) > 0:
                    ...
                    if isthereanyerror == 1:
                        ...
                        # print("""It may be an error or your altprot belongs to more than one category. Check these entries manually
                        # altprot name is {}""".format(altprot))
    ...
    for final_result in final_results_to_list:
        ...
        if "*" not in sequence_for_final_result[10:]:
            ...
            to_file_for_final_result = to_file_for_final_result + str(
                ">" + str(final_results[final_result].id) + "\n" + str(sequence_for_final_result) + "\n")
    # Was used for version 4 and not necessary for version 5+.
    # log_file = log_file + str("""
    # ------------------------------------------------
    # This file contains error(s) if the correct frameshift could not be determined.
    # If the file is empty, there is no error on determination of the correct frameshift.
    # In the future, more logs will be included.
    # ------------------------------------------------
    # End of the log file
    # (current version: 4.261221)""" )
    ...
    # Define the results directory for hidden files
    results_directory = ".mosaic_prot_results"
    ...
    # Run seqtk command within the .mosaic_prot_results directory
    os.system(
        f"seqtk seq -L 11 clean_upload_result_{idx_num}.fasta > {os.path.join(results_directory, f'requested_file_{idx_num}.fasta')}")

    # Optionally, handle file removal or further processing in the .mosaic_prot_results directory
    # Example for removing clean_upload_result.fasta
    try:
    ...
    # os.system("rm clean_upload_result.fasta")
    # os.system("rm your_final_result.fasta")
    # os.system("rm your_final_result_before_clean.fasta")
    # os.system("rm clean_final_result.fasta")
    ...
    # File path for temp_sequence_archieve.fasta inside .mosaic_prot_results
    ...
    # Run the Perl script to modify the temp file inside .mosaic_prot_results
    os.system(
        f"perl -i.withoutnumbers -pe 'BEGIN{{$N=\"00000001\"}}next unless /^>/;s/(>\\S+)/$1|$N/;$N++' {temp_path_}")

    # Parse the modified fasta file using SeqIO from the .mosaic_prot_results directory
    sequence_archive = SeqIO.to_dict(SeqIO.parse(temp_path_, "fasta"))

    # Parse the modified fasta file using SeqIO
    sequence_archive = SeqIO.to_dict(SeqIO.parse(temp_path_, "fasta"))
    # Define the results directory for hidden files
    results_directory = ".mosaic_prot_results"

    # Construct the full path to requested_file.fasta inside .mosaic_prot_results
    requested_file_path = os.path.join(results_directory, f"requested_file_{idx_num}.fasta")

    # Read the requested_file.fasta from the .mosaic_prot_results directory
    requested_file = SeqIO.to_dict(SeqIO.parse(requested_file_path, "fasta"))

    # Example of how to get the list from requested_file (if you have a getList function defined)
    ...
    # Define the file path inside the hidden directory
    file_path = os.path.join(results_directory, f'requested_file_filtered{idx_num}.fasta')

    # Write the filtered file into the hidden directory
    ...
    # Update the file content using Perl
...
def simulate_chimeric_proteins(input_file, refprots_file, altprots_file, transcript_file, processor_num, temp_file_name):
    """Generate chimeric proteins using multiprocessing."""
    print("Generating chimeric proteins...")

    # Read input and process transcript IDs
    with open(input_file, 'r') as f:
    ...
    list_of_chunks = []
    if processor_num == 1:
        # Combine always_be_together and list_of_altProts into a single chunk
        ...
        # Always treat always_be_together as the first chunk
        list_of_chunks.append(always_be_together_path)

        # Split list_of_altProts.txt into (processor_num - 1) chunks
        split_file(list_of_altProts_path, results_directory, processor_num - 1)

        # Add the split chunks to the list
...
def main():
    ...
    # Use ArgumentDefaultsHelpFormatter to show defaults in help
    ...
    # Subcommand: simulate_chimeric_proteins
    ...
    try:
        ...
        # Create a unique log file name for the selected command
        ...
        if args.command == "detect_ORFs":
            ...
            # Find all files starting with 'requested_file_filtered' and ending with '.fasta' in the hidden directory
            files_to_combine = glob.glob(os.path.join(results_directory, "requested_file_filtered*.fasta"))

            # Name of the final combined file in the hidden directory
            combined_file = os.path.join(results_directory, "simulated_chimeric_proteins.fasta")

            # Combine all files into the final file
            with open(combined_file, "w") as outfile:
                for file in files_to_combine:
                    with open(file, "r") as infile:
                        outfile.write(infile.read())

            # Move the combined file to the current directory where the script resides
            ...
            def find_duplicate_groups(records):
                """Identifies groups of duplicate sequences based on sequence, iteration, and overlapping tags."""
                # Group records by sequence and iteration
                ...
                for key in grouped:
                    ...
                    # Build adjacency list based on tag overlap
            ...
            def handle_duplicates(records, duplicate_groups, option='keep_first'):
                """Processes the records according to the specified duplicate handling option."""
                all_duplicates = set()
                header_to_group = {}
            ...
            def write_fasta(records, output_path):
                """Writes the processed records to a new FASTA file."""
            ...
            def count_categories(records):
                """Counts the categories in the headers of the records."""
                category_counts = defaultdict(int)

                for header, seq in records:
                    base_header = '_'.join(header.split('_')[:-2])

                    # First try to match 'iteration_<number>_<category>'
                    iteration_match = re.search(r"iteration_\d+_(.*)", base_header)
                    if iteration_match:
                        category = iteration_match.group(1)
                    else:
                        # Fallback: extract last part before _CHIMERIC, may include _width
            ...
            # Optionally, delete the .mosaic_prot_results directory if it's empty
            ...
            # Delete the original files in the hidden directory
            for file in files_to_combine:
                os.remove(file)

            # Define the paths of the additional files to remove in the hidden directory
            additional_files = [
            ...
            # Remove additional files if they exist
            for file in additional_files:
                if os.path.exists(file):
                    os.remove(file)

            # Let me remove the other files
        ...
        # Log time taken for the command
