...
async def read_excel_resource(uri:str) -> str:
    ...
    **路径格式要求**：
    1. 必须使用`file://协议开头
    2. 路径必须符合标准URI格式：
    - Windows路径示例：`file:///D:/_Endless/geoaiagent/soildata/安宁河流域粮食数据2021.xlsx`
    3. 特殊字符处理：
    - 空格需要转换为`%20`
    - 中文无需编码（后端自动处理）
    4. 路径分隔符必须使用正斜杠`/`
    ...
    **错误示例分析**：
    `D:\_Endless\geoaiagent\soildata\安宁河流域粮食数据2021.xlsx`
    - 错误原因：缺少协议头，使用反斜杠

    `file://D:\_Endless\geoaiagent\soildata\xxx.xlsx`
    - 错误原因：路径分隔符应为正斜杠

    正确格式：
    `file:///D:/_Endless/geoaiagent/soildata/%E5%AE%89%E5%AE%81%E6%B2%B3%E6%B5%81%E5%9F%9F%E7%B2%AE%E9%A3%9F%E6%95%B0%E6%8D%AE2021.xlsx`

    **转换规则**：
    1. 原始路径 → `D:\_Endless\geoaiagent\soildata\安宁河流域粮食数据2021.xlsx`
    2. 替换反斜杠 → `D:/_Endless/geoaiagent/soildata/安宁河流域粮食数据2021.xlsx`
    3. 添加协议头 → `file:///D:/_Endless/geoaiagent/soildata/安宁河流域粮食数据2021.xlsx`
    4. 编码空格（如有）→ 替换空格为`%20`
    """
    parsed = urlparse(uri)
...
async def read_grain_data(year: int = Tool(name="year", description="要查询的年份，仅包含2021,2022,2023年",inputSchema={"type": "integer"}),
                          ...
                          resource_uri: str = Tool(name="resource_uri",description="待查询文件的uri路径,必须使用`file://`协议开头,路径必须符合标准URI格式：Windows路径示例：`file:///D:/_Endless/geoaiagent/soildata/安宁河流域粮食数据2021.xlsx,路径分隔符必须使用正斜杠`/`",inputSchema={"type": "integer"
        }
    )) -> dict:
    """
    ...
    resource_uri:待查询文件的uri路径,必须使用`file://`协议开头,路径必须符合标准URI格式：Windows路径示例：`file:///D:/_Endless/geoaiagent/soildata/安宁河流域粮食数据2021.xlsx,路径分隔符必须使用正斜杠`/`。根据用户需求的不同，文件路径都与示例保持一致，文件名为安宁河流域粮食数据20xx.xlsx,其中20xx表示具体的年份
    """
    try:
        # 获取CSV数据
...
async def presict_capacity(
    train_uri: str = Tool(name="train_uri", description="训练集路径，必须使用`file://`协议开头,路径必须符合标准URI格式：Windows路径示例：`file:///D:/_Endless/geoaiagent/soildata/产量训练数据.xlsx,路径分隔符必须使用正斜杠`/`",inputSchema={"type": "string"}),
