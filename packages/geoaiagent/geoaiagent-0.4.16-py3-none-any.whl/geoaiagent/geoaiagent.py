...
async def read_grain_data(year: int = Tool(name="year", description="要查询的年份，仅包含2021,2022,2023年",inputSchema={"type": "integer"}),
    ...
    resource_uri:待查询文件的uri路径,必须使用file://协议开头,路径必须符合标准URI格式：Windows路径示例：file:///D:/_Endless/geoaiagent/soildata/安宁河流域粮食数据2021.xlsx,路径分隔符必须使用正斜杠‘/’。根据用户需求的不同，文件路径都与示例保持一致，文件名为安宁河流域粮食数据20xx.xlsx,其中20xx表示具体的年份
    """
    try:
        # 获取CSV数据
