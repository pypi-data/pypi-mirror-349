...
from wiim.bridge import LinkPlayBridge
from wiim.consts import UPNP_DEVICE_TYPE, LinkPlayCommand, MultiroomAttribute
from wiim.endpoint import LinkPlayApiEndpoint, LinkPlayEndpoint
from wiim.exceptions import LinkPlayInvalidDataException, LinkPlayRequestException


@deprecated(
    reason="Use wiim_factory_bridge_endpoint with a LinkPlayEndpoint or wiim_factory_httpapi_bridge instead.",
    version="0.0.7",
)
async def wiim_factory_bridge(
    ...
    try:
        return await wiim_factory_bridge_endpoint(endpoint)
    except LinkPlayRequestException:
        return None


async def wiim_factory_bridge_endpoint(
...
async def wiim_factory_httpapi_bridge(
    ip_address: str, session: ClientSession
) -> LinkPlayBridge:
    """Attempts to create a LinkPlayBridge from the given IP address.
    ...
    return await wiim_factory_bridge_endpoint(http_endpoint)


async def discover_wiim_bridges(
    ...
    async def add_wiim_device_to_list(upnp_device: CaseInsensitiveDict):
        ...
        try:
            bridge = await wiim_factory_httpapi_bridge(ip_address, session)
            bridges[bridge.device.uuid] = bridge
        except LinkPlayRequestException:
            pass

    await async_search(
        search_target=UPNP_DEVICE_TYPE, async_callback=add_wiim_device_to_list
    )
...
async def discover_bridges_through_multiroom(
    ...
    try:
        ...
        for follower in properties[MultiroomAttribute.FOLLOWER_LIST]:
            try:
                new_bridge = await wiim_factory_httpapi_bridge(
