...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="LogLama Decorator Application Tool")
...
class FunctionFinder(ast.NodeVisitor):
    ...
    def __init__(self, include_pattern: Optional[str] = None, exclude_pattern: Optional[str] = None):
        self.functions = []
    ...
    def visit_FunctionDef(self, node):
        ...
        include_match = True if self.include_pattern is None else bool(self.include_pattern.search(node.name))
        exclude_match = False if self.exclude_pattern is None else bool(self.exclude_pattern.search(node.name))
            ...
            for decorator in node.decorator_list:
                if isinstance(decorator, ast.Call) and hasattr(decorator.func, 'id'):
                    if decorator.func.id in [
                        'auto_fix', 'with_diagnostics', 'monitor_performance',
                        'resource_usage_monitor', 'diagnose_on_error', 'log_errors',
...
def find_functions_in_file(file_path: str, include_pattern: Optional[str] = None, exclude_pattern: Optional[str] = None) -> List[Dict[str, Any]]:
...
@with_diagnostics()
def apply_decorator_to_file(file_path: str, decorator: str, params: Optional[Dict[str, Any]] = None, backup: bool = True, dry_run: bool = False, include_pattern: Optional[str] = None, exclude_pattern: Optional[str] = None) -> Dict[str, Any]:
    ...
    functions = find_functions_in_file(file_path, include_pattern, exclude_pattern)
    ...
    functions_to_modify = [f for f in functions if not f['has_pylogs_decorator']]
    
    if not functions_to_modify:
        logger.info(f"All matching functions in {file_path} already have LogLama decorators")
        return result
    ...
    try:
        ...
        for func in sorted(functions_to_modify, key=lambda x: x['lineno'], reverse=True):
            lineno = func['lineno'] - 1  # Convert to 0-indexed
            
            # Check if we need to add imports
            if not any(line.strip().startswith(f"from loglama.decorators.{decorator.split('_')[0]} import {decorator}") for line in lines) and \
               not any(line.strip() == f"from loglama.decorators import {decorator}" for line in lines):
                ...
                if import_line >= 0:
                    ...
                    elif decorator in ["with_diagnostics", "monitor_performance", "resource_usage_monitor", "diagnose_on_error"]:
                        import_str = f"from loglama.decorators.diagnostics import {decorator}\n"
                    elif decorator in ["log_errors", "retry", "fallback", "timeout"]:
                        import_str = f"from loglama.decorators.error_handling import {decorator}\n"
                    else:
                        import_str = f"from loglama.decorators import {decorator}\n"
        ...
        # Log results
        if dry_run:
            logger.info(f"[DRY RUN] Would apply {decorator} to {len(result['functions_modified'])} functions in {file_path}")
        else:
            logger.info(f"Applied {decorator} to {len(result['functions_modified'])} functions in {file_path}")
...
def process_directory(directory: str, decorator: str, params: Optional[Dict[str, Any]] = None, backup: bool = True, dry_run: bool = False, recursive: bool = False, include_pattern: Optional[str] = None, exclude_pattern: Optional[str] = None) -> Dict[str, Any]:
            ...
            for file in files:
                if file.endswith('.py'):
                    ...
                    file_result = apply_decorator_to_file(
                        file_path, decorator, params, backup, dry_run, include_pattern, exclude_pattern
                    )
                    
                    result['files_processed'] += 1
                    if file_result['functions_modified']:
                        result['files_modified'] += 1
                        result['functions_modified'] += len(file_result['functions_modified'])
...
def main() -> int:
    ...
    if os.path.isfile(target_path):
        ...
        result = apply_decorator_to_file(
            target_path, args.decorator, params, args.backup, args.dry_run, args.pattern, args.exclude
        )
        ...
        result = process_directory(
            target_path, args.decorator, params, args.backup, args.dry_run, args.recursive, args.pattern, args.exclude
        )
