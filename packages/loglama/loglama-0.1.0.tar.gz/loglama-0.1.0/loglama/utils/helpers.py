...
def setup_basic_logging(level: Union[str, int] = logging.INFO, format_string: Optional[str] = None):
...
def configure_logging(
    ...
    if structured and STRUCTLOG_AVAILABLE:
        return _configure_structlog(name, level, console, file, file_path, database, db_path, json, config)
    ...
    if file:
        if file_path is None:
            log_dir = os.environ.get("LOGLAMA_LOG_DIR", os.path.expanduser("~/.loglama/logs"))
    ...
    if database:
        if db_path is None:
            db_dir = os.environ.get("LOGLAMA_DB_DIR", os.path.expanduser("~/.loglama/db"))
...
def _configure_structlog(name, level, console, file, file_path, database, db_path, json, config):
    """Configure structured logging with structlog.
    
    This is an internal function used by configure_logging.
    """
    if not STRUCTLOG_AVAILABLE:
        raise ImportError("The 'structlog' package is required for structured logging. "
