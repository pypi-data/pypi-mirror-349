...
def parse_args():
    ...
    verify_parser = subparsers.add_parser("verify", help="Verify logging setup")
    verify_parser.add_argument(
        "--log-dir", "-d", 
        help="Directory for test log files"
    )
    
    # Context diagnostics command
    context_parser = subparsers.add_parser("context", help="Diagnose context issues")
    ...
    db_parser = subparsers.add_parser("database", help="Check database connection")
    ...
    file_parser.add_argument(
        "--log-dir", "-d", 
        help="Directory to check permissions"
    )
    
    # Troubleshoot logging command
    troubleshoot_logging_parser = subparsers.add_parser("troubleshoot-logging", help="Troubleshoot logging issues")
    ...
    troubleshoot_logging_parser.add_argument(
        "--log-level", "-l", 
        default="INFO",
        help="Log level to use for tests"
    )
    
    # Troubleshoot context command
    troubleshoot_context_parser = subparsers.add_parser("troubleshoot-context", help="Troubleshoot context issues")
    ...
    troubleshoot_db_parser = subparsers.add_parser("troubleshoot-database", help="Troubleshoot database issues")
...
def format_result(result, title):
    ...
    if "tests" in result and result["tests"]:
        ...
        for i, test in enumerate(result["tests"], 1):
            ...
            output.append(f"  {i}. {name}: {status}" + (f" - {reason}" if reason else ""))
