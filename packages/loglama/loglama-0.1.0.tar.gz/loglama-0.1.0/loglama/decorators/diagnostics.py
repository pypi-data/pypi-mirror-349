...
def with_diagnostics(run_before: bool = True,
                    run_after: bool = True,
                ...
                if run_before:
                    ...
                    if log_results:
                        if pre_result['status'] == 'healthy':
                            logger.info(f"Pre-execution diagnostics for {func.__name__} passed")
                        else:
                            logger.warning(f"Pre-execution diagnostics for {func.__name__} found {len(pre_result['issues'])} issues")
                    ...
                    if fix_issues and pre_result['issues']:
                        ...
                        if log_results:
                            if fix_results['fixed']:
                                logger.info(f"Fixed {len(fix_results['fixed'])} issues before executing {func.__name__}")
                            if fix_results['failed']:
                                logger.warning(f"Failed to fix {len(fix_results['failed'])} issues before executing {func.__name__}")
                ...
                if run_after:
                    ...
                    if log_results:
                        if post_result['status'] == 'healthy':
                            logger.info(f"Post-execution diagnostics for {func.__name__} passed")
                        else:
                            logger.warning(f"Post-execution diagnostics for {func.__name__} found {len(post_result['issues'])} issues")
                    ...
                    if fix_issues and post_result['issues']:
                        ...
                        if log_results:
                            if fix_results['fixed']:
                                logger.info(f"Fixed {len(fix_results['fixed'])} issues after executing {func.__name__}")
                            if fix_results['failed']:
                                logger.warning(f"Failed to fix {len(fix_results['failed'])} issues after executing {func.__name__}")
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            with LogContext(**context):
                ...
                if execution_time_ms > threshold_ms:
                    ...
                    log_method(f"Slow execution: {func.__name__} took {execution_time_ms:.2f}ms (threshold: {threshold_ms}ms)")
                else:
...
def resource_usage_monitor(memory_threshold_mb: float = 100.0,
                          cpu_threshold_percent: float = 90.0,
                          log_level: str = "WARNING") -> Callable[[F], F]:
    ...
    def decorator(func: F) -> F:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                logger.warning(f"psutil not available, resource monitoring disabled for {func.__name__}")
            ...
            with LogContext(**context):
                if psutil_available:
                    ...
                    cpu_diff = final_cpu_percent - initial_cpu_percent if final_cpu_percent > initial_cpu_percent else 0
                    
                    # Add resource usage to context
                    with LogContext(memory_usage_mb=memory_diff, cpu_usage_percent=cpu_diff):
                        # Log warnings if thresholds are exceeded
                        log_method = getattr(logger, log_level.lower(), logger.warning)
                        
                        if memory_diff > memory_threshold_mb:
                            log_method(f"High memory usage: {func_name} used {memory_diff:.2f}MB (threshold: {memory_threshold_mb}MB)")
                        
                        if cpu_diff > cpu_threshold_percent:
                            log_method(f"High CPU usage: {func_name} used {cpu_diff:.2f}% CPU (threshold: {cpu_threshold_percent}%)")
                        
                        # Log debug message with resource usage
                        logger.debug(f"Function {func_name} used {memory_diff:.2f}MB memory and {cpu_diff:.2f}% CPU")
...
def diagnose_on_error(generate_report: bool = True,
                     report_path: Optional[str] = None,
                     fix_issues: bool = True) -> Callable[[F], F]:
            ...
            with LogContext(**context):
                try:
                    ...
                    if report['status'] == 'healthy':
                        logger.info(f"Diagnostics after error in {func_name} show no issues")
                    else:
                        logger.warning(f"Diagnostics after error in {func_name} found {len(report['issues'])} issues")
                    ...
                    if fix_issues and report['issues']:
                        ...
                        if fix_results['fixed']:
                            logger.info(f"Fixed {len(fix_results['fixed'])} issues after error in {func_name}")
                            # Try running the function again if fixes were applied
                            try:
                                logger.info(f"Retrying {func_name} after fixing issues")
                                return func(*args, **kwargs)
                            except Exception as retry_e:
                                logger.error(f"Retry of {func_name} failed after fixing issues: {str(retry_e)}")
                    ...
                    if generate_report:
                        ...
                        try:
                            ...
                            logger.error(f"Failed to save diagnostic report: {str(report_e)}")
