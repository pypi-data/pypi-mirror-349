...
def check_system_health() -> Dict[str, Any]:
    ...
    if not logging_status["status"]:
        ...
        health_report["recommendations"].extend(logging_status["recommendations"])
    ...
    if not context_status["status"]:
        ...
        health_report["recommendations"].extend(context_status["recommendations"])
        ...
        if not os.path.exists(log_file):
            ...
            with open(log_file, "r") as f:
                ...
                if test_message not in content:
                    result["status"] = False
                    result["issues"].append("Test message not found in log file")
                    result["recommendations"].append("Check file handler configuration")
        
        # Check if the database was created and contains the message
        if not os.path.exists(db_file):
            result["status"] = False
            result["issues"].append("Database file was not created")
            result["recommendations"].append("Check database permissions and path")
        else:
            try:
                conn = sqlite3.connect(db_file)
                cursor = conn.cursor()
                cursor.execute("SELECT * FROM logs WHERE message = ?", (test_message,))
                row = cursor.fetchone()
                if not row:
                    result["status"] = False
                    result["issues"].append("Test message not found in database")
                    result["recommendations"].append("Check SQLite handler configuration")
                conn.close()
            except Exception as e:
                result["status"] = False
                result["issues"].append(f"Database error: {str(e)}")
                result["recommendations"].append("Check database schema and configuration")
    ...
    try:
        ...
        if os.path.exists(log_file):
            with open(log_file, "r") as f:
                ...
                if test_user not in content or test_request_id not in content:
                    ...
                    result["recommendations"].append("Check ContextFilter and JSONFormatter")
        ...
        if os.path.exists(db_file):
            try:
                conn = sqlite3.connect(db_file)
                cursor = conn.cursor()
                cursor.execute("SELECT context FROM logs WHERE message = ?", ("Test context message",))
                row = cursor.fetchone()
                if row:
                    context = json.loads(row[0])
                    if context.get("user") != test_user or context.get("request_id") != test_request_id:
                        result["status"] = False
                        result["issues"].append("Context values incorrect in database")
                        result["recommendations"].append("Check SQLiteHandler context handling")
                else:
                    result["status"] = False
                    result["issues"].append("Context message not found in database")
                    result["recommendations"].append("Check SQLiteHandler emit method")
                conn.close()
            except Exception as e:
                result["status"] = False
                result["issues"].append(f"Database context error: {str(e)}")
                result["recommendations"].append("Check database schema and context handling")
        ...
        if os.path.exists(db_file):
            try:
                conn = sqlite3.connect(db_file)
                cursor = conn.cursor()
                cursor.execute("SELECT context FROM logs WHERE message = ?", ("Thread context test",))
                row = cursor.fetchone()
                if row:
                    context = json.loads(row[0])
                    if context.get("thread_user") != "thread_test" or "user" in context:
                        result["status"] = False
                        result["issues"].append("Thread context not isolated")
                        result["recommendations"].append("Check thread-local storage implementation")
                conn.close()
            except Exception as e:
                result["status"] = False
                result["issues"].append(f"Thread context error: {str(e)}")
                result["recommendations"].append("Check thread-local context handling")
    
    except Exception as e:
        result["status"] = False
        result["issues"].append(f"Context test error: {str(e)}")
        result["recommendations"].append("Check context handling implementation")
...
def check_database_connection(db_path: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        if not cursor.fetchone():
            ...
            result["recommendations"].append("Check SQLiteHandler initialization")
        else:
            # Check table schema
            cursor.execute("PRAGMA table_info(logs)")
            columns = {row[1] for row in cursor.fetchall()}
            required_columns = {
                "id", "timestamp", "level", "level_no", "logger_name", "message",
                "file_path", "line_number", "function", "module", "process",
                "process_name", "thread", "thread_name", "context", "extra"
            }
            
            missing_columns = required_columns - columns
            if missing_columns:
                result["status"] = False
                result["issues"].append(f"Missing columns in logs table: {missing_columns}")
                result["recommendations"].append("Check SQLiteHandler create_table method")
        ...
        try:
            ...
            cursor.execute("SELECT * FROM logs WHERE message = ?", ("Test database message",))
            if not cursor.fetchone():
                result["status"] = False
            ...
            result["recommendations"].append("Check SQLiteHandler emit implementation")
        ...
        result["recommendations"].append("Check database configuration and permissions")
    ...
    try:
        ...
        if not os.path.exists(log_dir):
            try:
                ...
                result["recommendations"].append("Check directory path and permissions")
                return result
        ...
        try:
            ...
            result["recommendations"].append("Check file permissions and disk space")
        ...
        try:
            ...
            if free_space < 10 * 1024 * 1024:  # Less than 10MB
                result["status"] = False
                result["issues"].append(f"Low disk space: {free_space / (1024 * 1024):.2f} MB free")
                result["recommendations"].append("Free up disk space or change log directory")
        except Exception as e:
            result["status"] = False
