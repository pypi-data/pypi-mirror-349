...
try:
    ...
    def _load_dotenv(dotenv_path=None, stream=None, verbose=False, override=False, **kwargs):
...
def find_project_root(start_dir: Optional[Union[str, Path]] = None) -> Path:
    ...
    _logger.warning(f"Could not find project root, using {start_dir} as fallback")
    return start_dir


def get_env_file_paths(custom_path: Optional[Union[str, Path]] = None) -> List[Path]:
    """
    ...
    env_paths.extend([
        ...
        Path(os.path.expanduser("~/.config/loglama/.env")),  # User's config directory
    ])
...
def load_env(env_file: Optional[Union[str, Path]] = None, override: bool = True, verbose: bool = True) -> bool:
    """
    ...
    if not DOTENV_AVAILABLE and verbose:
        _logger.warning("python-dotenv package not found. Install it with 'pip install python-dotenv' for .env file support.")
        return False
    ...
    if not env_loaded and verbose:
        _logger.warning(f"No .env file found in any of these locations: {[str(p) for p in env_paths]}")
        _logger.info(f"Using default environment variables")
...
def get_env(key: str, default: Any = None, as_type: Optional[type] = None) -> Any:
    """
    ...
    try:
        ...
        _logger.warning(f"Error converting environment variable {key}={value} to {as_type.__name__}: {e}")
        return default
