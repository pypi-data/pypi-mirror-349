...
class MemoryHandler(logging.Handler):
    ...
    def emit(self, record):
        ...
        try:
            ...
            if record.exc_info:
                record_dict["exception"] = self.formatter.formatException(record.exc_info) if self.formatter else logging.Formatter().formatException(record.exc_info)
    ...
    def get_records(self, level: Optional[int] = None, limit: Optional[int] = None) -> List[Dict]:
        ...
        if level is None:
            ...
            filtered_records = [r for r in self.records if r["level_no"] >= level]
