...
from isapilib.base.functions import lazy_foreign_key
from isapilib.core.models import BaseModel, DummyForeignKey

art_model_path = lazy_foreign_key('ART_MODEL', 'isapilib.Art')
sucursal_model_path = lazy_foreign_key('SUCURSAL_MODEL', 'isapilib.Sucursal')
almacen_model_path = lazy_foreign_key('ALMACEN_MODEL', 'isapilib.Almacen')
cte_model_path = lazy_foreign_key('CTE_MODEL', 'isapilib.Cte')
empresa_model_path = lazy_foreign_key('EMPRESA_MODEL', 'isapilib.Empresa')
agente_model_path = lazy_foreign_key('AGENTE_MODEL', 'isapilib.Agente')
vin_model_path = lazy_foreign_key('VIN_MODEL', 'isapilib.Vin')
venta_model_path = lazy_foreign_key('VENTA_MODEL', 'isapilib.Venta')
compra_model_path = lazy_foreign_key('COMPRA_MODEL', 'isapilib.Compra')
vin_tipo_accesorio_model_path = lazy_foreign_key('VINTIPOACCESORIO_MODEL', 'isapilib.VinTipoAccesorio')


class BaseAlmacen(BaseModel):
...
class BaseArtExistenciaNeta(BaseModel):
...
class BaseCte(BaseModel):
    ...
    tipo = models.CharField(db_column='Tipo', max_length=15, blank=True, null=True)
    situacion = models.CharField(db_column='Situacion', max_length=50, blank=True, null=True)
    agente = DummyForeignKey(agente_model_path, db_column='agente', to_field='agente', on_delete=models.PROTECT,
...
class BaseCompra(BaseModel):
    ...
    empresa = DummyForeignKey(empresa_model_path, db_column='Empresa', to_field='empresa', on_delete=models.PROTECT,
                              related_name='compras')
    sucursal = DummyForeignKey(sucursal_model_path, db_column='Sucursal', to_field='sucursal', on_delete=models.PROTECT,
                               related_name='compras')
    almacen = DummyForeignKey(almacen_model_path, db_column='Almacen', to_field='almacen', on_delete=models.PROTECT,
...
class BaseVin(BaseModel):
    vin = models.CharField(db_column='VIN', primary_key=True, max_length=20)
    articulo = DummyForeignKey(art_model_path, db_column='Articulo', to_field='articulo', on_delete=models.PROTECT,
    ...
    cliente = DummyForeignKey(cte_model_path, db_column='Cliente', to_field='cliente', on_delete=models.PROTECT,
                              related_name='vins')
    ...
    compra = DummyForeignKey(compra_model_path, db_column='CompraID', to_field='id', on_delete=models.PROTECT,
                             related_name='vins', blank=True, null=True)
...
class BaseVinTipoAccesorio(BaseModel):
...
class BaseSerieLote(BaseModel):
    empresa = DummyForeignKey(empresa_model_path, db_column='Empresa', to_field='empresa', on_delete=models.PROTECT,
                              related_name='series_lote')
    sucursal = DummyForeignKey(sucursal_model_path, db_column='Sucursal', to_field='sucursal', on_delete=models.PROTECT,
                               related_name='series_lote')
    DummyForeignKey(almacen_model_path, db_column='Almacen', to_field='almacen', on_delete=models.PROTECT,
                    related_name='series_lote')
...
class BaseVinAccesorio(BaseModel):
    vin = DummyForeignKey(vin_model_path, db_column='VIN', to_field='vin', on_delete=models.PROTECT,
                          related_name='accessories')
    tipo = DummyForeignKey(vin_tipo_accesorio_model_path, db_column='Tipo', to_field='tipo', on_delete=models.PROTECT,
...
class BaseVenta(BaseModel):
    ...
    servicio_articulo = DummyForeignKey(art_model_path, db_column='ServicioArticulo', to_field='articulo',
                                        on_delete=models.PROTECT,
                                        related_name='servicios', blank=True, null=True)
    servicio_serie = DummyForeignKey(vin_model_path, db_column='ServicioSerie', to_field='vin', on_delete=models.PROTECT,
                                     related_name='ventas')
...
class BaseVentaD(BaseModel):
    venta = DummyForeignKey(venta_model_path, db_column='ID', to_field='id', on_delete=models.PROTECT, related_name='details')
    ...
    sucursal = DummyForeignKey(sucursal_model_path, db_column='Sucursal', to_field='sucursal', on_delete=models.PROTECT,
                               related_name='details')
    sucursal_origen = DummyForeignKey(sucursal_model_path, db_column='SucursalOrigen', to_field='sucursal',
                                      on_delete=models.PROTECT,
...
class BaseTipoOrdenOperacion(BaseModel):
...
class BaseMaestroTipoOrdenOperacion(BaseModel):
...
class BaseMensajeLista(BaseModel):
...
class BaseVentaTraspasarArticulos(BaseModel):
...
class BaseFormaPagoTipo(BaseModel):
...
class BaseListaPreciosD(BaseModel):
