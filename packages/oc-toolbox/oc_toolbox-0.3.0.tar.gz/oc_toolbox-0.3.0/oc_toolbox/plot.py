...
def plot_heatmap_confusion_matrix(
    ...
    ----------
    y_true : pd.Series
        Série contenant les vraies classes.
    y_pred : pd.Series
        Série contenant les classes prédites.
    cmp : str, optional
        Palette de couleurs à utiliser pour la heatmap (par défaut "Blues").
    figsize : tuple of int, optional
        Taille de la figure matplotlib (par défaut (6, 6)).
    y_mapping : dict of int to str, optional
        Dictionnaire de correspondance entre les labels numériques et leurs
        noms lisibles. Si None, les indices numériques sont utilisés.
    ...
    y_values = list(sorted(np.unique(y_true)))
