...
def create_api_key(self, name, scope='user', organizationId=None, workspaceId=None, expires=None):
    ...
    if organizationId and organizationId not in [org['organizationId'] for org in self.organizations]: raise Exception("Organization ID not found.")
    if workspaceId and workspaceId not in [workspace['workspaceId'] for workspace in self.workspaces]: raise Exception("Workspace ID not found.")
