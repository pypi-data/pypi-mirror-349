...
pub fn generate_emulation_headers(
  ...
  let rust_viewport: Option<Viewport> = viewport.and_then(|vp| {
    ...
    let width = vp_ref.get_item("width").and_then(|w| match w {
      Some(w) => Ok(w.extract::<u32>().ok().unwrap_or_default()),
      _ => Ok(0),
    });
    let height = vp_ref.get_item("height").and_then(|w| match w {
      Some(w) => Ok(w.extract::<u32>().ok().unwrap_or_default()),
      _ => Ok(0),
    });
