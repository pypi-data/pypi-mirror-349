...
class GameExecutor(ExecutorInterface):
    ...
    def _make_ml_execute(self, scene_info_dict, keyboard_info) -> dict:
        ...
        try:
            ...
            for ml_name in self._active_ml_names:
                logger.debug(f"send_to_ml to {ml_name} at {self._game_executor_frame_count}")
        ...
        logger.debug(f"recv_from_all_ml at {self._game_executor_frame_count}")
    ...
    def _check_delay(self, ml_name, cmd_frame):
        ...
        if delayed_frame > 0:
            logger.debug(f"The client '{ml_name}' delayed {delayed_frame} frame at game frame({self.game.frame_count})")
