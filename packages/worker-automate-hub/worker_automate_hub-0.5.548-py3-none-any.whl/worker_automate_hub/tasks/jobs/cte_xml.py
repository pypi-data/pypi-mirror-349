...
async def selecionar_xml(cte):
    ...
    try:
        # Clica no ComboBox de "Download
        combo = main_window.child_window(class_name="ComboBox", found_index=0).wrapper_object()
        combo.click_input()

        await worker_sleep(1)
        
        # Caminho da imagem
        imagem = f"{ASSETS_BASE_PATH}download_bt.png"

        # Localiza e clica na imagem (com confiança 90%)
    ...
    # Clica no ComboBox de "Download"
    combo = main_window.child_window(class_name="ComboBox", found_index=1).wrapper_object()
    combo.click_input()
...
async def janela_conhecimento_frete(cte):
    ...
    try:
        ...
        btn_salvar = main_window.child_window(class_name="TDBIBitBtn", found_index=4).click()
