...
This module provides the main JtechMCPAgent class that integrates all components
...
from jtech_mcp_executor.connectors.base import BaseConnector
from jtech_mcp_executor.session import JtechMCPSession
...
class JtechMCPAgent:
    ...
    def __init__(
        ...
        client: JtechMCPClient | None = None,
        ...
        Args:
            ...
            client: The JtechMCPClient to use. If provided, connector is ignored.
        ...
        self._sessions: dict[str, JtechMCPSession] = {}
        self._system_message: SystemMessage | None = None
        self._tools: list[BaseTool] = []

    async def initialize(self) -> None:
        """Initialize the MCP client and agent."""
        logger.info("🚀 Initializing Jtech MCP agent and connecting to services...")
    ...
    async def _generate_response_chunks_async(
        ...
        if not self._agent_executor:
            raise RuntimeError("Jtech MCP agent failed to initialise – call initialise() first?")
    ...
    async def run(
        ...
        try:
            ...
            if not self._agent_executor:
                raise RuntimeError("Jtech MCP agent failed to initialize")
