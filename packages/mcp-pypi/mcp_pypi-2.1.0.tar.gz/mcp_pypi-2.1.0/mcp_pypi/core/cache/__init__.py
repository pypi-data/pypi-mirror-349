    ...
    async def set(self, key: str, data: Dict[str, Any], etag: Optional[str] = None) -> None:
        ...
        try:
            ...
            if estimated_size > self.config.cache_max_size * 0.8:
                logger.warning(f"Data for key {key} is too large ({estimated_size} bytes) to cache (limit: {int(self.config.cache_max_size * 0.8)} bytes)")
                return
    ...
    async def _prune_cache_if_needed(self) -> None:
        ...
        try:
            ...
            if cache_size > self.config.cache_max_size:
                logger.info(f"Cache size ({cache_size} bytes) exceeds max ({self.config.cache_max_size} bytes), pruning...")
                await self._prune_cache(target_size=int(self.config.cache_max_size * 0.8))
        except Exception as e:
            ...
            async with self._cache_lock:
                for file_path, _ in cache_files:
                    ...
                    try:
                        ...
                        logger.debug(f"Pruned cache file: {file_path.name} ({file_size} bytes)")
                    except Exception as e:
                        logger.warning(f"Failed to remove cache file {file_path}: {e}")
            
            logger.info(f"Pruned cache from {current_size} bytes to {remaining_size} bytes")
        except Exception as e:
