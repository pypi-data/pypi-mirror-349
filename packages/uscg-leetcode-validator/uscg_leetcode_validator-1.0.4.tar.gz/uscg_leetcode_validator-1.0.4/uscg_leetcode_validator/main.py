...
def run_test_cases(player_path, test_cases_path):
    ...
    if DEBUG_MODE:
        print("Case\tInput\tPlayer Output\tExpected\tTime (sec)\tCorrect")
        i = 1
        for inp, outp, expected, t, correct in test_results:
            print(f"{t}\t{inp}\t{outp}\t{expected}\t{t:.6f}\t{correct}")
            i+=1
    else:
        print("Case\tInput\tPlayer Output\tTime (sec)\tCorrect")
        i = 1
        for inp, outp, _, t, correct in test_results:
            print(f"{t}\t{inp}\t{outp}\t{t:.6f}\t{correct}")
            i+=1
