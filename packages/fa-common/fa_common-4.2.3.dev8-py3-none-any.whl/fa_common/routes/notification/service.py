...
async def send_email(
    to_email: str, subject: str, body: str, body_type: EmailBodyType = EmailBodyType.PLAIN, attachments: Optional[List[Attachment]] = None
):
        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                if workflow is not None:
                    ...
                    {stat_msg}
                    {summary_msg if metadata.show_workflow_details or workflow.status == JobStatus.FAILED else ""}

                    {link_to_res}<br><br>

                    {metadata.success_content.body if metadata.success_content and workflow.status == JobStatus.SUCCEEDED else ""}
                    {metadata.failed_content.body if metadata.failed_content and workflow.status == JobStatus.FAILED else ""}
                    </html>
                    </body>
                    """
                    ...
                    emails = [
                        metadata.user_email,
                        metadata.failed_email if metadata.failed_email and workflow.status == JobStatus.FAILED else "",
                    ]
                    await send_email(
                        ";".join(emails),
                        final_subject,
