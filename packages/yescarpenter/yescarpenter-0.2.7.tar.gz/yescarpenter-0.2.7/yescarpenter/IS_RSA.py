...
def align_data(*data_inputs):
    ...
    for i in range(len(aligned_data)):
        if isinstance(aligned_data[i], pd.DataFrame):
            ...
            print(f"The {i}th aligned data is a DataFrame with shape: {aligned_data.shape}")
        else:
            aligned_data[i] = aligned_data[i][keep_mask]
            print(f"The {i}th aligned data is a NumPy array with shape: {aligned_data.shape}")
