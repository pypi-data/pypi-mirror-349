...
class ParadigmPatterns(dict):
    ...
    def from_csv(self, path, pair, patterns_map, collection,
        ...
        self._check_missing_patterns(paradigms, table, pair)
    ...
    def _check_missing_patterns(self, paradigms, table, pair):
        """
        This is a security check that patterns do exist for all pairs of cells.
        To reduce computation time, we first check that the number of pairs is correct.
        If this fails, we check all pairs of cells one by one.
        """
        # Keep only those lexemes that have a record for both cells.
        lexeme_cellcounts = paradigms[paradigms.cell.isin(pair)]\
            .groupby('lexeme', observed=False).cell.nunique()
        paradigm_sample = paradigms.loc[paradigms.cell.isin(pair) &
                                        paradigms.lexeme.isin(
                                            lexeme_cellcounts[lexeme_cellcounts == 2].index),
                                        ['lexeme', 'cell']].reset_index()
        a = paradigm_sample[paradigm_sample.cell == pair[0]]
        b = paradigm_sample[paradigm_sample.cell == pair[1]]

        if len(pd.merge(a, b, on="lexeme")) != len(table):
            form_pairs = pd.merge(a, b, on="lexeme")
            table_pairs = table[['form_x', 'form_y']].apply(tuple, axis=1).to_list()
            missing = form_pairs[form_pairs.apply(lambda x: (x.form_id_x, x.form_id_y) not in table_pairs, axis=1)]
            raise ValueError("It looks like your paradigms and your patterns are not aligned."
                             f"Check that you have patterns for all pairs, missing {pair}:"
