...
from kvirt.common import error, pprint, success, warning, ssh, _ssh_credentials, container_mode, _filter_info_plan
...
def snapshotdelete_vm(args):
    ...
    code = common.handle_response(result, snapshot, element='', action='snapshot deleted')
    sys.exit(code)
...
def snapshotrevert_vm(args):
    ...
    code = common.handle_response(result, snapshot, element='', action='snapshot reverted')
    sys.exit(code)
