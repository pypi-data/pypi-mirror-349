...
#include "../deform_conv_transpose3d.h"
    ...
    namespace ops {
        namespace {
            ...
            public:
                static torch::autograd::Variable forward(
                    ...
                    auto output = deform_conv_transpose3d_symint(
                            input,
                            weight,
                            offset,
                            mask,
                ...
                static torch::autograd::variable_list backward(
                    ...
                    auto grads = detail::_deform_conv_transpose3d_backward_symint(
                            grad_output[0],
                            input,
                            weight,
                            offset,
            ...
            class DeformConvTranspose3dBackwardFunction
                    : public torch::autograd::Function<DeformConvTranspose3dBackwardFunction> {
            public:
                ...
                static torch::autograd::variable_list backward(
                        torch::autograd::AutogradContext *ctx,
                        const torch::autograd::variable_list &grad_output) {
                    TORCH_CHECK(0, "double backwards on deform_conv_transpose3d not supported");
                }
            };

            at::Tensor deform_conv_transpose3d_autograd(
            ...
            deform_conv_transpose3d_backward_autograd(
                    ...
                    at::SymIntArrayRef dilation,
                    c10::SymInt groups) {
                auto result = DeformConvTranspose3dBackwardFunction::apply(
                        grad,
                        input,
                        weight,
                        offset,
        ...
        TORCH_LIBRARY_IMPL(tvdcn, Autograd, m) {
            m.impl(
                    ...
                    TORCH_FN(deform_conv_transpose3d_autograd));
            m.impl(
                    TORCH_SELECTIVE_NAME("tvdcn::_deform_conv_transpose3d_backward"),
                    TORCH_FN(deform_conv_transpose3d_backward_autograd));
        }
    }
}
