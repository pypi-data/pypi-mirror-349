...
os.environ['CAL_DATA_ROOT'] = '.'
config = '''[reference-model]
model = ../example_model_files/H1_20240311.ini
[sensing-measurement]
mcmc = sensing_mcmc_chain.hdf5
gpr = 
[x-arm-measurement]
tst_mcmc =
pum_mcmc =
...
sample = True'''
test_unc = pydarm.uncertainty.DARMUncertainty(
    '../example_model_files/H1_20240311.ini', config)
frequencies = np.logspace(0, np.log10(5000.), 100)
samples = test_unc.compute_response_uncertainty(
    1394625618, frequencies, trials=1000)
response_mag_quant, response_pha_quant = test_unc.response_quantiles(samples)
fig = test_unc.plot_response_samples(frequencies, [], [],
                              response_mag_quant, response_pha_quant, 'H1',
                              filename='after_change.png')
