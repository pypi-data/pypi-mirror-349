...
class Settings {
    ...
    getTrackDownloadParams() {
        ...
        let audioType = audioTypeElem.value;

        // Legacy
        if (audioType == "mp4_aac") {
            audioTypeElem.value = "mp3_with_metadata";
        }

        // Safari
        if (audioType.startsWith('webm') &&
            audioElement.canPlayType("audio/webm;codecs=opus") != "probably" &&
            audioElement.canPlayType("audio/mpeg;codecs=mp3") == "probably") {
        ...
        return /** @type {[string, boolean, boolean]} */ ([audioType, stream, memeCover]);
    }
}
