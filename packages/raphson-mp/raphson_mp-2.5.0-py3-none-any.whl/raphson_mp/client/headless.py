    ...
    async def state(self) -> PlayerState:
        ...
        if currently_playing:
            track = Track(
                ...
                (
                    currently_playing["display"] if "display" in currently_playing else currently_playing["path"]
                ),  # missing in 1.2.1 and before
                currently_playing["mtime"] if "mtime" in currently_playing else 0,  # missing in 1.2.1 and before
                currently_playing["ctime"] if "ctime" in currently_playing else 0,  # missing in 1.2.1 and before
                ...
                currently_playing["tags"] if "tags" in currently_playing else [],  # missing in 1.2.1 and before
                currently_playing["video"] if "video" in currently_playing else None,  # missing in 1.2.1 and before
            )
        else:
            track = None
