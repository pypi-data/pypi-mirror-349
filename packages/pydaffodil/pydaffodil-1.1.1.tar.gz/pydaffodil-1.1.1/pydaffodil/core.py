...
if __name__ == "__main__":
    ...
    ssh_key_path = "/path/to/your/private_key"  # Optional
    ssh_key_pass = "your_key_passphrase"  # Optional, if your key has a passphrase

    cli = Daffodil(remote_user=remote_user, remote_host=remote_host, remote_path=remote_path,
                     ssh_key_path=ssh_key_path, ssh_key_pass=ssh_key_pass)

    steps = [
        {"step": "Creating remote directory", "command": lambda: cli.make_directory("release")},
        {"step": "Transferring files", "command": lambda: cli.transfer_files("./", f"{cli.remote_path}/release")},
        {"step": "Running remote commands", "command": lambda: cli.ssh_command("cd /var/www/your_project/release && echo 'Deployment done!'")},
    ]
