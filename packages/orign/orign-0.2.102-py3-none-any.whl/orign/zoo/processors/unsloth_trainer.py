...
def train_unsloth_sft(message: Message[TrainingRequest]) -> TrainingResponse:
    ...
    try:
        ...
        if adapter_resource:
            ...
            try:
                # Instead of syncing all checkpoints, only sync the latest one
                latest_checkpoint = find_latest_checkpoint(sft_checkpoints_bucket_uri)
                if latest_checkpoint:
                    # Extract checkpoint number from path
                    checkpoint_name = os.path.basename(latest_checkpoint)
                    # Create the checkpoint directory locally
                    local_checkpoint_dir = os.path.join(
                        local_sft_runs_dir, checkpoint_name
                    )
                    os.makedirs(local_checkpoint_dir, exist_ok=True)
                    # Sync only the latest checkpoint
                    bucket.sync(latest_checkpoint, local_checkpoint_dir)
                    print(
                        f"Synced latest SFT checkpoint {checkpoint_name} to {local_checkpoint_dir}"
                    )
                    sft_checkpoint_to_resume_from = local_checkpoint_dir
                    print(
                        f"Will resume from checkpoint: {sft_checkpoint_to_resume_from}"
                    )
                else:
                    print(f"No SFT checkpoint found in {sft_checkpoints_bucket_uri}")
                    is_continue = False
            except Exception as e:
                print(
