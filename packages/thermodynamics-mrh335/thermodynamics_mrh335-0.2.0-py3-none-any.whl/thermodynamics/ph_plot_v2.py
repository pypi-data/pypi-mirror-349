...
def plot_ph_v2():
    ...
    def calculate_slhx_state_points(refrigerant='R134a', effectiveness=0.6, T_evap_C=0, T_cond_C=45, superheat=5, subcool=5, use_slhx=True, use_two_phase_evap_exit=False, high_side_pressure_bar=None):
        ...
        if use_slhx:
            ...
            # Method 3: Rigorous Cmin * ΔT approach
    # =============================================================================
    #         cp_hot = CP.PropsSI('C', 'T', T3, 'P', P_cond, refrigerant)
    #         cp_cold = CP.PropsSI('C', 'T', T6, 'P', P_evap, refrigerant)
    #         C_min = min(cp_hot, cp_cold)
    #         delta_T = T3 - T6
    #         q_max_method3 = C_min * delta_T
    # =============================================================================
            # print(f"q_max (method 3 - Cmin * ΔT): {q_max_method3 / 1000:.3f} kJ/kg")
        ...
        # State points:
        # 1: Suction line after SLHX (pre-compression)
        # 2: Discharge after compression
        # 3: Condenser outlet (before SLHX)
        # 4: High-pressure line after SLHX (before expansion valve)
        # 5: Evaporator inlet (after expansion valve)
        # 6: Evaporator outlet (before SLHX)
    ...
    # Call the function with SLHX and superheated evaporator exit (R134a)
    ...
    # Call the function for transcritical CO2 operation
    # data_co2_transcritical = calculate_slhx_state_points(refrigerant='CO2', use_slhx=True, use_two_phase_evap_exit=False)


    # Cycle sequence

    # Create enthalpy and pressure lists for plotting the cycle
    cycle_points = [1, 2, 3, 4, 5, 6, 1]
    cycle_h = [data_with_slhx[i]['H'] / 1000 for i in cycle_points]  # kJ/kg
    cycle_p = [data_with_slhx[i]['P'] / 1e5 for i in cycle_points]  # bar

    # Plot cycle
    cycle_h_plot = [val / 1 for val in cycle_h]
    cycle_p_plot = [val / 1 for val in cycle_p]
    label = 'Cycle with SLHX' if use_slhx else 'Cycle without SLHX'
    ax.plot(cycle_h_plot, cycle_p_plot, 'ko-', lw=2, label=label)
    ax.legend()
    ...
    if use_slhx:
      ...
      ax.set_title(f'Pressure-Enthalpy Diagram for {refrigerant.upper()} without SLHX')
