...
def plot_ndvi_timeseries_from_csv(csv_path, polygon_id=None, base_dir=None, sample_size=None, 
    ...
    for idx, pid in enumerate(polygon_ids):
        ...
        if len(polygon_df) == 0:
            print(f"No se encontraron datos para el polígono {pid} en el CSV")
            continue
        ...
        if num_bands == 0:
            print(f"No se encontraron columnas de banda en el CSV para el polígono {pid}")
            continue
        ...
        if not dates or len(dates) != num_bands:
            print(f"Extrayendo series para el polígono {pid}. Usando índices numéricos.")
        ...
        if include_mean or include_std:
            ...
            if include_mean:
                plt.plot(range(num_bands), mean_values, color=colors[idx % len(colors)], linewidth=2)
...
if __name__ == "__main__":
    ...
    csv_path = "/home/agrosavia/Documents/rs_agrosavia/DATA_CUBE_AGROSAVIA/ROI/GIS_FEDEPANELA/8.CSV_ALL/669_chipata_all_polygons_cluster_signatures.csv"
    
    # Directorio base donde se encuentran las carpetas de los polígonos
    base_dir = "/home/agrosavia/Documents/rs_agrosavia/DATA_CUBE_AGROSAVIA/ROI/GIS_FEDEPANELA/5.RECORTES/669_chipata"
    
    # Parámetros configurables
    #polygon_id = None  # Si es None, grafica todos los polígonos
    polygon_id = 1405.0 # Si se especifica, grafica solo este polígono
