...
class Connection(object):
    ...
    def register_job(self, description, notes="", batch_size=1,
                     dry_run=False, no_history=False):
        """
        ...
        :param bool no_history: if True no history will be generated for the job
        ...
        if not dry_run:
            data = self.shooju_api.register_job(description, notes, no_history=no_history)
            return RemoteJob(
...
class ShoojuApi(object):
    ...
    def register_job(self, description, notes='', source='python', no_history=False):
        """
        ...
        :param bool no_history: whether to write history
        :return: api response
        :rtype: dict
        """
        payload = {
...
class BaseJob(object):
    ...
    def delete_reported(
        self,
        series_query: str,
        reported_dates: Optional[Union[str, datetime, List[Union[str, datetime]]]] = None,
        ...
        :param reported_dates: `[default: empty]` A single date, list of dates, or None, If 'all', deletes all reported dates.
        :param delete_all: must be set to True to remove all reported dates when reported_dates is empty.

        Deletes reported date(s) for a given series.
        """
        pass
...
class RemoteJob(BaseJob):
    ...
    def submit(self):
        ...
        for (query, reported_date), values in self._queries.items():
            ...
            if reported_date is not None:
                if reported_date != 'all':
                    tmp_dict['reported_date'] = to_milli(reported_date)
                else:
                    tmp_dict['reported_date'] = reported_date
    ...
    def _delete_reported_date(self, series_query: str, reported_date: Union[str, datetime]) -> Optional[ShoojuApiError]:
        """
        Deletes reported dates - for a given series and reported date using the /series/write API.

        :param series_query: query string
        :param reported_dates: `[default: empty]` delete only specific reported dates. If not provided will delete all.
        :param delete_all: must be set to True to remove all reported dates when reported_dates is empty.
        """
        if not (reported_date == 'all' or isinstance(reported_date, datetime)):
            raise ShoojuApiError("reported_date must be a datetime object or the string 'all'")

        self._init_get_query_dict(
            query=series_query,
            delete=True,
    ...
    def delete_reported(
        ...
        reported_dates: Optional[Union[str, datetime, List[Union[str, datetime]]]] = None,
        delete_all: bool = False
    ) -> Optional[ShoojuApiError]:
        """
        Deletes reported date(s) for a given series.

        Args:
            series_query (str): Query string identifying the target series
            reported_dates (Optional[Union[str, datetime, List[Union[str, datetime]]]]): 
                A single date, list of dates, or None.
                If 'all', deletes all reported dates
            delete_all (bool): Must be True if reported_dates is not provided to delete all data points
        Raises:
            ShoojuApiError: If arguments are invalid or deletion fails
        """
        if reported_dates:
            if delete_all:
                raise ShoojuApiError("Delete_all = True required if reported_dates is not provided")

        if not reported_dates:
            if not delete_all:
                raise ShoojuApiError("Must set delete_all=True to delete all reported dates")
            reported_date = "all"
        elif isinstance(reported_dates, (list, tuple)):
            for date in reported_dates:
                self._delete_reported_date(series_query, date)
            return
        else:
            reported_date = reported_dates  # single value fallback

        self._delete_reported_date(series_query, reported_date)
