        ...
        with self:
            for packet in self.container.demux(self.streams + self.audio_streams):
                ...
                while (result := self.alpha_merger.pull()) is not None:
                    yield result
        if self.alpha_merger is not None:
...
class AlphaMerger(BaseAlphaMerger, Filter):
...
class BaseOverlayer(Filter):
    def __init__(
        ...
        pre_overlay_filter: Callable[
            [av.filter.Graph, av.filter.context.FilterContext], av.filter.context.FilterContext
        ] = lambda g, f: f,
