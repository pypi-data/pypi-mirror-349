...
class ExcelModifier:
    ...
    def use_sheet(self, sheet_name: str) -> bool:
        """
        切换到指定名称的工作表。
    ...
    def write_cell(self, sheet_name: str, row_no: int, col_no: int, content: Any) -> bool:
        ...
        :return: 操作状态（成功为 True，失败为 False）
        """
        if not isinstance(row_no, int) or not isinstance(col_no, int) or not self.use_sheet(sheet_name):
            logger.warning(
    ...
    def write_row(self, sheet_name: str, row_no: int, row_data: List[Any]) -> bool:
        ...
        :return: 操作状态（成功为 True，失败为 False）
        """
        if not isinstance(row_no, int) or not self.use_sheet(sheet_name):
            logger.warning(
    ...
    def write_rows(self, sheet_name: str, rows_data: List[List[Any]], start_row_no: Optional[int] = None) -> bool:
        ...
        :return: 操作状态（成功为 True，失败为 False）
        """
        if not self.use_sheet(sheet_name):
            return False
        try:
            if start_row_no:
