...
class MaskedGlobalMaxPooling1D(layers.Layer):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.supports_masking = True  # Let Keras know this layer handles masks
    ...
    def call(self, inputs, mask=None):
        if mask is not None:
            ...
            # Mask the inputs by setting padding positions to a large negative value
            # So they won't affect max
            inputs = inputs - (1.0 - mask) * 1e9

        return tf.reduce_max(inputs, axis=1)

    def compute_mask(self, inputs, mask=None):
        # This layer outputs a single vector per sample, so no mask needed
        return None
