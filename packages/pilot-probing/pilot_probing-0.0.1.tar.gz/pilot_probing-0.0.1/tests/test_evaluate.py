...
from pilot_probing.eval.eval_client import eval_service_input_response_evaluate
from pilot_probing.config import config as actual_global_config
from pilot_probing.eval.models import Metric
...
def mock_get_config(monkeypatch):
    """
    Fixture to configure the global pilot_probing.config.config instance for tests.
...
def test_evaluate_success_local_debug(
    ...
    assert 'X-Top-User-Id' in call_args[1]['headers']
    assert call_args[1]['json']['EvalDataExample'] == example_input_response_payload_data
    assert call_args[1]['json']['MetricPrompt'] == sample_metric_template.model_dump()
