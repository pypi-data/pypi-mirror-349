...
from pilot_probing.optimize.optimize_client import optimize_service_start, optimize_service_get_progress
from pilot_probing.config import Config
from pilot_probing.optimize.models import (
    OptimizeServiceStartOptimizeResult,
    OptimizeServiceProgressResult,
    OptimizeJobInfoPayload,
    OptimizeProgress,
)
import pilot_probing.http_client
...
def reset_http_client_singleton_before_each_test():
    """
    Ensure the pilot_probing.http_client.http_client singleton is reset before each test run.
    This will force get_http_client() to create a new instance for each test.
    """
    pilot_probing.http_client.http_client = None


@pytest.fixture
def mock_get_config():
    """Fixture to mock get_config."""
    with patch('pilot_probing.http_client.get_config') as mock_get:
...
def mock_requests_post():
    ...
    with patch('pilot_probing.http_client.requests.post') as mock_post:
...
def test_start_optimize_success_local_debug(mock_get_config, mock_requests_post):
    ...
    assert 'X-Top-Account-Id' in call_args[1]['headers']
    assert {'TaskId': TEST_TASK_ID, 'Version': TEST_VERSION}.items() <= call_args[1]['json'].items()
...
def test_get_progress_missing_api_key_production(mock_get_config, mock_requests_post):
    ...
    with patch('pilot_probing.optimize.optimize_client.hasattr') as mock_hasattr:
