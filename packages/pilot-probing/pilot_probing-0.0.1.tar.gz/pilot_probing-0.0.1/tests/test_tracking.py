...
from pilot_probing.tracking import track_event, wrap
from pilot_probing.models import TrackingEvent
from pilot_probing.context import tags_ctx, Run
from pilot_probing.event_queue import EventQueue
...
class TestTrackEvent:
    ...
    def mock_config(self):
        ...
        with patch('pilot_probing.config.get_config', autospec=True) as mock_get_config:
            config = MagicMock()
    ...
    def test_track_event_basic(self):
        ...
        # Apply patches
        with (
            patch('pilot_probing.config.get_config', return_value=mock_config),
            patch('pilot_probing.utils.create_uuid_from_string', side_effect=mock_create_uuid_side_effect),
        ):
    ...
    def test_track_event_with_verbose_logging(self, caplog):
        ...
        # Apply patches
        with patch('pilot_probing.config.get_config', return_value=mock_config):
            # Call track_event
            track_event(
                run_type='test_run',
                event_name='test_event',
...
class TestWrap:
    ...
    def mock_track_event(self):
        ...
        track_event_module = sys.modules['pilot_probing.tracking']
    ...
    def mock_run_manager(self):
        ...
        module = sys.modules['pilot_probing.tracking']
