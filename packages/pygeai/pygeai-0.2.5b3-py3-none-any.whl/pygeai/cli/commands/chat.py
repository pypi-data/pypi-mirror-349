...
def chat_with_agent(option_list: list):
    ...
    for option_flag, option_arg in option_list:
        ...
        if option_flag.name == "gui":
            use_gui = get_boolean_value(option_arg)
    ...
    if use_gui:
        ...
        if use_gui:
            ...
            try:
                # Dynamically locate ui.py within the pygeai package
                with resources.path("pygeai.chat", "ui.py") as ui_path:
                    ui_file_path = str(ui_path)
                # Prepare Streamlit command
                streamlit_cmd = [
                    "streamlit", "run", ui_file_path,
                    "--server.address", "127.0.0.1",
                    "--", "--agent-name", agent_name
                ]
                # On Linux, use headless mode to prevent browser opening issues
                ...
                url = "http://localhost:8501"  # Default fallback
                url_found = False
                for line in process.stdout:
                    if "Network URL" in line or "Local URL" in line:
                        match = re.search(r"http://[a-zA-Z0-9.:]+(:\d+)?", line)
                        if match:
                            url = match.group(0)
                            url_found = True
                    if url_found or "You can now view your Streamlit app" in line:
                        break
                ...
                try:
                    ...
                    pass
                if stderr_output and ("Error" in stderr_output or "Exception" in stderr_output):
                    logger.error(f"Streamlit failed to start: {stderr_output}")
                    Console.write_stderr(f"Failed to launch Streamlit UI: {stderr_output}")
                    if "address already in use" in stderr_output.lower():
                        Console.write_stderr(
                            "A port is in use. Check with 'lsof -i :8501' and 'kill <pid>', or wait for Streamlit to select a new port.")
                    elif platform.system() == "Linux" and "xdg-open" in stderr_output.lower():
                        Console.write_stderr(f"Ensure 'xdg-open' is installed or open {url} manually.")
                    else:
                        Console.write_stderr(
                            f"Check if the port is open (e.g., 'sudo ufw allow 8501') or try accessing {url} manually.")
                    process.terminate()
                    sys.exit(1)
                # Print URL for manual access
                if platform.system() == "Linux":
                    Console.write_stdout(f"Streamlit is running in headless mode. Open the app manually at {url}")
                    logger.info(f"Streamlit running in headless mode on Linux. URL: {url}")
                else:
                    logger.info(f"Streamlit running. URL: {url}")
                # Keep process running (Streamlit server runs until interrupted)
                try:
                    process.wait()
                except KeyboardInterrupt:
                    process.terminate()
                    Console.write_stdout("Streamlit server stopped.")
                    sys.exit(0)
            except FileNotFoundError:
                logger.error("Could not locate pygeai/chat/ui.py in the installed package")
                Console.write_stderr("Streamlit UI file (pygeai/chat/ui.py) not found in the installed pygeai package.")
                sys.exit(1)
            except Exception as e:
                logger.error(f"Error launching Streamlit UI: {e}")
                Console.write_stderr(
                    "An unexpected error occurred while launching the GUI. Please contact the developers.")
                sys.exit(1)
    else:
...
chat_with_agent_options = [
    ...
    Option(
        "gui",
        ["--gui", "-g"],
        "Launch a Streamlit GUI chat interface. Possible values: 0: OFF; 1: ON",
        True
    ),
]
