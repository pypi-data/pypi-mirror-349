    ...
    if use_gui:
        ...
        if use_gui:
            ...
            try:
                ...
                streamlit_cmd = ["streamlit", "run", ui_file_path, "--", "--agent-name", agent_name]
                ...
                if platform.system() == "Linux":
                    ...
                    Console.write_stdout(
                        "Streamlit is running in headless mode. Open the app manually at http://localhost:8501")
                    logger.info("Streamlit running in headless mode on Linux. URL: http://localhost:8501")
                # Run Streamlit app
                ...
                if result.returncode != 0:
                    logger.error(f"Streamlit failed to run: {result.stderr}")
                    Console.write_stderr(f"Failed to launch Streamlit UI: {result.stderr}")
                    if platform.system() == "Linux" and "xdg-open" in result.stderr.lower():
                        Console.write_stderr(
                            "On Linux, ensure 'xdg-open' is installed or open http://localhost:8501 manually.")
                    sys.exit(1)
