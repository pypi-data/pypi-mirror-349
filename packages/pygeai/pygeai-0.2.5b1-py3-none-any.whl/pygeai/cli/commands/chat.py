...
def chat_with_agent(option_list: list):
    ...
    if use_gui:
        ...
        try:
            ...
            # Run Streamlit app with agent_name as an argument
            result = subprocess.run(
                ["streamlit", "run", ui_file_path, "--", "--agent-name", agent_name],
                capture_output=True,
                text=True
            )
            ...
            Console.write_stderr("An unexpected error occurred while launching the GUI. Please contact the developers.")
            sys.exit(1)
    else:
