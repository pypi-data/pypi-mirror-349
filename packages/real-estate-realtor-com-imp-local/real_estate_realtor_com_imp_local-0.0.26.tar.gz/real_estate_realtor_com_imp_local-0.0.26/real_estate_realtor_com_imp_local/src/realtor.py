...
class Realtor(GenericCRUD, metaclass=MetaLogger, object=logger_code_init):
    ...
    def insert_to_table(
        ...
        try:
            ...
            self.insert_many_dicts(data_dicts=curr_location_listings_data_dict)
        except Exception as exception:
            self.logger.error(
    ...
    def extract_data(self, link: str) -> dict:
        ...
        try:
            ...
            if match:
                importer_id = int(match.group(0))
            else:
                importer_id = None
                self.logger.warning(
                    "No listing id found", object={"title": title, "url": url}
                )
            # importer_id = int(listing_id_element.text)
    ...
    def main_function(self, locations: list[str] = None) -> None:
        ...
        for location in locations:
            ...
            importer_id = self.insert_to_table(curr_location_listings, location)
