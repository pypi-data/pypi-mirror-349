...
def setup_logger(name: str):
    ...
    log_config_path = os.path.abspath(os.path.join(
        os.path.dirname(__file__),  # ตำแหน่ง log_util.py
        "..",                       # ขึ้น 1 ขั้นไปยัง `util`
        "..",                       # ขึ้นอีกขั้นไปยัง `app`
        "resource",                 # เข้าสู่ resource
        "log-config.json"
    ))
