...
class Normalizer:
    ...
    async def _get_text_representation(
        ...
        if agg_meta.get("connector_type"):
            connector_source = agg_meta["connector_type"]
        elif first_entry_metadata.get("connector_type"):
            connector_source = first_entry_metadata["connector_type"]
        elif first_entry_metadata.get("source"): # Fallback to 'source' if 'connector_type' is missing
             connector_source = first_entry_metadata["source"]
        elif agg_meta.get("source"):
            connector_source = agg_meta["source"]
        ...
        for entry_index, entry in enumerate(entries):
            ...
            try:
                if entry_type == "text":
                    ...
                    try:
                        if self.config.caption.enabled:
                            # Получаем информацию о доступных провайдерах описаний изображений
                            from mindbank_poc.api.routers.processing_providers import MOCK_PROVIDERS
                            
                            # Фильтруем провайдеры по типу CAPTION
                            caption_providers = [p for p in MOCK_PROVIDERS.values() if p["provider_type"] == ProviderType.CAPTION.value]
                    ...
                    try:
                        if self.config.transcript.enabled:
                            # Получаем информацию о доступных провайдерах транскрипции
                            from mindbank_poc.api.routers.processing_providers import MOCK_PROVIDERS
                            
                            # Фильтруем провайдеры по типу TRANSCRIPTION
                            transcript_providers = [p for p in MOCK_PROVIDERS.values() if p["provider_type"] == ProviderType.TRANSCRIPTION.value]
    ...
    async def process(self, aggregate: Dict[str, Any]) -> NormalizedUnit:
        ...
        try:
            ...
            try:
                # Получаем информацию о доступных провайдерах классификации
                from mindbank_poc.api.routers.processing_providers import MOCK_PROVIDERS
                
                # Фильтруем провайдеры по типу CLASSIFICATION
                classification_providers = [p for p in MOCK_PROVIDERS.values() if p["provider_type"] == ProviderType.CLASSIFICATION.value]
            ...
            logger.info(f"Normalized aggregate {group_id} into unit with text length {len(normalized_unit.text_repr)}")
