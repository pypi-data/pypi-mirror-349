...
def timeIt(num: int = 1, show: bool = True) -> Callable:
    ...
    def decorator(func: Callable) -> Callable:
        @functools.wraps(func)
        def wrapper(*args, **kwargs) -> Tuple[Optional[Callable], Dict]:
            result = None  # 存储结果
            tempElapsed = []  # 存储耗时
...
# ================================================== 异常处理 ==================================================
def catch(exc: Union[Type[Exception], Tuple[Type[Exception]]] = Exception, value: Any = None, reRaise: bool = False, show: bool = True, eValue: bool = False) -> Callable:
    """
    异常捕获装饰器工厂
    :param exc: 要捕获的异常类型
    ...
    :param show: 是否显示错误信息
    :return: 返回 (默认值, 异常对象)元组 或 目标函数返回值
    """

    def decorator(func: Callable) -> Callable:
        @functools.wraps(func)
        def wrapper(*args, **kwargs) -> Tuple[Any, Exception] | Any:
            # 捕获异常并返回默认值
            try: return func(*args, **kwargs)  # 正常执行
            except exc as e:
                if show: print(f"[异常捕获 {func.__name__}], 错误信息: [{str(e)}]")
                if reRaise: raise e  # 重新抛出异常开关
                if eValue: return value, e  # 返回元组
                return value  # 返回元组
        return wrapper
    return decorator
...
# ================================================== 全局异常捕获 ==================================================
def _catchAllErrors(func: Callable, show: bool = True, value: Any = None, eValue: bool = False) -> Callable:
    """
    内部实现的全局异常捕获装饰器
    :param func: 需要装饰的目标函数
    :param show: 是否显示错误信息
    :param value: 异常发生时返回的默认值
    :param eValue: 是否返回异常对象
...
def catchAll(name: str, show: bool = True, value: Any = None, eValue: bool = False) -> None:
    """
    模块级全局异常捕获
    ...
    for attrName in dir(targetModule):
        ...
        if callable(attr) and not isinstance(attr, type):
            try:
                # 添加异常捕获装饰器
                wrappedFunc = _catchAllErrors(attr, show, value, eValue)
...
def analyse(sampling: float = 0.1, cpuSampling: float = 0.1, show: bool = True) -> Callable:
    """
    性能监控装饰器。
