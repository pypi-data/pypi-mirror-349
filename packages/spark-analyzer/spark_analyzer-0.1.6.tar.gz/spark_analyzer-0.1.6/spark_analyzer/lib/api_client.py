...
class APIClient:
    ...
    def __init__(self, base_url: str = "http://server-url:18080/api/v1", config_file: str = "config.ini", args: Optional[argparse.Namespace] = None):
        ...
        try:
            # Try to load workflow type keywords
            keyword_paths = [
                os.path.join("resources", "workflow_type_keywords.json"),
                pkg_resources.resource_filename("spark_analyzer", os.path.join("resources", "workflow_type_keywords.json"))
            ]
    ...
    def is_indexing_candidate(self, stage):
        ...
        # Use indexing keywords from workflow type keywords
        indexing_keywords = self.workflow_type_keywords.get("indexing", [])
