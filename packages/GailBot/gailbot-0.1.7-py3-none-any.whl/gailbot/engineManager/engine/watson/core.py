...
# @Last Modified time: 2024-04-11 00:21:21
...
class WatsonCore:
    ...
    def __init__(self, apikey: str, region: str, watson_setting: WatsonSetting):
        self.apikey = apikey
        self.region = region
        self.setting = watson_setting
    ...
    def transcribe(
        ...
        try:
            if get_size(audio_path) >= WATSON_CONFIG.max_file_size_bytes:
                audio_path = MediaHandler.compress_to_opus(
                    audio_path, payload_workspace
                )
                logger.info(
                    f"File size exceeds size limit: compressed audio_path is {audio_path}"
                )
        except Exception as e:
        ...
        try:
            ...
            utterances = self._prepare_utterance(
                audio_name, recognize_callbacks.get_results()
            )
            return utterances
        except Exception as e:
            logger.error(e, exc_info=e)
    ...
    def _websockets_recognize(
        ...
        with open(audio_path, "rb") as f:
            ...
            kwargs.update(
                ...
                "profanity_filter": setting.profanity_filter,
                "word_confidence":  setting.word_confidence,
                "ssl_verification": setting.ssl_verification,
                "interim_results": setting.interim_results,
                "timestamps": setting.timestamps,
                "smart_formatting": setting.smart_formatting,
                "redaction": setting.redaction,
                "processing_metrics": setting.processing_metrics,
                "audio_metrics": setting.audio_metrics,
                "split_transcript_at_phrase_end": setting.split_transcript_at_phrase_end,
                "inactivity_timeout": setting.inactivity_timeout,
                "keyword_threshold": setting.keyword_threshold,
                "max_alternatives": setting.max_alternatives,
                "processing_metrics_interval": setting.processing_metrics_interval,
                "end_of_phrase_silence_time": setting.end_of_phrase_silence_time,
                "speech_detector_sensitivity": setting.speech_detector_sensitivity,
                "background_audio_supression": setting.background_audio_supression
                }
            )
