# predictram-parameter-data/query_parser.py
import re

# Mapping of display names to JSON keys
METRIC_MAPPING = {
...
def parse_query(query_string):
    ...
    Returns:
        list: List of conditions (dicts with metric, operator, value)
    """
    # Split into individual conditions
    conditions = re.split(r'\s+(?:and|or)\s+', query_string, flags=re.IGNORECASE)
    
    parsed_conditions = []
    
    for condition in conditions:
        # Match metric operator value pattern
        match = re.match(
            r'^(.*?)\s*(>=|<=|>|<|=|!=|contains|like)\s*(.*?)$', 
            condition.strip(), 
            flags=re.IGNORECASE
        ...
        # Get the JSON key for this metric
        json_key = METRIC_MAPPING.get(metric, metric)
        
        # Clean and convert value
        try:
            # Remove % signs and convert to float if numeric
            if '%' in value:
