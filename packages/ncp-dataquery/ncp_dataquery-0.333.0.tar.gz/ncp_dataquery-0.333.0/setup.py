...
with open(os.path.join(here, "ncp_dataquery", "_version.py"), "r", "utf-8") as f:
    exec(f.read(), about)
...
setup(
    ...
    url=about["__url__"],
    packages=find_packages(include=["ncp_dataquery", "ncp_dataquery.*"]),
    ...
    entry_points={
        "sqlalchemy.dialects": [
            "trino = ncp_dataquery.sqlalchemy.dialect:TrinoDialect",
        ]
    },
)
