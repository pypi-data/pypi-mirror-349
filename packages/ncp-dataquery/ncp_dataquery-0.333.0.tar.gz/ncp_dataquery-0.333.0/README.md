# NCP DataQuery Python client

## ** Notice **
- This Project is forked from [trinodb/trino-python-client](https://github.com/trinodb/trino-python-client) and customized for [**NCP Data Query Service**](https://www.ncloud.com/product/analytics/dataQuery).
- Following codes are added/modified to support NCP IAM Authentication and Data Query connection parameters.
  - `trino/**` -> `ncp_dataquery/**` (Moved)
  - `ncp_dataquery/auth.py` (Modified)
  - `ncp_dataquery/client.py` (Modified)
  - `ncp_dataquery/dbapi.py` (Modified)
...
### Usage
The `NcpIamAuthentication` class can be used to connect to NCP Data Query Service (ref: [NCP IAM authentication](https://api.ncloud-docs.com/docs/common-ncpapi)).
Following additional parameters can be used with the connection :
- `reuse_query_result` : Enable reuse query result if exist.
  - boolean type, default: false
- `reuse_query_max_age` : When reuse-query-result enabled, maxAge(unit: minutes) will be used to find same query request within the period.
  - integer type, default: 60, range: 1~10080
<br>

Support DBAPI interface and SQLAlchemy to connect to DataQuery service. 
...
- DBAPI
    ...
    conn = connect(
        host="kr.dataquery.naverncp.com",
        port=443,
...
- SQLAlchemy
    ...
    engine = create_engine(
        "trino://kr.dataquery.naverncp.com:443/<catalog>",
        connect_args={
            "auth": NcpIamAuthentication("<access_key>", "<secret_key>"),
            "http_scheme": "https",
            "reuse_query_result": True, # optional, default=False
...
### NCP IAM authentication

The `NcpIamAuthentication` class can be used to connect to a Trino cluster configured with
the [NCP IAM authentication type](https://api.ncloud-docs.com/docs/common-ncpapi)
