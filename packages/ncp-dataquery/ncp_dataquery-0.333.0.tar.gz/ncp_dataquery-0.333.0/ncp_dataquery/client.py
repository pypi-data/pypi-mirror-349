...
from ncp_dataquery._version import __version__
from ncp_dataquery.auth import Authentication
from ncp_dataquery.exceptions import TrinoExternalError
from ncp_dataquery.exceptions import TrinoQueryError
from ncp_dataquery.exceptions import TrinoUserError
from ncp_dataquery.mapper import RowMapper
from ncp_dataquery.mapper import RowMapperFactory
...
class TrinoRequest:
    ...
    def _process_error(error, query_id: Optional[str]) -> Union[TrinoExternalError, TrinoQueryError, TrinoUserError]:
        error_type = error.get("errorType", None)
...
class TrinoQuery:
    ...
    def execute(self, additional_http_headers=None) -> TrinoResult:
        ...
        try:
            ...
            raise ncp_dataquery.exceptions.TrinoConnectionError("failed to execute: {}".format(e))
    ...
    def fetch(self) -> List[Union[List[Any]], Any]:
        ...
        try:
            ...
            raise ncp_dataquery.exceptions.TrinoConnectionError("failed to fetch: {}".format(e))
        status = self._request.process(response)
        self._update_state(status)
        if status.next_uri is None:
            self._finished = True
    ...
    def cancel(self) -> None:
        ...
        try:
            ...
            raise ncp_dataquery.exceptions.TrinoConnectionError("failed to cancel query: {}".format(e))
