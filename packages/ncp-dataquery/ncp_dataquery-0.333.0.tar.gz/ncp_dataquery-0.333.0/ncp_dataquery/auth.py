...
from ncp_dataquery import exceptions
from ncp_dataquery.constants import HEADER_USER
from ncp_dataquery.constants import MAX_NT_PASSWORD_SIZE
...
class _NcpSignatureAuth(AuthBase):
    """
    Custom implementation for NCP IAM Authentication
    """
    ...
    def __call__(self, request: PreparedRequest):
        ...
        string_to_sign = f"{request.method} {request.path_url}\n{timestamp}\n{access_key}"

        signature = base64.b64encode(
            hmac.new(secret_key, bytes(string_to_sign, 'UTF-8'), digestmod=hashlib.sha256).digest()
        )

        request.headers["x-ncp-iam-access-key"] = access_key
        request.headers["x-ncp-apigw-signature-v2"] = signature
        request.headers["x-ncp-apigw-timestamp"] = timestamp
        return request


class NcpIamAuthentication(Authentication):
    def __init__(self, access_key: str, secret_key: str):
        self._access_key = access_key
        self._secret_key = secret_key

    def set_http_session(self, http_session: Session):
        http_session.auth = _NcpSignatureAuth(self._access_key, self._secret_key)

    def get_exceptions(self) -> Tuple[Any, ...]:
        return ()

    def __eq__(self, other: object) -> bool:
        if not isinstance(other, NcpIamAuthentication):
            return False
        return self._access_key == other._access_key and self._secret_key == other._secret_key
...
class _OAuth2KeyRingTokenCache(_OAuth2TokenCache):
    ...
    def get_token_from_cache(self, key: Optional[str]) -> Optional[str]:
        ...
        try:
            ...
            raise ncp_dataquery.exceptions.NotSupportedError("Although keyring module is installed no backend has been "
