...
class Transaction:
    def __init__(self, request: ncp_dataquery.client.TrinoRequest) -> None:
    ...
    @property
    def request(self) -> ncp_dataquery.client.TrinoRequest:
        return self._request

    def begin(self) -> None:
        response = self._request.post(START_TRANSACTION)
        if not response.ok:
            raise ncp_dataquery.exceptions.DatabaseError(
    ...
    def commit(self) -> None:
        query = ncp_dataquery.client.TrinoQuery(self._request, COMMIT)
        try:
            list(query.execute())
    ...
    def rollback(self) -> None:
        query = ncp_dataquery.client.TrinoQuery(self._request, ROLLBACK)
        try:
            list(query.execute())
        except Exception as err:
            raise ncp_dataquery.exceptions.DatabaseError(
