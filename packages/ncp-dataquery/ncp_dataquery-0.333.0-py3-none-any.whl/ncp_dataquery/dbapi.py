...
import ncp_dataquery.client
import ncp_dataquery.exceptions
import ncp_dataquery.logging
from ncp_dataquery import constants
from ncp_dataquery.constants import LENGTH_TYPES
from ncp_dataquery.constants import PRECISION_TYPES
from ncp_dataquery.constants import SCALE_TYPES
from ncp_dataquery.exceptions import DatabaseError
from ncp_dataquery.exceptions import DataError
from ncp_dataquery.exceptions import Error
from ncp_dataquery.exceptions import IntegrityError
from ncp_dataquery.exceptions import InterfaceError
from ncp_dataquery.exceptions import InternalError
from ncp_dataquery.exceptions import NotSupportedError
from ncp_dataquery.exceptions import OperationalError
from ncp_dataquery.exceptions import ProgrammingError
from ncp_dataquery.exceptions import Warning
from ncp_dataquery.transaction import IsolationLevel
from ncp_dataquery.transaction import NO_TRANSACTION
from ncp_dataquery.transaction import Transaction
...
logger = ncp_dataquery.logging.get_logger(__name__)
...
class Connection:
    ...
    def __init__(
        ...
        # begin : ncp additional connection properties
        project_id=None,
        reuse_query_result=False,
        reuse_query_max_age=60,
        # end : ncp additional connection properties
    ):
        ...
        self._client_session = ncp_dataquery.client.ClientSession(
            user=user,
            catalog=catalog,
            schema=schema,
            source=source,
        ...
        if http_session is None:
            self._http_session = ncp_dataquery.client.TrinoRequest.http.Session()
        ...
        if project_id is not None:
            if not isinstance(project_id, int):
                raise ValueError(f"project_id must be IntegerType")
            self._http_session.headers["x-ncp-data-query-project-id"] = str(project_id)
        if reuse_query_result:
            if not isinstance(reuse_query_result, bool):
                raise ValueError(f"reuse_query_result must be BooleanType")
            if not isinstance(reuse_query_max_age, int):
                raise ValueError(f"reuse_query_max_age must be IntegerType")
            if reuse_query_max_age < 1 or reuse_query_max_age > 10080:
                raise ValueError(f"Invalid reuse_query_max_age (out of range): {reuse_query_max_age}")
            else:
                self._http_session.headers["x-ncp-data-query-is-reuse"] = str(reuse_query_result)
                self._http_session.headers["x-ncp-data-query-reuse-max-age"] = str(reuse_query_max_age)
        # end : ncp additional connection properties
    ...
    def _create_request(self):
        return ncp_dataquery.client.TrinoRequest(
            self.host,
            self.port,
    ...
    def _use_legacy_prepared_statements(self):
        ...
        if value is None:
            try:
                query = ncp_dataquery.client.TrinoQuery(
...
class Cursor:
    ...
    def setinputsizes(self, sizes):
        raise ncp_dataquery.exceptions.NotSupportedError
    ...
    def _prepare_statement(self, statement: str, name: str) -> None:
        ...
        query = ncp_dataquery.client.TrinoQuery(self.connection._create_request(), query=sql,
    ...
    def _execute_prepared_statement(
        ...
        return ncp_dataquery.client.TrinoQuery(self._request, query=sql, legacy_primitive_types=self._legacy_primitive_types)
    ...
    def _execute_immediate_statement(self, statement: str, params):
        ...
        return ncp_dataquery.client.TrinoQuery(
    ...
    def _format_prepared_param(self, param):
        ...
        raise ncp_dataquery.exceptions.NotSupportedError("Query parameter of type '%s' is not supported." % type(param))
    ...
    def execute(self, operation, params=None):
        if params:
            ...
            self._query = ncp_dataquery.client.TrinoQuery(self._request, query=operation,
                                                  legacy_primitive_types=self._legacy_primitive_types)
            self._iterator = iter(self._query.execute())
        return self
    ...
    def fetchone(self) -> Optional[List[Any]]:
        ...
        try:
            ...
            return None
        except ncp_dataquery.exceptions.HttpError as err:
            raise ncp_dataquery.exceptions.OperationalError(str(err))
    ...
    def describe(self, sql: str) -> List[DescribeOutput]:
        ...
        try:
            ...
            self._query = ncp_dataquery.client.TrinoQuery(
                self._request,
                query=sql,
                legacy_primitive_types=self._legacy_primitive_types,
            )
