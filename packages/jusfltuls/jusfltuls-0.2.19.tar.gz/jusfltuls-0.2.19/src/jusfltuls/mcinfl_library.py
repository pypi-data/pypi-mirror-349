...
#          CALL  COMMAND
# --------------------------------------------------------------------------------
def call_cmd(CMD2, database=None, fromdb=None, todb=None, silent=False, format=""):
    u, p, _ = get_user_pass()
    m = get_local_machine()
    CMD = ""
    ssl = ""
    if not is_int(m.split(".")[-1]):
        ssl = "--ssl"
    if database is None:
        CMD = f"influx -username '{u}' -password '{p}' {ssl} -host {m} -execute '{CMD2}' {format}"
    else:
        CMD = f"influx -username '{u}' -password '{p}'  {ssl} -host {m}  -database {database} -execute '{CMD2}' {format}"
        #print(CMD)
...
def backup_portable(PATH):
    ...
    with open("/tmp/mcdebu.log", "a") as f:
        ...
        f.write("CMDx\n")
        f.write(f"{CMDx}\n")
        #result = sp.Popen(CMDx , stdin=sp.DEVNULL, stdout=sp.PIPE, stderr=sp.DEVNULL)
    ...
    #    ok = False
    #else:
    #    print("i...  running backup... can take long ....")
    #    pass
    #---------------------------------------------------
...
def export_measurement(m1, db1, silent=False):
    ...
    CMD = f"SELECT * FROM {m1}"
    FORMAT = "-format csv"
    #res = CMD
    res = call_cmd( CMD, database=db1, silent=silent, format=FORMAT)
    now = dt.datetime.now().strftime( "%Y%m%d_%H%M%S")
    with open(f"export_{db1}_{m1}_{now}.csv", 'w') as f:
        f.write(res)
    return "ok"
