...
def build_repository_contextree(repository_path, output_path):
    """
    构建项目级上下文树
    """
    # 创建根节点(项目文件夹)
    ...
    folder_nodes = build_folder(repository_path, root_node)
    ...
    save_path = os.path.join(output_path, repository_name)
    if not os.path.exists(save_path):
        os.makedirs(save_path)

    torch.save(context_trees_dict, os.path.join(save_path, "ContexTree.pth"))
    return context_trees_dict
...
def get_tooleval(data_path, source_code_path, result_path):
    ...
    for data in data_list:
        ...
        # 查看项目是否已被上下文化
        repository_name = data["project_path"].split("/")[-1]
        if os.path.exists(os.path.join(result_path, repository_name)):
            continue
        
        context_trees_dict  = build_repository_contextree(project_path, result_path)
...
def initial_bm25(context_dict,
    ...
    """
    all_candidate = [node for node in context_dict.values() if isinstance(node, ContextNode) and node.type in ["function", "class"] and "test" not in node.file_ns.split(".")[-1] and node.namespace not in target_namespace]
    all_candidate_content = []
    all_candidate_nodes = []
    max_node_num = max_node_num + len(init_context)
    for node in all_candidate:
        code_list = DFS2leaf(context_dict, node)
        candidate_length = 0
        candidate_tokens = []
        candidate_nodes = []
        idx=0
        while candidate_length < max_context_length and idx < len(code_list):
            code_node = context_dict[code_list[idx]]
            code_tokens = code_node.content.split() if tokenizer == "cut" else tokenizer.tokenize(code_node.content)
            code_tokens = code_tokens[:max_node_length]
            if candidate_length + len(code_tokens) > max_context_length:
                candidate_tokens += code_tokens[:max_context_length - candidate_length]
            else:
                candidate_tokens += code_tokens
            candidate_length = len(candidate_tokens)
            candidate_nodes.append(code_node)
            idx += 1
        all_candidate_content.append(candidate_tokens)
        all_candidate_nodes.append(candidate_nodes)
    
    
    query_tokens = query.split() if tokenizer == "cut" else tokenizer.tokenize(query)
    BM25_model = BM25Okapi(all_candidate_content)
    ...
    for idx in top_index:
        if cross_num+infile_num >= max_node_num:
            break
        candidate = context_dict[all_candidate[idx].namespace]
        node_list = all_candidate_nodes[idx]
        if candidate.file_path != target_file:
            ...
            flag = True
            for node in node_list:
                if node not in infile_list and node.end_line <= target_node.begin_line:
                    infile_num += 1
                    if cross_num+infile_num > max_node_num:
                        break
...
def initial_random(context_dict, target_namespace, num=10):
    """
    随机从context_dict中选择num个code节点, 不能选择目标节点对应子树内容
    """
    all_code_nodes = [node for node in context_dict.values() if isinstance(node, ContextNode) and node.type in ["class", "function"] and "test" not in node.file_ns.split(".")[-1] and node.namespace not in target_namespace]
    random.shuffle(all_code_nodes)
    init_context = []
    for node in all_code_nodes:
        leaf_nodes = DFS2leaf(context_dict, node)
        if len(leaf_nodes) > 1:
            qika = 0
        for leaf_node in leaf_nodes:
            if target_namespace in leaf_node:
                continue
            if leaf_node not in init_context:
                init_context.append(leaf_node)
        if len(init_context) >= num:
            break
    return init_context
