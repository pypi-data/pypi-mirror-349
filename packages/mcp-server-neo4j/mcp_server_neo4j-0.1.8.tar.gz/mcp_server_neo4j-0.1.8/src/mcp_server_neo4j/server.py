        ...
        try:
            ...
            with self.driver.session(database=self.database) as session:
                ...
                count_result = session.run("""
                    ...
                    RETURN coalesce(node_count, 0) as node_count, 
                           coalesce(relationship_count, 0) as relationship_count
                """)
                counts = count_result.single()
                if counts is None:  # 这种情况理论上不应该发生，但为了安全起见保留检查
                    raise McpError("无法获取数据库统计信息")
            ...
            raise McpError(f"获取数据库信息失败: {error_msg}")
