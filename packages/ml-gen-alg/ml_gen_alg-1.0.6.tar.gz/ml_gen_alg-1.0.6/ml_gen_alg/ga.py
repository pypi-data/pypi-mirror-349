...
class GA:
    ...
    def genetic_algorithm(self):
        ...
        for generation in range(self.num_generations):
            # Corrección: Comprobar primero la firma de la función fitness
            if hasattr(self.fitness_func, "__code__") and self.fitness_func.__code__.co_argcount > 1:
                # Si la función fitness toma más de un argumento (incluye índice)
                fitnesses = [self.fitness_func(ind, idx) for idx, ind in enumerate(self.population)]
            else:
                # Si la función fitness toma solo un argumento
                try:
                    # Asegurarse de que los individuos sean del tipo correcto para la función fitness
                    fitnesses = [self.fitness_func(ind) for ind in self.population]
                except TypeError as e:
                    # Mostrar un error más descriptivo
                    raise TypeError(f"Error en fitness_func: {e}. Asegúrate de que la función fitness sea compatible con el tipo de individuos en la población.")
