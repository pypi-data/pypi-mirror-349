...
from mantarix.core.types import AppView, WebRenderer
...
def app(
    ...
    host=None,
    port=0,
    view: Optional[AppView] = AppView.MANTARIX_APP,
    assets_dir="assets",
    upload_dir=None,
    ...
    if export_asgi_app:
        ensure_mantarix_web_package_installed()
        from mantarix_web.fastapi.serve_fastapi_web_app import get_fastapi_web_app
...
async def app_async(
    ...
    force_web_server = get_bool_env_var("MANTARIX_FORCE_WEB_SERVER") or is_linux_server()
    if force_web_server:
        view = AppView.WEB_BROWSER

    env_port = os.getenv("MANTARIX_SERVER_PORT")
    ...
    env_host = os.getenv("MANTARIX_SERVER_IP")
    ...
    is_socket_server = (
        is_embedded() or view == AppView.MANTARIX_APP or view == AppView.MANTARIX_APP_HIDDEN
    ) and not force_web_server

    url_prefix = os.getenv("MANTARIX_DISPLAY_URL_PREFIX")
    ...
    def exit_gracefully(signum, frame):
        logger.debug("Gracefully terminating Mantarix app...")
    ...
    logger.info("Mantarix app has started...")

    try:
        if (
            (
                view == AppView.MANTARIX_APP
                or view == AppView.MANTARIX_APP_HIDDEN
                or view == AppView.MANTARIX_APP_WEB
            )
            and not force_web_server
            and not is_embedded()
            ...
            ensure_mantarix_desktop_package_installed()
            from mantarix_desktop import close_mantarix_view, open_mantarix_view_async

            on_app_startup(conn.page_url)

            fvp, pid_file = await open_mantarix_view_async(
                conn.page_url,
                assets_dir if view != AppView.MANTARIX_APP_WEB else None,
                view == AppView.MANTARIX_APP_HIDDEN,
            )
            try:
                await fvp.wait()
            except:
...
async def __run_socket_server(port=0, session_handler=None, blocking=False):
    from mantarix.mantarix_socket_server import MantarixSocketServer

    uds_path = os.getenv("MANTARIX_SERVER_UDS_PATH")
...
async def __run_web_server(
    ...
    ensure_mantarix_web_package_installed()
    from mantarix_web.fastapi.serve_fastapi_web_app import serve_fastapi_web_app
    ...
    logger.info(f"Starting Mantarix web server on port {port}...")

    log_level = logging.getLogger(mantarix.__name__).getEffectiveLevel()
...
def __run_pyodide(target):
    import mantarix_js
    from mantarix.pyodide_connection import PyodideConnection
...
def __get_page_name(name: str):
    env_page_name = os.getenv("MANTARIX_WEB_APP_PATH")
...
def __get_assets_dir_path(assets_dir: Optional[str], relative_to_cwd=False):
    ...
    env_assets_dir = os.getenv("MANTARIX_ASSETS_DIR")
    if env_assets_dir:
