...
class GestureDetector(ConstrainedControl, AdaptiveControl):
    ...
    def main(page: mx.Page):
        def on_pan_update1(e: mx.DragUpdateEvent):
            c.top = max(0, c.top + e.delta_y)
            c.left = max(0, c.left + e.delta_x)
            c.update()

        def on_pan_update2(e: mx.DragUpdateEvent):
            e.control.top = max(0, e.control.top + e.delta_y)
            e.control.left = max(0, e.control.left + e.delta_x)
            e.control.update()

        gd = mx.GestureDetector(
            mouse_cursor=mx.MouseCursor.MOVE,
            drag_interval=50,
            on_pan_update=on_pan_update1,
        )

        c = mx.Container(gd, bgcolor=mx.colors.AMBER, width=50, height=50, left=0, top=0)

        gd1 = mx.GestureDetector(
            mouse_cursor=mx.MouseCursor.MOVE,
            drag_interval=10,
            on_vertical_drag_update=on_pan_update2,
            left=100,
            top=100,
            content=mx.Container(bgcolor=mx.colors.BLUE, width=50, height=50),
        )

        page.add( mx.Stack([c, gd1], width=1000, height=500))
    ...
    Online docs: https://mantarix.dev/docs/controls/gesturedetector
    """
