...
class FloatingActionButton(ConstrainedControl):
    ...
    def main(page: mx.Page):
        ...
        page.scroll = mx.ScrollMode.HIDDEN
        page.appbar = mx.AppBar(
            title=mx.Text(
                "Floating Action Button", weight=mx.FontWeight.BOLD, color=mx.colors.BLACK87
            ),
            bgcolor=mx.colors.BLUE,
            center_title=True,
            actions=[
                mx.IconButton(mx.icons.MENU, tooltip="Menu", icon_color=mx.colors.BLACK87)
            ],
            color=mx.colors.WHITE,
        )
        ...
        def fab_pressed(e):
            page.add(mx.ListTile(title=mx.Text(f"Tile {page.count}")))
            page.show_snack_bar(
                mx.SnackBar(mx.Text("Tile was added successfully!"), open=True)
            )
            page.count += 1

        page.floating_action_button = mx.FloatingActionButton(
            icon=mx.icons.ADD, on_click=fab_pressed, bgcolor=mx.colors.LIME_300
        )
        page.add(mx.Text("Press the FAB to add a tile!"))
    ...
    Online docs: https://mantarix.dev/docs/controls/floatingactionbutton
    """
