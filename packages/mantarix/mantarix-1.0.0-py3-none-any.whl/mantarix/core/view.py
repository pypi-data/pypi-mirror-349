...
class View(ScrollableControl, AdaptiveControl):
    ...
    A root view is automatically created when a new user session started. From layout perspective the View represents a `Column`(https://mantarix.dev/docs/controls/column/) control, so it has a similar behavior and shares same properties.

    -----

    Online docs: https://mantarix.dev/docs/controls/view
    """
