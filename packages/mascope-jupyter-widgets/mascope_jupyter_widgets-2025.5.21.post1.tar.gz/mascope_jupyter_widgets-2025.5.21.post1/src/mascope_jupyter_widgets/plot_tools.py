...
# Helper function
def auto_trace_name(trace_name: str, mz_val: float, original_trace_name: str) -> str:
    """
    If the new trace name contains the original trace name (or vice versa),
    append (mz=...) for clarity.
    Otherwise, use the new trace name as is.

    :param trace_name: The new trace name.
    :type trace_name: str
    :param mz_val: The mz value to append.
    :type mz_val: float
    :param original_trace_name: The original trace name.
    :type original_trace_name: str
    :return: The modified trace name.
    :type: str
    """
    if (
        trace_name == original_trace_name
        or original_trace_name in trace_name
        or trace_name in original_trace_name
    ):
        return f"{trace_name} (mz={mz_val:.4f})"
    return trace_name
