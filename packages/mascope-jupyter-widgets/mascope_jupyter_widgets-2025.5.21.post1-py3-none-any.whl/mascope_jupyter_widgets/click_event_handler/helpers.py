...
from ..sample_timeseries.plots import SampleTimeSeriesPlotter
...
def group_clicked_data_by_trace_and_mz(
    ...
    for selected_data in clicked_dots_dict[trace_name].values():
        ...
        if merged_df.empty or "mz" not in merged_df.columns:
            continue
        mz_val = float(merged_df["mz"].iloc[0])
        unique_key = (trace_name, mz_val)
        plot_data.setdefault(unique_key, [])
        if not any(df.equals(merged_df) for df in plot_data[unique_key]):
            plot_data[unique_key].append(merged_df)
    return plot_data


# Helper function
def build_combined_annotation_text(plot_data: dict) -> str:
    """
