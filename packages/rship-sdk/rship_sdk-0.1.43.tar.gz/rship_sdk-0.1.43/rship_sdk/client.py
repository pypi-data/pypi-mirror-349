...
class RshipExecClient:
    ...
    async def connect(self, close_timeout: int = 60, max_queue: int = 32):
        uri = f"ws://{self.rship_host}:{self.rship_port}/myko"
        print(f"Attempting to connect to {uri}")
        try:
            self.websocket = await websockets.connect(uri=uri, close_timeout=close_timeout, max_queue=max_queue)
