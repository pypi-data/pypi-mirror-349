    ...
    def create(
        ...
        **kwargs,
        ...
        if asyncio.get_event_loop().is_running():
            raise RuntimeError("This method can only be called in a sync context. Use the async version, acreate.")
...
class LLM:
    ...
    async def fine_tune(
        ...
        if lora_rank is not None:
            job.lora_rank = lora_rank
        if jinja_template is not None:
            job.jinja_template = jinja_template
        if early_stop is not None:
            job.early_stop = early_stop
        if max_context_length is not None:
            job.max_context_length = max_context_length
        if base_model_weight_precision is not None:
            job.base_model_weight_precision = base_model_weight_precision
        if wandb_config is not None:
            job.wandb_config = wandb_config
        if evaluation_dataset is not None:
            job.evaluation_dataset = evaluation_dataset
        if accelerator_type is not None:
            job.accelerator_type = accelerator_type
        if accelerator_count is not None:
            job.accelerator_count = accelerator_count
        if is_turbo is not None:
            job.is_turbo = is_turbo
        if eval_auto_carveout is not None:
            job.eval_auto_carveout = eval_auto_carveout
        ...
        job = await self._get_or_create_job(name, dataset, job)

        # poll until job is COMPLETED
        while job.state != JobState.COMPLETED:
            logger.info(f"Fine-tuning job {name} is in state {job.state}. Waiting for completion...")
            await asyncio.sleep(500)
            job = await self._gateway.get_supervised_fine_tuning_job(name)
            logger.info(f"Fine-tuning job {name} updated. Current state: {job.state}")

        return LLM(model=job.output_model)

    async def _get_or_create_job(
        self, name: str, dataset: Dataset, job: SupervisedFineTuningJob
    ) -> SupervisedFineTuningJob:
        existing_job = await self._gateway.get_supervised_fine_tuning_job(name)
        if existing_job is not None:
            return existing_job
        await dataset.sync()
        request = CreateSupervisedFineTuningJobRequest(
            supervised_fine_tuning_job=job,
            supervised_fine_tuning_job_id=name,
        )
        new_job = await self._gateway.create_supervised_fine_tuning_job(request)
        return new_job
