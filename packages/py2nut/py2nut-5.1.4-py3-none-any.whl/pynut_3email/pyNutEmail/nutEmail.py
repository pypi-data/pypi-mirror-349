        ...
        try:
            ...
            if not str_File_startW == '':
                ...
                if len(o_emails_sub) == 0:
                    ...
                    logger.warning('   --- list of emails we found in the box before to filter with str_File_startW')
                    for o_mail in o_emails:
                        logger.warning([str(o_attach) for o_attach in o_mail.Attachments])
                    logger.warning('___ END of email list _______________________________________________________')
                    raise
                else:   o_emails = o_emails_sub
            # PJ name end       
            ...
            if not str_File_endW == '':
                ...
                if len(o_emails_sub) == 0:
                    ...
                    logger.warning('   --- list of emails we found in the box before to filter with str_File_endW')
                    for o_mail in o_emails:
                        logger.warning([str(o_attach) for o_attach in o_mail.Attachments])
                    logger.warning('___ END of email list _______________________________________________________')
                    raise
            ...
            logger.error(' ERROR in filterEmails attachName 1: |{}|'.format(err))
            logger.error(self.o_emailParam)
            raise
        self.o_emails = o_emails
...
class c_Webmail_dwld(c_Webmail):
    ...
    def filterEmails_attachName(self):
        try:
            ...
            logger.error(' ERROR in filterEmails attachName 2: |{}|'.format(err))
            logger.error(self.o_emailParam)
            raise
        self.o_emails = o_emails
