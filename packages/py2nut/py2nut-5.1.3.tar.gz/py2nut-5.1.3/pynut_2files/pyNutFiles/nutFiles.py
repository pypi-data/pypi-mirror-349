...
def fStr_fillXls_celByCel(str_path, df_data, str_SheetName='', xl_sheet=0, int_nbRows=0, int_rowsWhere=1, xl_app=None):
    ...
    ### TODO: NEW VERSION is not working - Need to test out
    ...
    #         bl_CloseExcel = False
    # except Exception as err:
    #     logger.error('  ERROR in fl filXls_celByCel 1: Could not find Excel: |{}|'.format(err))
    #     logger.error('  - Path : |{}|'.format(str_path))
    #     logger.error('  - SheetName : |{}|'.format(str_SheetName))
    #     return False
    #
    ...
    # try:
    #     if int_nbRows > 0:
    #         for i in range(0, int_nbRows):      xl_sheet.Rows(int_rowsWhere).EntireRow.Insert()
    #     elif int_nbRows < 0:
    #         for i in range(0, -int_nbRows):     xl_sheet.Rows(int_rowsWhere).EntireRow.Delete()
    # except Exception as err:
    #     logger.error('  ERROR in fl filXls_celByCel 2, Insert or delete ROWS: |{}|'.format(err))
    ...
    #             xl_sheet.Cells(i + 1, j + 1).Value = str(df_data.iat[i, j])
    #             j_max = j + 1
    #         i_max = i + 1
    # except Exception as err:
    #     logger.error('  ERROR in fl filXls_celByCel 3, Fill Cell by Cell: |{}|'.format(err))
    #     try:
    ...
    # except Exception as err:
    #     logger.error('  ERROR in fl filXls_celByCel 4: Empty the rest of the sheet: |{}|'.format(err))
    #     try:
    #         logger.error(f'  - i_max= {i_max}, j_max= {j_max}')
    ...
    #         time.sleep(5)
    #     except Exception as err:    logger.error('  ERROR in fl filXls_celByCel : Excel workbook could not be closed | {}'.format(str(err)))
    #     try:
    #         xl_app.QuitXlApp()
    #         time.sleep(10)
