...
class c_db_sqlServer:
    ...
    def connect(self):  # db_sqlConnectCursor
        ...
        if t_keyConnect not in self.__dict_Connect:
            ...
            # NEW connexion
            try:
                print('Pynut update 2025: We changed the SQL Driver for AWS !!!')
                self.cnxn = pyodbc.connect('DRIVER={ODBC Driver 17 for SQL Server};SERVER=' + self.__server +
...
def db_SelectReq_single(str_req, server=None, database=None, uid=None, pwd = None):
    try:
        dbServer = c_db_sqlServ_single()
        d_param = dict(server=server, database=database, uid=uid, pwd=pwd)
        dbServer.defineCredentials(**d_param)
        dbServer.request = str_req
        dbServer.connect()
        dbServer.commit()
        df_result = dbServer.getDataframe()
        dbServer.closeConnection()
    except Exception as err:
        print('  ERROR in dbSelectReq: {}'.format(err))
        raise
    return df_result
