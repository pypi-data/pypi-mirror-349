...
def sketch_to_image(img, prompt, style='<anime>'):
    ...
    try:
        ...
        if AIWebApp.mode == 'openai':
            input_str = json.dumps({"prompt": current_prompt})
            parameters = {"size": "768*768",  # OpenAI path keeps its original params
                          "n": 1,
                          "style": style}
            parameters_str = json.dumps(parameters)

            img_path = resolve_resource_path(img)
            try:
                with open(img_path, "rb") as f_img:
                    response = requests.post(
                        ...
                        data={
                            ...
                            # Assuming this model name is for your OpenAI-compatible endpoint
                ...
                return f"错误: 涂鸦图像文件 '{img_path}' 未找到。请确认文件路径或URL是否正确。"
            except TypeError:
                return f"错误: 无法直接打开URL '{img_path}'"
            ...
            final_prompt_for_dashscope = f"{current_prompt}，{style_suffix_text}"
            if len(final_prompt_for_dashscope) > 800:  # Max prompt length from docs
                final_prompt_for_dashscope = final_prompt_for_dashscope[:800]
