...
def sketch_to_image(img, prompt, style='<anime>'):
    ...
    try:
        ...
        if not current_prompt:
            current_prompt = '根据输入图像的轮廓特征，少量扩充和涂色'
        ...
        if len(final_prompt_with_style) > 800: # Max prompt length from wanx2.1-imageedit docs
            final_prompt_with_style = final_prompt_with_style[:800]

        if AIWebApp.mode == 'openai':
            # OpenAI path now also uses wanx2.1-imageedit structure via a proxy
            input_data_dict = {
                "function": "doodle",
            ...
            parameters_dict = {
                "n": 1,
                "is_sketch": True  # Input is a sketch for doodle function
            }

            data_payload = {
                "model": "wanx2.1-imageedit",  # Specify the underlying model for the proxy
            ...
            img_local_path = None
            try:
                # Ensure img is a local file path for upload
                img_local_path = resolve_resource_path(img)
                with open(img_local_path, "rb") as f_img:
                    response = requests.post(
                        AIWebApp.server_url + "/images/generations/graffiti",  # Your proxy endpoint
                ...
                if str(img).startswith('http://') or str(img).startswith('https://'):
                    if img_local_path and os.path.exists(
                            img_local_path) and "/tmp" in img_local_path:  # Basic check for temp file
                        try:
                            os.remove(img_local_path)
                            print(f"Cleaned up temporary file: {img_local_path}")
                        except OSError as e:
                            print(f"Warning: Failed to clean up temporary file {img_local_path}: {e}")

                if response.status_code == HTTPStatus.OK:
                    response_json = response.json()
                    if 'data' in response_json and isinstance(response_json['data'], list) and \
                            len(response_json['data']) > 0 and 'url' in response_json['data'][0]:
                        return response_json['data'][0]['url']
                    else:
                        # If proxy returns Dashscope-like task_id, it needs polling here.
                        # For now, assume direct URL or error.
                        return f"错误: 响应格式不正确或未包含URL - {response.text}"
                else:
                    return f"错误: {response.status_code} - {response.text}"

            except FileNotFoundError:
                return f"错误: 涂鸦图像文件 '{img}' (resolved to '{img_local_path}') 未找到。"
            except ValueError as ve:  # From resolve_resource_path for download failures
                return f"错误: 处理输入图像失败 - {str(ve)}"
        else:
            # Dashscope path using wanx2.1-imageedit
            ...
            headers = {
                'Content-Type': 'application/json',
                'Authorization': f'Bearer {dashscope.api_key}',  # Ensure Bearer prefix
                'X-DashScope-Async': 'enable',
                # 'X-DashScope-OssResourceResolve': 'enable' # Removed as not in new API's doodle example
            }
            ...
            payload = {
                ...
                'input': {
                    ...
                    'prompt': final_prompt_for_dashscope
                },
                'parameters': {
                    'n': 1,
                    'is_sketch': True
                    # 'size' parameter is NOT supported by wanx2.1-imageedit for 'doodle'
                }
            }

            response = requests.post(url, headers=headers, json=payload, timeout=30)

            if response.status_code == HTTPStatus.OK:  # HTTP 200 for task creation
                response_data = response.json()
                if 'output' not in response_data or 'task_id' not in response_data['output']:
                    return f"错误: API响应格式不正确 - {response.text}"
                ...
                for _ in range(max_retries):
                    ...
                    task_status_response = requests.get(
                        ...
                        timeout=30  # Timeout for status check
                    )
                    ...
                    if task_status_response.status_code == HTTPStatus.OK:
                        ...
                        if task_status == 'SUCCEEDED':
                            ...
                            if results and isinstance(results, list) and len(results) > 0 and 'url' in results[0]:
                                return results[0]['url']
                            else:
                                return f"错误: 结果中未找到URL - {task_status_response.text}"
                        elif task_status == 'FAILED':
                            error_message = task_output.get('message', '未知错误')
                            code = task_data.get('code', '')  # Check for top-level error code
                            if not error_message and code:  # Sometimes error is at top level
                                error_message = task_data.get('message', error_message)
                            return f"错误: {error_message} (Code: {code}) - API Response: {task_status_response.text}"
                        elif task_status in ['PENDING', 'RUNNING']:
                            continue  # Continue polling
                        else:
                            return f"错误: 未知或意外任务状态 '{task_status}' - {task_status_response.text}"
                    else:
                        return f"错误: {task_status_response.status_code} - {task_status_response.text}"
                return "错误: 任务处理轮询超时。"
            else:  # Error on task creation
                error_detail = "未知错误"
                try:
                    error_json = response.json()
                    error_detail = error_json.get('message', error_detail)
                    if 'code' in error_json:
                        error_detail = f"Code: {error_json['code']}, Message: {error_detail}"
