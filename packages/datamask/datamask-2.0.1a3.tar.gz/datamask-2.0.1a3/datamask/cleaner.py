...
class RowMapper:
    def __init__(self, table, piis_for_table):
        ...
        self.mappers = []
        self.natives = []
        self.pii_columns = set()  # Track PII columns
        ...
        while remaining:
            for col, (spec, mapper) in remaining.items():
                if isinstance(mapper, NativeFaker):
                    ...
                    added.add(col)
                    self.pii_columns.add(col)  # Add to PII columns
                    continue
                if spec["depends"]:
    ...
    def mask(self, row):
        modified_columns = set()  # Track which columns were actually modified
        for mapspec in self.mappers:
            try:
                data = mapspec.mapper(row[mapspec.col], row)
                if row[mapspec.col] != data:  # Only consider it modified if value changed
                    row[mapspec.col] = data
                    modified_columns.add(mapspec.col)
...
def mask_pii(table: str, pii_spec, dsn, keepers, fixed):
    ...
    if not row_mapper.mappers and not row_mapper.natives:
        print(f"Skipping {table} - no PII columns to process")
        return
    print(f"Executing {table} - processing {len(row_mapper.pii_columns)} PII columns")
    ...
    # Process native fakers - these directly execute SQL
    if row_mapper.natives:
    ...
    for row in read_cursor:
        try:
            ...
            new_row, modified_columns = row_mapper.mask(row_dict)
        except Exception as exe:
        ...
        # Only include columns that were actually modified in the UPDATE statement
        replacements = ",".join((f'"{colname}"={p}') for colname in modified_columns)
        
        def mapdict(v):
            return json.dumps(v) if isinstance(v, (dict, list)) else v
            
        # Only include modified columns in the values
        new_values = [mapdict(new_row[k]) for k in modified_columns]
        old_values = [mapdict(row[k]) for k in pks]
