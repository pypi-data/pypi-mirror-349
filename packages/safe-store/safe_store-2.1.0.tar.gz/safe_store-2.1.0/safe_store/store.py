...
class SafeStore:
    ...
    def __init__(
        ...
        try:
            ...
            raise
    # A helper method to preload vectorizers
    def preload_vectorizer(self,
        ...
        which is a write operation and thus protected by a file lock.
        ...
        Raises:
            ConfigurationError: If there are issues with vectorizer configuration,
                                such as missing dependencies or unsupported types.
            VectorizationError: If the vectorizer instantiation fails (e.g., model
                                download issues, invalid parameters).
            DatabaseError: For errors during database interaction, especially if
                           a new method needs to be registered.
            ConcurrencyError: If acquiring the file lock times out.
        ...
        # This operation *might* write to the database if the vectorization method
        # is new and needs to be registered. Thus, it requires a file lock for process safety.
        with self._instance_lock:  # Ensures thread-safety for instance attributes like self.conn
            ASCIIColors.debug(f"Attempting to acquire file lock for preloading vectorizer: {_vectorizer_name_to_load}")
            try:
                # The VectorizationManager.get_vectorizer method can register a new
                # vectorization method in the database if it's encountered for the first time.
                # This database write operation necessitates the use of _file_lock.
                with self._file_lock:
                    ASCIIColors.debug(f"File lock acquired for preloading vectorizer: {_vectorizer_name_to_load}")
                    self._ensure_connection()  # Ensure the database connection is active
                    assert self.conn is not None  # For type checker, after _ensure_connection call

                    # The primary action: get (and thus cache/register) the vectorizer.
                    # The returned vectorizer and method_id are not explicitly used here,
                    # as the VectorizationManager handles its own internal caching.
                    # The call itself ensures the vectorizer is loaded and ready.
                ...
                ASCIIColors.debug(f"File lock released after preloading vectorizer: {_vectorizer_name_to_load}")

            except Timeout as e:
                msg = f"Timeout ({self.lock_timeout}s) acquiring file lock for preloading vectorizer '{_vectorizer_name_to_load}'"
                ASCIIColors.error(msg)
                raise ConcurrencyError(msg) from e
            except (ConfigurationError, VectorizationError, DatabaseError, ConnectionError) as e:
                # These are specific, expected errors from get_vectorizer or _ensure_connection
                ASCIIColors.error(f"Error preloading vectorizer '{_vectorizer_name_to_load}': {e.__class__.__name__}: {e}", exc_info=False)
                raise
            except Exception as e:
                # Catch-all for any other unexpected errors
                msg = f"Unexpected error while preloading vectorizer '{_vectorizer_name_to_load}': {e}"
