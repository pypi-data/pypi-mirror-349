...
class ConflictingVariableRoles(ValidationError):
    def __init__(
        self, which_var: str, roles: list[str], patch: Optional[JsonPatch] = None
    ):
        super(ConflictingVariableRoles, self).__init__(
            "Variable '%s' plays the following roles simultaneously: %s."
            % (which_var, ", ".join(roles)),
            {"variable": which_var, "roles": roles},
            patch,
        )
