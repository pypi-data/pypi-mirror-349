...
class VFG(BaseModel):
    ...
    def validate(
        ...
        for factor_idx, factor in enumerate(self.factors):
            ...
            # A factor.value with an empty shape tuple indicates a single value,
            # rather than an array.
            factor_value_is_array = factor.values.shape != ()
            if factor_value_is_array and len(factor.values) == 0:
                ...
                if all(k in self.variables for k in factor.variables):
                    ...
                    if factor_value_is_array:
                        actual_var_shape = list(factor.values.shape)
                    else:
                        # Assume a shape of 1 if the factor.value is not an array.
                        actual_var_shape = [1]
