...
from pyvfg.versions.vfg_0_5_0 import ModelType, Variable

from gpil.api.action_selection import process_action_selection_request
from gpil.api.types import ActionSelectionRequest
from gpil.lib.pymdp.vfg.pymdp_vfg_model import MissingControlState, PymdpModel
...
def test_validation_errors_messages():
    ...
    try:
        ...
        assert (
            ...
            == "Factors involving variables 'A, B' have role 'FactorRole.Preference' which is undefined for this model type."
        )
...
# GPIL-647: Handle a factor value that is a number, and not an array
def test_validation_factor_value_as_number():
    vfg = VFG.model_validate(
        {
            "variables": {
                "A": {"elements": ["a1"]},
    ...
    assert not vfg.validate_as(
        ModelType.BayesianNetwork
    ), "Model should pass validation as a Bayesian Network"
