...
from simplejrpc.daemon.exceptions import (
...
class DaemonContext:
    ...
    def terminate(self, signal_number, stack_frame):
        ...
        * Raise a ``SystemExit`` exception explaining the signal.
        """
        exception = SystemExit("Terminating on signal {signal_number!r}".format(signal_number=signal_number))
        raise exception
    ...
    def _get_exclude_file_descriptors(self):
        ...
        * Otherwise, include the item verbatim in the return set.
        ...
        if files_preserve is None:
            files_preserve = []
        files_preserve.extend(item for item in {self.stdin, self.stdout, self.stderr} if hasattr(item, "fileno"))
    ...
    def _make_signal_handler_map(self):
        ...
        """
        signal_handler_map = {
            signal_number: self._make_signal_handler(target) for (signal_number, target) in self.signal_map.items()
        }
...
def get_stream_file_descriptors(
    ...
    """
    file_descriptors = {
        fd for fd in {_get_file_descriptor(stream) for stream in {stdin, stdout, stderr}} if fd is not None
    }
...
def change_working_directory(directory):
    ...
    try:
        ...
        error = DaemonOSEnvironmentError("Unable to change working directory ({exc})".format(exc=exc))
...
def change_file_creation_mask(mask):
    ...
    try:
        ...
        error = DaemonOSEnvironmentError("Unable to change file creation mask ({exc})".format(exc=exc))
        raise error from exc
...
def change_process_owner(uid, gid, initgroups=False):
    ...
    try:
        ...
        error = DaemonOSEnvironmentError("Unable to change process owner ({exc})".format(exc=exc))
        raise error from exc
...
def prevent_core_dump():
    ...
    try:
        ...
        error = DaemonOSEnvironmentError(
            "System does not support RLIMIT_CORE resource limit" " ({exc})".format(exc=exc)
        )
        raise error from exc
    ...
    def fork_then_exit_parent(error_message):
        ...
        try:
            ...
            error = DaemonProcessDetachError(
                "{message}: [{exc.errno:d}] {exc.strerror}".format(message=error_message, exc=exc)
            )
            raise error from exc
...
def close_file_descriptor_if_open(fd):
    ...
    try:
        ...
        if exc.errno == errno.EBADF:
            ...
            error = DaemonOSEnvironmentError("Failed to close file descriptor {fd:d} ({exc})".format(fd=fd, exc=exc))
            raise error from exc
...
def make_default_signal_map():
    ...
    signal_map = {getattr(signal, name): target for (name, target) in name_map.items() if hasattr(signal, name)}
