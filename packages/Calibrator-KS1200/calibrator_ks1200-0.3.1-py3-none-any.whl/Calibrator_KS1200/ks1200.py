...
#   -----------------------------------------------------------------
        ...
        self.pushButton_archive_back.setGeometry(QtCore.QRect(830, 120, 141, 31))
        ...
        if count_of_measurements:
            for i in range(count_of_measurements):
                measurement = self.db_query("SELECT * FROM measurements WHERE id="+str(i+1))
            ...
            for chan in range (8):
                if len(self.measured_data_channel[chan][0]):
                    ...
                    for i in range(len(self.measured_data_channel[chan][0])):
                        ...
                        self.db_query("INSERT INTO measurements_data (id, measurement, sensor_id, datetime, t, r_or_e, ustavka_number) VALUES (?, ?, ?, ?, ?, ?, ?);", "write", data_tuple)
                    if self.measuring_mode == 'auto':
    ...
    def make_protocol(self):
        ...
        doc = Document(f'{os.path.dirname(__file__)}\\templates\\{template}')
