...
class ReactFile(BaseFile):
    ...
    mutation_method_input_types: list[ReactType]
...
class ReactRebootProtocPlugin(TypescriptRebootProtocPlugin):
    ...
    def _react_mutation_methods(
        ...
        return [
            ReactMethod(
                proto=mutation.proto, options=ReactMethodOptions(
    ...
    def _react_mutation_method_input_types(
        self, file: BaseFile, clients: Sequence[BaseClient]
    ) -> list[ReactType]:
        input_types = []
        for method in self._react_mutation_methods(file, clients):
    ...
    def add_language_dependent_data(self, file: BaseFile) -> BaseFile:
        react_file: BaseFile = ReactFile(
            ...
            mutation_methods=self._react_mutation_methods(file, file.clients),
            mutation_method_input_types=self.
            _react_mutation_method_input_types(file, file.clients),
