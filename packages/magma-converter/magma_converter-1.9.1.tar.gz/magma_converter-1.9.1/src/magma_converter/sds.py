...
class SDS(NewTrace):
    ...
    def stations(self) -> Dict[str, Any]:
        ...
        stations: Dict[str, Any] = {'nslc': nslc,
    ...
    @staticmethod
    def revalidate(sds_directory: str,
                   station: str,
                   channel: str,
        ...
        for date_obj in dates:
            ...
            mseed_file = os.path.join(sds_directory, year, network, station, f"{channel}.{station_type}", filename)
            ...
            if os.path.exists(mseed_file):
                try:
                    ...
                    sds = SDS(trace=stream[0], output_dir=output_directory, directory_structure='sds', force=True)
    ...
    def save(self, min_completeness: float = 70.0, encoding: str | int = 'STEIM2', **kwargs) -> bool:
        """Save into SDS
