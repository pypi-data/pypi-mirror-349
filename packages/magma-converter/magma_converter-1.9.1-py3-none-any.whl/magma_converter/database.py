        ...
        for date_obj in dates:
            ...
            if os.path.exists(file_location):
                try:
                    ...
                    if completeness > 100:
                        if verbose is True:
                            print(f'⚠️ {trace.id} :: Completeness is {completeness}%. Trying to fix it.')
                        stream = read(file_location)
                        stream = stream.merge(fill_value=0)
                        trace = stream[0]
                        completeness: float = (np.count_nonzero(trace.data) / total_sample_per_day) * 100
                        if verbose is True:
                            print(f'⚠️ {trace.id} :: New completeness: {completeness}%.')
