...
class FileInterface(Interface[FileSettings]):
    ...
    async def conditional_shutdown(self, remote: bool = False) -> None:
        ...
        if self._settings.get("auto_shutdown", True) or remote:
            ...
            await self.station.shutdown(
                stop_loop=self._settings.get("stop_loop", True),
            )
    ...
    def start(self) -> None:
        ...
        assert self._input
        assert self._output
        self._close_input = self._input.seekable()
        self._close_output = self._output.seekable()
    ...
    async def read_loop(self) -> None:
        ...
        read_amount = 256 if self._close_input else 1
        self.station.frame_router.process_frame(
            self,
            Frame(
                route=self.route,
        ...
        while True:
            ...
            # yet.
            message = await async_wrap(self._input.read)(read_amount)
    ...
    async def shutdown(self) -> None:
        ...
        if self._close_input and self._input:
            self._input.close()
