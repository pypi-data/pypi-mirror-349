...
def crack_akamai_v3(user_token, requests_args, proxy, other_requests=None, region="hk", developer_id=None, debug=False):
    try:
        language = {
            "jp": "ja-JP,ja;q=0.9",
            "us": "en-US,en;q=0.9",
        ...
        if "var chlgeId = ''" in response.text:
            ...
            resp = requests.post(
                'http://api.nocaptcha.cn/api/wanda/akamai/v2', headers={
        ...
        try:
            api = re.findall("src\=\"((?:\/[A-Za-z0-9\-\_\+]*?)+)\"\>\<\/script\>", response.text)[0]
        ...
        resp = requests.post(
            'http://api.nocaptcha.cn/api/wanda/akamai/v3', headers={
                'user-token': user_token,
                **({ "Developer-Id": developer_id } if developer_id else {})
            }, json={
                **requests_args,
