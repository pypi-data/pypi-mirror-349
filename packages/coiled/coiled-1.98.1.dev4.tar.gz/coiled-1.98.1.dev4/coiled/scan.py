...
async def handle_conda_package(pkg: CondaPackage) -> PackageInfo | None:
    ...
    if metadata_location:
        if not metadata_location.exists():
            ...
            has_get = hasattr(dist.metadata, "get")
            name = (has_get and dist.metadata.get("Name")) or (not has_get and dist.metadata["Name"]) or pkg.name  # type: ignore
...
async def handle_dist(dist: Distribution, locations: List[Path]) -> PackageInfo | CondaPlaceHolder | None:
    ...
    # of returning None. Some old versions will raise an AttributeError
