...
def EstimatedPayCalendar(min_hours=None,med_hours=None,max_hours=None,rate=None,taxRate=None,roundTo=None,quiet=False):
    display(f"Your Estimated PayDay Calendar Worksheet TDY(is={datetime.now().ctime()})\n")
    work_paydays=DayNamesPerMonthForYear(2025,"Thursday")
    if not min_hours:
        min_hours=24.0
    if not med_hours:
        med_hours=32.0
