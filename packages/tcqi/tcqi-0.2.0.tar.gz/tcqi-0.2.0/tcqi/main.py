...
class TCQiTools:
    ...
    def get_column_cell(column, row):
        # 1). Copy input info into local variable
        cell = column.copy()
    ...
    def merge_tables(table_1, columns_1, id_1, table_2, columns_2, id_2):
        return table_1.copy()[columns_1].join(table_2[columns_2].set_index(id_2), on=id_1, lsuffix='_caller',
                                              rsuffix='_other')

    def get_kpi_first(table, kpi_name, kpis=[], phases=[], element=[]):
    ...
    def get_kpi_second(table, kpi_name, kpis=[], phases=[], element=[]):
        # 1). Filter origin columns
        result = table.copy()[
    ...
    def get_kpi_third(table, kpi_name, kpis=[], phases=[], element=[]):
        # 1). Filter origin columns
        result = table.copy()[
    ...
    def get_sum_total(table):
        result = 0
        for i in table.values:
            result += float(i[1])
        return result

    def excel_export(table, filename):
