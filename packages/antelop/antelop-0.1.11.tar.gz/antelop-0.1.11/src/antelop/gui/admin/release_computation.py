...
def show(username, tables):
	...
	with col2:
		...
		sac.buttons(
			[
				sac.ButtonsItem(label="Spikesorting"),
				sac.ButtonsItem(label="DeepLabCut Training"),
			],
			key="releasecomputemode",
			gap="md",
			use_container_width=True,
		)

		if st.session_state.releasecomputemode == "Spikesorting":
		...
		if st.session_state.releasecomputemode == "DeepLabCut Training":

			# just check spikesorting table
			subtables = {key: val for key, val in tables.items() if key == "LabelledFrames"}
