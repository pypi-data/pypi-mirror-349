                ...
                if st.session_state.mode == "Label Frames":
                    if name not in st.session_state.dlc_folder.keys():
                        ...
                        if st.button("Label frames"):
                            conf_path = Path(conf).as_posix()
                            selected_vid_path = Path(selected_vid).as_posix()
                            command = f"""import deeplabcut; deeplabcut.label_frames('{conf_path}')"""
                            st.text("")
                            st.write(
                        ...
                        if st.button("Upload results"):
                            ...
                            future = dlcup_pool.submit(
                                upload_dlc,
                                key,
                                st.session_state.dlc_folder[name],
