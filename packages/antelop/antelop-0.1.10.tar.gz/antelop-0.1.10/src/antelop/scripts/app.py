...
from antelop.connection.transaction import clear_cache
...
def main():
    ...
    if not validate_config(config_file):
        st.warning("Config file is not valid. Please check the folders in the config file.")
        config.show(None, None)
        st.stop()
    ...
    if st.session_state.authentication_status:
        try:
            ...
            with st.sidebar.container():
                ...
                for i in range(6):
                    st.text("")
                _, col, _ = st.columns([0.2, 0.7, 0.1])
                with col:
