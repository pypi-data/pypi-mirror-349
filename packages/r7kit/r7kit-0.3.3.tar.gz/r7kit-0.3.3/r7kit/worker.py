...
import asyncio, importlib, inspect, logging, pkgutil, sys
        ...
        for obj in mod.__dict__.values():
            if inspect.isclass(obj) and (
                ...
                if id(obj) not in seen_wf:
                    workflows.append(obj); seen_wf.add(id(obj))
            elif callable(obj) and hasattr(obj, _ACTIVITY_ATTR):
                if id(obj) not in seen_act:
                    activities.append(obj); seen_act.add(id(obj))
...
# --------------------------------------------------------------------- #
class R7Worker:
    def __init__(self, package: str, *, queue: str = "default",
    ...
    async def _run_inner(self) -> None:
        ...
        activities = [
            *user_act,
            create_act, get_act, patch_act, delete_act,
        ]
        ...
        log.info("R7Worker started (queue=%s, wf=%d, act=%d)",
