...
class _Configuration:
    ...
    def __init__(self, config_json_string, logger):
        ...
        self.config_json = self.load_and_validate_json(config_json_string, self.CONFIG_SCHEMA)
        self._parse_config()
        self._validate_business_rules()

        self.logger.info("Configuration loaded and validated successfully.")

    def load_and_validate_json(self, config_json_string, schema):
        try:
    ...
    def _create_persistent_view(self):
        ...
        if self.config.is_active_col:
            definition = sa.select(*columns).select_from(self.meta.persistent_table).where(
                sa.literal_column(self.config.is_active_col) == 1)
        else:
...
class _DataManager:
    ...
    def _capture_data_change(self):
        ...
        if self.config.is_active_col:
            per_subquery_where.append(per.c[self.config.is_active_col] == 1)
    ...
    def _scd(self):
        ...
        def soft_delete(connection, table, operations, last_change_date):
            ...
            if self.config.is_active_col:
                where_conditions.append(table.c[self.config.is_active_col] == 1)
...
class ChangeDataCapture:
    ...
    def run(self):
        try:
            ...
            self.logger.exception(f"An unexpected error occurred during the synchronization run: {e}")
            raise
        finally:
