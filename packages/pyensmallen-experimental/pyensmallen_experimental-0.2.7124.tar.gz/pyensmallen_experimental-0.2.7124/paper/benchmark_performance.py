...
# -*- coding: utf-8 -*-
"""
Benchmarking Performance of pyensmallen-experimental Optimization Library

This script compares the performance of pyensmallen-experimental against other optimization
...
import time
import argparse
import pyensmallen-experimental
...
def linear_objective(params, gradient, X, y):
    """Linear regression objective function for pyensmallen-experimental"""
...
def logistic_objective(params, gradient, X, y):
    """Logistic regression objective function for pyensmallen-experimental"""
    z = X @ params
    h = expit(z)
...
def poisson_objective(params, gradient, X, y):
    """Poisson regression objective function for pyensmallen-experimental"""
    params = params.reshape(-1, 1)
    y = y.reshape(-1, 1)
    Xbeta = X @ params
...
def benchmark_linear_regression(X, y, true_params, libraries=None):
    ...
    if libraries is None:
        libraries = ['pyensmallen-experimental', 'scipy', 'statsmodels']
        if HAS_JAX:
    ...
    # pyensmallen-experimental
    if 'pyensmallen-experimental' in libraries:
        start_time = time.time()
        optimizer = pyensmallen-experimental.L_BFGS()
        result_ens = optimizer.optimize(
            lambda params, gradient: linear_objective(params, gradient, X, y),
            initial_params.copy(),
        )
        end_time = time.time()
        results['pyensmallen-experimental'] = {
            'params': result_ens,
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Run performance benchmarks for pyensmallen-experimental')
    ...
    parser.add_argument('--libraries', type=str, default="pyensmallen-experimental,scipy,statsmodels",
