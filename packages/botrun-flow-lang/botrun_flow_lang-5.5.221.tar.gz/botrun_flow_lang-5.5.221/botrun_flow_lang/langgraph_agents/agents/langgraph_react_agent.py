...
def transform_anthropic_incompatible_schema(schema_dict: dict) -> tuple[dict, bool, str]:
    """
    ...
    if problematic_key:
        ...
        if problematic_key == 'allOf':
            ...
            if options:
                 description_notes += f" 通常需要滿足以下條件之一：{'; 或 '.join(options)}。]"
            else:
                 description_notes += " 請注意參數間的選擇關係。]"


        print(f"  轉換後 schema: {json.dumps(new_schema, indent=2, ensure_ascii=False)}")
...
def _process_mcp_tools_for_anthropic(langchain_tools: List[Any]) -> List[Any]:
    ...
    logger.info(f"[_process_mcp_tools_for_anthropic] --- 開始處理 {len(langchain_tools)} 個原始 MCP 工具 ---")
    ...
    for mcp_tool in langchain_tools:
        ...
        if not hasattr(mcp_tool, 'args_schema') or not mcp_tool.args_schema:
            logger.debug(f"[_process_mcp_tools_for_anthropic] 工具 '{mcp_tool.name}' 沒有 args_schema，直接加入。")
        ...
        try:
            ...
            if hasattr(mcp_tool.args_schema, 'model_json_schema'): # Pydantic V2
                 ...
                 logger.warning(f"[_process_mcp_tools_for_anthropic] 無法獲取工具 '{mcp_tool.name}' 的 schema 字典 ({type(mcp_tool.args_schema)})，跳過轉換。")
                 processed_tools.append(mcp_tool)
                 continue

            # 進行轉換檢查
            logger.debug(f"[_process_mcp_tools_for_anthropic] 檢查工具 '{mcp_tool.name}' 的 schema...")
            new_schema_dict, transformed, desc_notes = transform_anthropic_incompatible_schema(
                copy.deepcopy(original_schema_dict) # 使用深拷貝操作
            )

            if transformed:
                mcp_tool.description += desc_notes
                logger.info(f"[_process_mcp_tools_for_anthropic] 工具 '{mcp_tool.name}' 的描述已更新。")
                if isinstance(mcp_tool.args_schema, dict):
                    logger.debug(f"[_process_mcp_tools_for_anthropic] args_schema 是字典，直接替換工具 '{mcp_tool.name}' 的 schema。")
            ...
            logger.error(f"[_process_mcp_tools_for_anthropic] 處理工具 '{mcp_tool.name}' schema 時發生錯誤: {e_schema}", exc_info=True)
            processed_tools.append(mcp_tool) # 保留原始工具

    logger.info(f"[_process_mcp_tools_for_anthropic] --- 完成工具處理，返回 {len(processed_tools)} 個工具 ---")
        ...
        try:
            ...
            if raw_mcp_tools:
                ...
                if mcp_tools:
                    ...
                    logger.debug(f"加入的 MCP 工具名稱: {[tool.name for tool in mcp_tools]}")
                else:
            ...
            logger.error(f"從 MCP Client 獲取或處理工具時發生錯誤: {e_get}", exc_info=True)
