...
class StockData:
    ...
    @timeout(100)
    def get_daily_all(self, method='default'):
        """
        获取所有股票当天的交易数据
        ...
        if method == 'default':
            ...
            try:
                ...
                try:
                    ...
                    if res.empty:
                        ...
                        return self.get_daily_all(method='akshare')
                    return res
            ...
            try:
                ...
                logger.debug(f"ProxyError，请检查代理服务器设置是否正常！")
                return None
            res.rename(columns={
                "代码": "symbol",
                "今开": "open",
            ...
            res['trade_date'] = datetime.date.today().strftime('%Y%m%d')
            logger.debug(f"akshare获取到当日所有股票交易数据：{res.shape=}")
            return res[
                ["symbol", "open", "high", "low", "close", "vol", "amount", "换手率", "市值", "流通市值", "市盈率",
                 "市净率", "trade_date"]]
        return None
