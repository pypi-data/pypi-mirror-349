...
class Model:
    ...
    def status(self):
        ...
        url = bdb_constants.BASE_URL + bdb_constants.get_model_status_url.format(self.config.provider.value)
    ...
    def predict(self, input_data:any) -> object:
        """
        ...
        url = bdb_constants.BASE_URL + bdb_constants.model_predict_url.format(self.config.provider.value, self.config._id)
...
class HuggingFaceModel(Model):
    ...
    def request(self):
        ...
        url = bdb_constants.BASE_URL + bdb_constants.model_deploy_url.format(self.config.provider.value, self.config._id)
        params = {"apiKey": self._api_key}
        response = requests.post(url, json=payload, params=params)
        if response.status_code != 200:
            Utils.handleApiCallFailure(response.json(), response.status_code)
...
class CustomModel(Model):
    ...
    def shutdown(self):
        ...
        url = bdb_constants.BASE_URL + bdb_constants.model_shutdown_url.format(self.config.provider.value, self.config._id)
        params = {"apiKey": self._api_key}
