...
class TestUsageResultItems(unittest.TestCase):
    """UsageResultItems unit test stubs"""
    ...
    def make_instance(self, include_optional) -> UsageResultItems:
        """Test UsageResultItems
            include_optional is a boolean, when False only required
            params are included, when True both required and
            optional params are included """
        # uncomment below to create an instance of `UsageResultItems`
        """
        model = UsageResultItems()
        if include_optional:
            return UsageResultItems(
                name = '',
                sims = 56
            )
        else:
