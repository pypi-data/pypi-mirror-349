...
class ErrorBudgetAlertRule(ServiceLevelsAlertRule):
    ...
    """ # noqa: E501
    metric: StrictStr = Field(description="This is the service levels metric type. Error budget alert uses <b>spent</b> or <b>burn rate</b> metric.")
    __properties: ClassVar[List[str]] = ["alertType", "metric"]

    @field_validator('metric')
    def metric_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['BURN_RATE', 'BURNED_PERCENTAGE']):
            raise ValueError("must be one of enum values ('BURN_RATE', 'BURNED_PERCENTAGE')")
        return value
