...
class AccessLogEntry(BaseModel):
    ...
    timestamp: StrictInt
    __properties: ClassVar[List[str]] = ["action", "email", "fullName", "tenantId", "tenantUnitId", "timestamp"]

    @field_validator('action')
    def action_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['GRANT_TEMP_ACCESS', 'FIRST_LOGIN', 'LOGIN', 'ACCESS', 'FAILED_LOGIN', 'LOGOUT']):
            raise ValueError("must be one of enum values ('GRANT_TEMP_ACCESS', 'FIRST_LOGIN', 'LOGIN', 'ACCESS', 'FAILED_LOGIN', 'LOGOUT')")
        return value
