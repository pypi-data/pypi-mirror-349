...
class Trigger(BaseModel):
    ...
    """ # noqa: E501
    id: StrictStr = Field(description="Trigger (Instana event or Smart Alert) identifier.")
    name: Optional[StrictStr] = Field(default=None, description="A name of the trigger (name of Instana event or Smart Alert).")
    type: StrictStr = Field(description="Instana event or Smart Alert type. The type must be one of the enum values.")
    __properties: ClassVar[List[str]] = ["id", "name", "type"]

    @field_validator('type')
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['customEvent', 'builtinEvent', 'applicationSmartAlert', 'globalApplicationSmartAlert', 'websiteSmartAlert', 'infraSmartAlert', 'mobileAppSmartAlert', 'syntheticsSmartAlert', 'logSmartAlert', 'sloSmartAlert']):
            raise ValueError("must be one of enum values ('customEvent', 'builtinEvent', 'applicationSmartAlert', 'globalApplicationSmartAlert', 'websiteSmartAlert', 'infraSmartAlert', 'mobileAppSmartAlert', 'syntheticsSmartAlert', 'logSmartAlert', 'sloSmartAlert')")
        return value
