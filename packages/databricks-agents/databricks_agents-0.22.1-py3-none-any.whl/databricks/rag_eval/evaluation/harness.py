...
def _should_clone_trace(
    ...
    is_trace_from_same_exp = trace.info.experiment_id == experiment_id

    # Do a best effort attempt to not copy the trace if it is from the same run
    trace_run_id = trace.info.request_metadata.get(
...
def log_traces_and_assessments(
    ...
    with mlflow.utils.logging_utils.suppress_logs(
        ...
        for assessment in assessments:
            ...
            if run_id is not None:
                assessment.metadata = (
                    # TODO: Replace this with the new source run ID metadata key in MLflow 3
                    {**assessment.metadata, "mlflow.assessment.sourceRunId": run_id}
                    if assessment.metadata is not None
                    else {"mlflow.assessment.sourceRunId": run_id}
                )
