...
CONFIG_PATH = os.path.expanduser("~/.astroquery_cli_lang")
...
for idx, arg in enumerate(sys.argv):
    if arg in ("-d", "--default") and idx + 1 < len(sys.argv):
        set_default_lang = sys.argv[idx + 1]
        break
if set_default_lang:
    save_default_lang(set_default_lang)
    default_lang = set_default_lang
else:
...
def main():
    completion_keywords = {
        "--install-completion", "--show-completion", "install-completion", "show-completion", "_completion"
    }
    lang = i18n.INITIAL_LANG
    default_lang = None
    set_default_lang = None
    ...
    setup_subcommands()
    if any(arg in completion_keywords for arg in sys.argv):
        app()
        return
    app()
