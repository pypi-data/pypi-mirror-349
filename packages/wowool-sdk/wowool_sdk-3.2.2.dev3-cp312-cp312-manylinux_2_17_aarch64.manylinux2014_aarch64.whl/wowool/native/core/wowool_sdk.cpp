...
namespace detail {
	...
	struct InputPool
	{
		...
		typedef std::tuple<std::string, InputProviderPtr, bool> thread_data_type;
		typedef std::shared_ptr<thread_data_type> thread_data_ptr;
	...
	InputPool::thread_data_ptr InputPool::get()
	{
		...
		thread_data_ptr ret = thread_data_ptr(new thread_data_type("", inprv, false));
