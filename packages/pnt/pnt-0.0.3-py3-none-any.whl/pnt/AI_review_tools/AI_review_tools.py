...
def _build_prompt(df: pd.DataFrame, topic: str, mode: str, filter_by: str = 'titles', num_summary_points: int = 3,
    ...
    if custom_prompt_path:
        ...
        if mode == 'filter':
            with importlib.resources.files(__package__).joinpath("base_filter_prompt.txt").open('r', encoding='utf-8') as f:
                prompt = f.read()

        else:
            with importlib.resources.files(__package__).joinpath("base_summarize_prompt.txt").open('r', encoding='utf-8') as f:
                prompt = f.read()
...
def summarize_literature(df: pd.DataFrame, summary_topic: str, **kwargs):
    ...
    **kwargs:
    ...
    if kwargs.get('provide_overall_summary', False):
        ...
        with importlib.resources.files(__package__).joinpath("overall_summary_prompt.txt").open('r', encoding='utf-8') as f:
                prompt = f.read()
