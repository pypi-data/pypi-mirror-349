...
def _build_prompt(df: pd.DataFrame, topic: str, mode: str, filter_by: str = 'titles', num_summary_points: int = 3,
    ...
    if custom_prompt_path:
        ...
        template_file = 'base_filter_prompt.txt' if mode == 'filter' else 'base_summarize_prompt.txt'
        prompt = _load_prompt(template_file)
...
def summarize_literature(df: pd.DataFrame, summary_topic: str, **kwargs):
    ...
    **kwargs:
    ...
    if kwargs.get('provide_overall_summary', False):
        ...
        prompt = _load_prompt('/Users/jacobrohde/Library/CloudStorage/GoogleDrive-jarohde1@gmail.com/My Drive/Scripts/pnt_packaging/pnt/pnt/AI_tools/overall_summary_prompt.txt')
