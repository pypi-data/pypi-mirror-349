...
class ArdhiGenericViewSet(ArdhiBaseView, GenericViewSet):
    ...
    serializer_class = None
    perm_decorator: Callable = None
    user_type = None

    def __init__(self):
        super().__init__()
        assert self.user_type is not None, 'View user type must be defined'

        if self.user_type == 'PROFESSIONAL':
            self.perm_decorator = professional_action_view
        elif self.user_type == 'STAFF':
            self.perm_decorator = officer_action_view
        else:
            self.perm_decorator = public_action_view
