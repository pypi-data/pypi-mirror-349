...
class ArdhiBaseView(ArdhiGeneralView):
    ...
    def decode_jwt(self):
        return jwt.decode(self.return_headers.get('JWTAUTH').split(' ')[1], settings.SECRET_KEY, algorithms='RS256',
                          options={"verify_signature": False}) or {}
    ...
    def return_invalid_serializer(serializer):
        return ArdhiResponse({'details': serializer.errors}, status=status.HTTP_400_BAD_REQUEST)
