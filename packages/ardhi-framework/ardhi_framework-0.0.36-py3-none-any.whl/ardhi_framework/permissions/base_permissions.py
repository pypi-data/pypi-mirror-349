...
def check_authenticated(headers):
    url = settings.SERVICE_URLS['ACL_BASE_URL'] + 'auth/' + 'status'
...
class AuthenticatedUserPermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        try:
            jwt.decode(headers.get('JWTAUTH').split(' ')[1], settings.SECRET_KEY, algorithms='RS256', options={"verify_signature": False})
            if check_authenticated(headers):
                return True
        except:
            return False
        return True
